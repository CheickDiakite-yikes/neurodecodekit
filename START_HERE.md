...
The implementation proof is deliberately null. `execute` therefore refuses
at `MARC2VR11P-F01` before any real readiness, ignored path, or private source
operation. Commit, push, and green this exact implementation in both remote
jobs, then bind that proof in a proof-only closeout before the one authorized
Stage 2 structural diagnostic. FW2, CIL1, archive payload, neural data, target,
model, prediction, score, live decoding, and scientific claims remain closed.
