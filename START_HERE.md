...
> Scientific strategy, updated 2026-08-30: read
...
> acceptance or protected work. Corrected `FMSR1-v1` is exact-green at
> `e09f6cc`, Base `99197577034`, Optional `99197577007`, and CI
> `33289147031`. It freezes five official source indexes, four exact queries, at least ten complete
...
> packet-bound `FMSR1-DISCOVERY-M0-D0` decision is exact-green at `b66a3a1`,
> Base `99316322973`, Optional `99316322922`, and CI `33333549395` on GitHub
> `main`. Generated-only `FMSR1-DISCOVERY-M0-G1` is exact-green at `e92e9c2`,
> Base `99327605264`, Optional `99327605346`, and CI `33337741165`. It passes two deterministic
> 17-root mock-HTTP replays and 25 adversarial refusals in
> 0.0161428339779377 seconds at 52,510,720-byte peak RSS, with zero real
> network, payload, model, score, or claim operations. Independent review found
...
## Latest Irreversible Result: Ofner Range Header Consumed At Transport H0
...
Start with `registries/current_research_frontier.v18.json`, the machine-readable
...
R1-G registration, rejected first implementation, corrected local durability
successor, and next transition. Versions 17, 16, 15, 14, 13, 12, 11, 10, 9,
and 7 remain immutable predecessors.
...
Exact refresh `85400fe` passed both required jobs in CI `33128665324` and its
proof closeout passed at `171d615` / CI `33129343388`; both are on GitHub
`main`. The proof binds three exact artifacts / 23,934 bytes and performs no
neural-data or claim operation.

Then read `docs/COMMUNICATION_EEG_TESSCCO_SOURCE_READINESS_2026_08_27.md`.
The paper-cited Kaggle dataset DOI resolves to dataset ID `9993149`, but the
...
Exact decision `9c3489c` passed both required jobs in CI `33069830931`.
...
Read the generated-only Stage Q implementation and result next:

1. `docs/BNCI_2014_001_STAGE_Q_IMPLEMENTATION.md`;
2. `registries/bnci_2014_001_stage_q_implementation.v0.json`; and
3. `registries/bnci_2014_001_stage_q_generated_result.v0.json`.

The one generated qualification passed and is consumed. It processed
40,069,562 generated bytes, six task runs, and 288 trials in 3.657 seconds at
...
VR37A registration `a677e7abd2b89e92bb7bcc3f823a3493c6a32ad0`
passed Base job `97226913287`, Optional job `97226913421`, and CI
`32652807264` before implementation. Its sole generated qualification passed
24 paths with G1 and R1-R5 each appearing four times, exactly 24 unchanged
VR35A calls, 87 direct refusals, exact replay, and zero source mutation or
retention.

The pass processed 10,606,968 generated bytes in 2.4954585829982534 seconds
at 49,266,688-byte peak RSS. It distinguishes baseline, single-cell contiguous
and noncontiguous extensions, multi-cell pure surplus, mixed surplus/deficit
...
VR37P is now an all-false Tier C request for a future two-stage sequence:
generated/mock fixed-path qualification first, then only after its own green
implementation and proof closeout one route-only private topology read. The
...
called VR33A once, called VR31A once, and returned `MARC2VR34P-R2`. Runtime
was 10.147974541061558 seconds at 26,935,296-byte peak RSS under one thread,
...
difference magnitude, private value, participant, selection, and cohort remain
unavailable. No cohort was frozen. Unlike consumed VR32P, this invocation
...
and both matching machine records. Registration
...
The one measured generated-only qualification returned `MARC2VR31A-G1`.
All 32 paths passed with 32 unchanged VR29A calls, eight R1 direction
comparisons, one immutable threshold predicate, and exact G1/G2/R1/R2/R3
counts 4/4/4/4/16. It also passed 78 direct refusals, exact replay,
order-invariant route distribution, source immutability, zero observed-count
retention, and zero retained output.

The pass processed 14,137,216 generated bytes in 2.8035786249674857 seconds at
39,174,144-byte peak RSS. The canonical aggregate report was 2,957 bytes.
Every private, ignored-path, readiness, archive, neural, target, model, score,
network, FW2/CIL1, hardware, other-project, and claim counter was zero.
...
`97165896610`, and CI `32627856478`. The proof-only closeout now binds both
...
first. Proof-only closeout `7ce2c5e7f7dde15dfe1dfafa35058613ae09b016`
...
consumed at `MARC2VR30P-R1`. Runtime was 10.091467417078093 seconds at
30,752,768-byte peak RSS under one CPU thread, one worker, and one numerical
job. Network and new-payload bytes were zero.

R1 means the filtered eligible total differs from the registered public total
of 195. It does not reveal the observed total, direction, magnitude, failed
private value, row, path, identity, participant, selection, or cohort. Because
the route is ordered, it also does not establish whether the later participant-
session distribution check would pass. No cohort was frozen.

Exact combined readiness, marker, and report bytes were not returned and were
not recovered through private inspection; the 1,246-byte canonical aggregate
return is the only reconstructed output measurement. Post-execution free disk
was 113,453,273,088 bytes.
...
root. The next safe task is an artifact-only and generated-only direction
discriminator for below- versus above-expected eligible totals. Any new
private read remains a new Tier C packet and decision. FW2/CIL1, archive and
neural payload, targets, models, scores, devices, streams, releases, and
scientific claims remain closed.

## Generated Evidence: MARC2-VR29A Remotely Closed

Registration `fcd088cc2eef6556f36ed596c6d9bb6c7ee9d7c3` passed CI
`32618866986` before implementation. The one measured generated-only
qualification returned `MARC2VR29A-G1`: all 32 paths passed with 32 unchanged
VR25A calls, 16 additional unchanged VR2 eligible-filter calls,
G1/G2/R1/R2/R3 counts 4/4/8/8/8, 77 direct refusals, exact replay, order-
invariant route distribution, source immutability, and zero retention.

The pass processed 14,137,216 generated bytes in 2.2789369999663904 seconds at
37,371,904-byte peak RSS and retained zero. Exact implementation
`2e73c9176d243b5deccbf8416bb59fdf053ba762` passed CI `32620018855`; exact
proof-only closeout `80badb9c1410c1661403aae966b1ea31fa0a45f1` passed CI
`32620685817`. This proves only generated discrimination of eligible-total
versus participant-session distribution arithmetic. It does not identify the
consumed private R1 subclass, establish a cohort, access neural data, or
establish decoding performance.
...
Registration `cee91b0` passed both jobs in CI `32596045581` before
implementation. The additive standard-library VR23A adapter then passed all
24 frozen generated paths. G1 and R1-R5 each appeared four times across six
cases, two orders, and two replays; unchanged VR20A was called 24 times;
diagnostic/upstream route disagreements and source mutations were zero; and 62
direct refusals passed. Static proof bound two VR20A F06 wrapper sites and
seven VR2 helper reasons while proving two defensive branches non-independent.

The pass processed 10,603,766 generated bytes in 2.412034 seconds at
39,944,192-byte peak RSS and retained zero output. Exact implementation
`9e1b121` passed both jobs in CI `32596999769`; proof-only closeout `7d6b6c5`
then passed both jobs in CI `32597604907` without repeating qualification or
touching private state.

The all-false `MARC2-VR24P` request now binds 16 tracked inputs totaling
224,299 bytes. It proposes a future generated fixed-path wrapper and, only
...
only the unchanged two-stage sequence by reference. Exact decision `53bc114`
passed both jobs in CI `32599996090` before Stage 1.

The generated fixed-path wrapper passes 24/24 paths: G1 and R1-R5 each
four times, 24 exact VR23A calls, 24 nested unchanged VR20A calls, deterministic
replay, and 130 direct refusals. It processed 10,603,766 generated bytes in
3.135488 seconds at 48,152,576-byte peak RSS and retained zero output. It has
no private cohort-manifest surface. Exact implementation `a2e19ba` passed Base
job `97099725036`, Optional job `97099725145`, and CI `32601257065`. Read the
...
VR13A freezes and generated-qualifies the next safe question without reopening
that evidence. It partitions the remaining first-failure surface into seven
ordered classes:
...
taxonomy/eligibility mismatch. Registration `1177174` passed both jobs in CI
`32424688012` before implementation. The generated matrix then exercised one
success control plus all seven classes in two orders and two replays for 32
unchanged VR12A calls. Every route appeared four times, both replays matched,
54 direct refusals passed, and zero generated output was retained.

The measured pass processed 13,741,736 generated bytes in 2.401634 seconds at
36,978,688-byte peak RSS and emitted 5,514 aggregate bytes. No private path,
archive, neural payload, target, model, score, FW2, or CIL1 operation occurred.
Exact implementation `63a0b8e` passed Base job `96610793887`, Optional job
`96610793714`, and CI `32426975815`. The proof-only closeout repeats no
qualification and performs no private operation. Tier A may now prepare one
...
Registration `5607fe8` passed Base job `95263869003`, Optional job
`95263869149`, and CI `31987093865` before implementation. Generated route
`MARC2VR8B-G1` now sends all 1,227 entries through the exact parser and live
manifest producer before VR2/VR6. Canonical and reversed success plus forced
F02/F03/F04 cases preserve only outer `MARC2VR6-F02` and the exact nested VR2
code. Two full replays pass all 16 gates and 29 direct refusals over 4,650,480
generated bytes in 2.421215 seconds at 59,310,080-byte peak RSS with zero
retention and zero forbidden operations.
...
generated implementation began. Now read
`docs/MARC_2_F03_PREDICATE_DECOMPOSITION_IMPLEMENTATION.md` and
`docs/MARC_2_F03_PREDICATE_DECOMPOSITION_RESULT.md`. Local route
`MARC2VR10A-G1` verifies all 20 AST signatures and sends one full-scale witness
for each unresolved class through the exact parser, producer, VR2, and VR6.
Across two orders and two replays, four controls succeed and all 20 witness
paths return outer F02 plus nested F03. The pass used 6,979,708 generated bytes
in 1.8363693330029491 seconds at 45,072,384-byte peak RSS, retained zero, and
passed 47 direct refusals. All 31 focused and 4,042 complete local tests pass.
Exact implementation `84103a5` passed Base job `95295212461`, Optional job
`95295212440`, and CI `31998811585`. This result narrows the engineering
...
That discriminator is now implemented locally as `MARC2-VR10B`. Read
...
Registration `d642eae` passed Base job `95308775711`, Optional job
`95308775577`, and CI `32003674374` before implementation. Then read
...
The ordered first-match classifier maps P03, P15, P16, P18, and P19 to R1-R5;
G1 is generated-control only. Across six exact-parser cases, two orders, and
two replays, every route appeared exactly four times. The pass completed 24
parser/producer paths, 24 VR6 calls, 24 discriminator calls, and 29,448 parser-
entry visits; it passed 60 direct refusals, used 6,979,708 generated bytes in
2.725759166991338 seconds at 44,564,480-byte peak RSS, emitted 7,515 aggregate
bytes, and retained zero. Thirty-two focused and all 4,074 base tests pass with
204 expected skips. Exact implementation/result `61bb801` passed Base job
`95320325187`, Optional job `95320325136`, and CI `32007641751`; no
qualification or private operation was repeated for proof closeout.
...
unchanged two-stage packet. Decision `4fa2771` passed Base job `95414004791`,
Optional job `95414004814`, and CI `32038683203` before Stage 1 began.

Now read `docs/MARC_2_F03_PRIVATE_DISCRIMINATOR_IMPLEMENTATION.md`, its
implementation registry, and its test. The generated/mock wrapper passes 24
exact VR6/VR10B paths, one temporary exact-size fixed-path state-machine pass,
and 81 direct refusals. Each G1/R1-R5 route appears four times. The measured
qualification used 7,398,463 generated bytes, 3.424092 seconds, 63,619,072-byte
peak RSS, 7,268 aggregate bytes, and zero retained output or forbidden
operations. The clean base suite passes 4,117 tests with 204 expected skips.
...
4. Read `docs/MARC_2_CONFOUND_TRIANGULATION_RESEARCH.md`, its registry,
   ...
   Then read `docs/MARC_2_FREEWILL_PREFIX_SELECTION_IMPLEMENTATION.md`,
   `docs/MARC_2_FREEWILL_PREFIX_SELECTION_SYNTHETIC_RESULT.md`, and their
...
10. Read `docs/LOOP_29_PRIMARY_SOURCE_RESEARCH.md` for the local-first scalp
...
11. Read `docs/LOOP_53_PRIMARY_SOURCE_RESEARCH.md`,
   ...
   freeze before generated target delivery. Portability correction `af7488a`
   passed both jobs in CI `31451262840` before one registered generated
   closeout passed all 15 gates in 5.024801 seconds at 257,130,496-byte peak
...
rerun is authorized. Loop 26 planning at `03605c5` has advanced to a green
shared preregistration at `881145d`: a 2,908-parameter causal candidate, a
2,884-parameter linear comparator, 21 fits, 24 target-blind inferences, six
priors, 31 frozen prediction sets, the complete encoder-attribution matrix,
the bounded `8, 16, 24, 32, 44, 55` curve, and all 64 paired sign assignments
over six validation sentences. The request remains unauthorized. No real-cache
value, target delivery, model run, training run, prediction, or validation
score occurred. The next decision is the exact sentence in
`docs/LOOP_26_AUTHORIZATION_PACKET.md` or hold.
Loop 29 planning research records scalp EEG as the immediate local-first lane,
OPM-MEG as the same-modality partner/lab lane, and cryogenic MEG as the
scientific reference. Its 15 requirements, four modality profiles, six
qualification levels, 12 future packet gates, and 24 false authorization flags
are machine checked. The preferred 5,000,000,000-byte and absolute
10,000,000,000-byte incremental storage limits are capacity only. S20 plus S25
...
24 refusal IDs, and 19 false authorization flags. The encoder conditions now
share the green Loop 26 preregistration; the language-model extension and all
execution remain closed. No protected cache, target, checkpoint, model,
training, validation, LLM, Neuro Token, S20, S25, stream, or device operation
exists. Even a future
...
Loop 26/31/33 validation event. The bounded curve now shares the green Loop 26
preregistration, but execution remains unauthorized. The current rows support no physical-
...
Loop 34 planning research separates seven confidence semantics, eight score and
control roles, and recommended fresh synthetic `128/64/256`
calibration/selection/final counts. The six real validation rows stay reserved
for the shared Loop 26/31/33 event and cannot fit and independently qualify
confidence. The experiment remains `Not Started`; all 26 authorization fields
are false, confidence is unavailable, and fixture, confidence-fit, target,
scoring, product-confidence, real-data, and download work is unauthorized.
Loop 35 planning research freezes ten confound classes, nine future synchronized
stream classes, 13 conditions, three stages, 24 gates, 32 refusals, and 31 false
authorization fields. Current S21/S7 evidence lacks a fresh complete peripheral-
control set. The maximum future local claim is incremental brain-sensor
...
Started`. Loop 31's encoder-only matrix is frozen inside the shared Loop 26
event, while its language-model/NeuroToken extension remains unpreregistered;
no Loop 31 execution exists. Loops 32-44 remain at their documented planning
or artifact-only boundaries.
...
`registries/loop50_research_boundary.v0.json`. The design freezes global text
grouping, five historical S21 out-of-fold folds, a 16-group S24 development
...
live endpoint. Contract `cf63043` passed Base Python job `93837415016` and
Optional Neuro Readers job `93837415174` in CI `31508903399` before the
generated-only implementation was written. Read
`docs/MARC_1_FREEWILL_CENTRAL_DIRECTORY_IMPLEMENTATION.md` and
`registries/marc1_freewill_central_directory_implementation.v0.json`. The
module has only `plan`, generated `qualify`, and aggregate `inspect`; it has no
network opener, real archive path, member reader, or `execute` mode. It
materializes 280,249 generated bytes, validates the 128-KiB tail and
148,910-byte directory, inventories all 18 entries, reconciles complete ZIP64,
keeps exact names and offsets private, replays both transport paths, and
refuses all 32 mutations. The latest development run used 0.007407 seconds,
  26,181,632-byte peak RSS, and 11,573 temporary output bytes, all removed. It
  had no scientific value. Exact implementation `211fd78` then passed Base
  Python job `93846584402` and Optional Neuro Readers job `93846584527` in CI
  `31511626051` before one registered closeout. Read
  `docs/MARC_1_FREEWILL_CENTRAL_DIRECTORY_RESULT.md` and
  `registries/marc1_freewill_central_directory_result.v0.json`. The run passed
  `MARC1CDG-R1` in 0.006544 seconds at 27,131,904-byte peak RSS, with 280,249
  generated input bytes, 11,574 temporary output bytes, 18 entries, 32/32
  ...
  Contract `8f64ccb` passed Base Python job `94180673330` and Optional Neuro
  Readers job `94180673125` in CI `31616551270` before the
  [MARC1-SA1 implementation](docs/MARC_1_SOURCE_AWARE_INVENTORY_ATTESTATION_IMPLEMENTATION.md).
  The generated attestor reached `MARC1SA-G1` with all six family routes,
  52/52 refusals, and 25/25 gates. It processed 732,811 generated bytes and
...
generated preflight. That preflight caught a preregistration error: the two
already-supported controls pass both VR15A and VR16A, while only the four
extended-width variants move from VR15A R15 to VR16A success. All six preserve
one semantic selection digest.

VR17A is therefore parked at `MARC2VR17A-P01`; its full equivalence and residual
matrices did not run. The measured preflight used 2,651,670 generated bytes,
0.6208636660012417 seconds, 33,390,592-byte peak RSS, and zero retained output
or private operations. Next: preregister corrected `MARC2-VR17B` with the
observed 2-control/4-repair mapping. No real cohort or neural evidence exists
in this lane, and FW2/CIL1 remain closed.

Corrected `MARC2-VR17B` is now preregistered against green parked-result commit
`48775bb` / CI `32468221097`. It freezes the observed two G1-to-G1 controls and
four R15-to-G1 repairs before replaying the full generated equivalence and
four-class residual matrices. Eight focused and 4,550 complete dependency-
...
Registration `a34896d` passed both jobs in CI `32470828824` before VR17C
implementation. The generated-only classifier now passes 24 paired
equivalence paths, 20 residual paths, all four hypotheses, and 50 direct
refusals. G1 and R1-R4 each appear four times under both orders and two exact
replays.

The measured pass used 19,213,944 generated bytes in 3.7416582500445656
seconds at 36,569,088-byte peak RSS, emitted 2,719 aggregate bytes, and
retained zero. Twenty focused and all 4,576 dependency-light tests pass with
...
generated fixed-path wrapper passed 20 matrix paths, one temporary success
rehearsal, and 82 refusal checks. G1 and R4-R7 each appeared exactly four
times. The pass used 9,037,650 generated bytes in 1.4131330830277875 seconds
at 37,879,808-byte peak RSS and retained zero.

All 4,620 dependency-light tests pass with 204 expected skips. Immediate gate:
commit, push, and green the exact implementation/result, then green a separate
...
Registration `9365b0f` passed both jobs in CI `32480420157` before
implementation. VR19A then passed all 32 generated paths, all four hypotheses,
and 40 direct refusals. G1/R1/R2 counts were exactly 4/16/12 across two orders
and two replays.

This tightens the real consumed structural result: under exact VR16A code, R4
means the task token was not exactly lowercase `freewill`, not a generic
identity failure. The token itself remains unavailable. The pass used
13,748,732 generated bytes, 1.759938333008904 seconds, 51,429,376-byte peak
RSS, 2,326 output bytes, and zero retention.
