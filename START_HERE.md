...
`32618866986` before implementation. Read the VR29A preregistration,
implementation document, contract, implementation registry, and result
registry first.

The one measured generated-only qualification returned `MARC2VR29A-G1`. All
32 paths passed with 32 unchanged VR25A calls, 16 additional unchanged VR2
eligible-filter calls, G1/G2/R1/R2/R3 counts 4/4/8/8/8, two independent
...
37,371,904-byte peak RSS under one CPU thread, one worker, and one numerical
job. It emitted 2,880 aggregate bytes and retained zero. Raw-data reads,
