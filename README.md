...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR24P completed one proof-separated 418,755-byte target-free structural pass and consumed at `MARC2VR24P-R2`, localizing the blocker to complete-bundle count arithmetic against the public 238-bundle assumption. The observed count and private details were not retained, no cohort was frozen, and no neural access occurred, so FW2 remains closed |
