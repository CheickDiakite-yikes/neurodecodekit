...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR22P completed one proof-separated 418,755-byte target-free structural pass and consumed at `MARC2VR22P-R4`, excluding F07 and narrowing the blocker to VR20A F06. VR23A separated all five reachable public F06 classes. VR24P generated Stage 1 passes 24/24 paths with 130 direct refusals and zero retained output, and exact implementation `a2e19ba` is remotely green. Its proof-only closeout is pending its own CI before the sole private structural read. The real F06 class remains unknown, no cohort was frozen, and no neural access occurred, so FW2 remains closed |
