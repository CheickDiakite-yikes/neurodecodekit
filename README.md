...
| MARC-1 positive-control preparation | The standards-aligned `MARC1-HT1A` wrapper passed 21/21 generated gates and then accepted one live 2,917-byte uncoded metadata body with absent `Content-Encoding`, resolving the prior transport blocker | The consumed run parked at `MARC1HTL-F04` because the live file-list row count differed from the frozen 55-row contract. The actual rows/count were not retained; no participant, payload, signal, target, model, score, or scientific claim exists from this lane. This remains the same thought-to-text path, not a pivot |
