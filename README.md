...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | After proof-gated narrowing, the sole VR30P pass established that the filtered eligible total differs from 195 without exposing a count or cohort. Generated VR31A then passed 32/32 paths and 78 refusals, proving a privacy-preserving below-versus-above direction interface. The real direction is still untested, and no neural payload, target, model, prediction, or score was accessed |
