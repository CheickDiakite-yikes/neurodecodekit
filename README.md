...
| MARC-2 confound triangulation | Five ordered work orders test Freewill target choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR12P read 418,755 target-free structural bytes once and consumed at aggregate `R4`: the run-index repair was insufficient, but no private detail or cohort was retained. VR13A now freezes seven residual identity/task/companion classes and a 32-call generated discriminator matrix. It is registration, not a neural run; FW2 remains closed until generated proof and a separate private cohort gate succeed |
