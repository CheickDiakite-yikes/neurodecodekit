...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR25A passed 40/40 generated paths and 72 direct refusals: known non-selected count drift preserved one exact 16-subject/96-bundle selection, while every eligible drift, unknown participant, and incomplete bundle refused. VR26P's request and proof are remotely green, and fresh packet-bound authorization is recorded. The decision is still inert until its own CI is green; no real cohort or neural access exists yet, so FW2 remains closed |
