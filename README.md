...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | The one-shot structural lane reached `VR18P-R4` without a cohort; primary-source review then corrected inherited `task-freewill` to published `task-reachingandgrasping`. VR20A passes 20 generated source-exact paths and 53 refusals. Decision `49b0f4b` and exact VR20P Stage 1 implementation `81784e7` are remotely green; Stage 1 passes 24/24 generated fixed-path cases, four copies each of G1 and R2-R6, 24 VR20A calls, and 91 direct refusals with zero retention. The proof-only closeout must still become remotely green before one private structural read. No real cohort or neural payload exists yet and FW2 remains closed |
