...
| MARC-2 confound triangulation | Five ordered work orders test Freewill target choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR11P localized the structural blocker to P15. VR12A's generated repair passed 12/12 paths and 36/36 refusals. Green decision `b0f251a` then opened only VR12P Stage 1: its generated fixed-path wrapper passed 12/12 cases and 61 refusals in 0.676 sec at 31.3 MiB RSS, with 213 KiB peak temporary output and zero retention. The real executor remains proof-gated before readiness; no cohort, payload, neural target, model, prediction, or score is available, and FW2 remains closed |
