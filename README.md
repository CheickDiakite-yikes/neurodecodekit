...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR25A passed 40/40 generated paths and 72 direct refusals. The one proof-gated VR26P real structural pass then consumed at R5 without a cohort. Generated VR27A passed 20/20 paths and 57 refusals, proving the interface can separate eligible-inventory drift from unknown-participant taxonomy. VR28P now freezes an all-false request for one future aggregate-only discriminator; it authorizes no private or neural access and establishes no scientific result |
