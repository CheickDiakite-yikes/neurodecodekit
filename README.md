...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR22P completed one proof-separated 418,755-byte target-free structural pass and consumed at `MARC2VR22P-R4`. This excludes F07 and narrows the blocker to VR20A F06. VR23A now freezes a 24-path generated decomposition of five reachable F06 classes while proving two defensive guards redundant. The real run used 0.056 seconds and 30.9 MB peak RSS with zero neural, target, model, score, or network operations. No cohort was frozen, so FW2 remains closed |
