...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR25A passed 40/40 generated paths and 72 direct refusals. The one proof-gated VR26P real structural pass then consumed at R5 without a cohort. Generated VR27A now passes 20/20 paths and 57 refusals, proving we can distinguish eligible-inventory drift from unknown-participant taxonomy in the interface; it does not identify the consumed real branch. FW2 and neural access remain closed |
