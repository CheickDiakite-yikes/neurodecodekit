...
| MARC-2 confound triangulation | Five ordered work orders test Freewill target choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR11P localized the structural blocker to P15, and VR12A's generated run-index repair passed 12/12 paths and 36 refusals. After exact remote proof, VR12P read 418,755 target-free structural bytes once and consumed at `MARC2VR12P-R4`: accepting one- or two-digit run indices was insufficient, and the source still refused within the frozen identity/task/companion layer. No cohort or neural payload was opened, no private failure detail was retained, no rerun is open, and FW2 remains closed |
