...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR24P localized the blocker to complete-bundle count arithmetic and is consumed. VR25A now freezes a generated selection-boundary firewall that keeps exact 195-bundle eligible-distribution and selection identity hard while converting only known non-selected count drift into a warning. No cohort or neural access exists yet, so FW2 remains closed |
