...
| MARC-1 positive-control preparation | The standards-aligned `MARC1-HT1A` wrapper passed 21/21 generated gates and 31/31 adversarial refusals across absent and case-insensitive identity encoding, exact target-free cohort replay, privacy checks, and fresh-root isolation | This is a control and attribution checkpoint on the same thought-to-text path, not a pivot. The wrapper has not yet made its one permitted metadata request, and no EEG payload, signal, target, model, score, or scientific claim exists from this lane |
