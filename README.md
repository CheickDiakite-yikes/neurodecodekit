...
| MARC-2 confound triangulation | Five ordered work orders test Freewill target choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR11P consumed one target-free structural read at aggregate R2, localizing the remaining F03 blocker to P15. VR12A now freezes a generated-only, standards-backed repair for one- or two-digit BIDS run indices while preserving source-exact names and every identity, companion, split, and storage gate. No failed private value, cohort, payload, neural target, model, prediction, or score is available; FW2 remains closed |
