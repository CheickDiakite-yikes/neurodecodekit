...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR20P performed one proof-separated 418,755-byte target-free structural read and consumed at `MARC2VR20P-R5`. VR21A separates F06 taxonomy/eligibility from F07 selection/split/rank/reservation. After packet-bound decision `197f253` passed CI, generated VR22P Stage 1 passed 12/12 fixed paths, 12 exact VR20A calls, eight conditional VR21A maps, and 121 refusals in 0.867 seconds with zero retained payload. Exact implementation and closeout proof are still required before the one private structural discriminator; no cohort was frozen, no neural payload was opened, and FW2 remains closed |
