...
| MARC-2 confound triangulation | Five ordered work orders test Freewill target choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR12P consumed at aggregate `R4` without a cohort. Remotely green VR13A separates seven residual classes plus a clean control across 32 generated calls, 54 refusals, and zero retention. All-false VR13P now requests one future read that either freezes a target-free structural cohort or returns one blocker class; it is not yet authorized, no neural payload is open, and FW2 remains closed |
