...
- **Next scientific gate, IACKD-1 Cue-to-Action Reversal:** WO9R's strongest
  ...
  **7,249,113,684 bytes**, under the approved 10 GiB ceiling, while excluding
  ...
  Registration `e42b799` passed both jobs in CI `31400450392`; request
  `ef78c06` passed both jobs in CI `31401738032`; and the maintainer's actual
  packet-bound continuation is recorded at decision `1f48b30`, green in CI
  `31403012709`. The exact implementation is now generated-fixture qualified:
  128 runs, 2,048 trials, 300 fits, and 420 target-blind prediction sets in
  **8.674020 seconds** at **263,618,560-byte peak RSS**, with **5,683,285
  generated bytes** removed afterward and zero real-data, target, dependency,
  or network operations. Its synthetic `IACKD-R2` route has no scientific
  meaning. Real acquisition remains closed until this implementation commit
  passes both CI jobs; final targets remain closed until the later aggregate
  freeze is remotely green. Read the [primary-source research](docs/IACKD_CUE_ACTION_DISSOCIATION_PRIMARY_SOURCE_RESEARCH.md),
  [frozen preregistration](docs/IACKD_CUE_ACTION_DISSOCIATION_PREREGISTRATION.md),
  and [implementation record](docs/IACKD_CUE_ACTION_DISSOCIATION_IMPLEMENTATION.md).
