...
| Loop 48 failure-localization boundary | primary blank `0.993477`; all-condition blank range `0.997146`; all 6 prefix groups cross `0.25` seed dispersion; 17 unavailable root-cause fields; 30 refusals | post-outcome artifact-only preregistration; authorization pending | Records model-fit/output-distribution instability as the leading failure phenotype while refusing to call CTC, preprocessing, signal quality, or weak neural information the proven cause |
...
| Test and release surface | 53 focused Loop 48 tests, including 10 additive discrimination invariants; the last qualified clean-host baseline ran 733 dependency-light tests with 142 expected skips and 765 optional-neuro tests with 22 expected skips | Contract CI `29431318268`/`29431347801` and request/portfolio CI `29433294092`/`29433297546` passed; the additive discrimination milestone awaits its own clean-host qualification | Makes the consumed negative result, research contracts, target isolation, freeze binding, access order, tamper checks, and authorization boundaries executable and reviewable without hiding environment-specific timing evidence |
