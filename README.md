...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR25A passed 40/40 generated paths and 72 direct refusals. Its proof-gated VR26P wrapper then passed 40/40 fixed-path cases and 106 direct refusals: 20 accepted variants shared one exact 16-subject/96-bundle selection even when the compatibility warning changed. Stage 1 is remotely green and its proof-only closeout is prepared; no real cohort or neural access exists yet, so FW2 remains closed |
