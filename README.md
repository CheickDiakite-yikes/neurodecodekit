...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR25A passed 40/40 generated paths and 72 direct refusals. The one proof-gated VR26P real structural pass then consumed at R5: participant taxonomy or exact eligible inventory refused, so no cohort was frozen. VR27A now freezes a generated-only test that separates eligible-inventory drift from unknown-participant taxonomy without reopening private data. FW2 and neural access remain closed |
