...
| MARC-2 confound triangulation | Five ordered work orders test Freewill target choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR12P read 418,755 target-free structural bytes once and consumed at aggregate `R4`, retaining no private detail or cohort. VR13A now passes a 32-call generated discriminator: seven residual structural classes plus a clean control each appeared 4/4 times across order and replay, with 54 refusals and zero retention. This is generated engineering proof pending remote implementation green, not a neural run; FW2 remains closed |
