...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR22P completed one proof-separated 418,755-byte target-free structural pass and consumed at `MARC2VR22P-R4`. This excludes F07 selection/split/rank/reservation and narrows the remaining blocker to VR20A F06: source-domain totals versus taxonomy/eligibility arithmetic. Runtime was 0.056 seconds at 30.9 MB peak RSS with zero neural, target, model, score, or network operations. No cohort was frozen, so FW2 remains closed |
