...
| MARC1-CD1A decision routing | 12 decision invariants; 168 focused tests; 2,378 optional-neuro tests with 35 expected skips; 0 public requests | Request `950796d` passed CI `31513578445`; the maintainer's exact 60-byte continuation is hash-bound in a separate decision that still requires its own green CI | Authorizes only the staged wrapper-then-one-audit sequence after two green milestones; adds no archive-content or neural evidence |
| MARC1-CD1A live-wrapper qualification | 280,249 generated bytes; 18 entries; 32 parser plus 8 wrapper refusals; 14/14 gates; 12,182 output bytes; 0 network calls | Decision `624cc4e` passed CI `31519016891` before implementation; generated `MARC1CDL-G1` passed in 0.006050 sec at 40.8 MB peak RSS | Adds proof-gated, storage-safe three-range inventory mechanics; the exact wrapper still needs green CI before one public audit, and no neural claim is added |
...
- **MARC-1 is the next scientific effect lane:** WO9R is a real positive
  ...
  A public request remains closed until the exact wrapper is committed,
  pushed, and green; whole-download, member, neural, model, score, and
  scientific-claim operations remain forbidden.
