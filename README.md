...
| MARC-2 confound triangulation | Five ordered work orders test Freewill target choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | VR11P consumed one target-free structural read at aggregate R2, localizing the remaining F03 blocker to P15, the suffix-bearing BIDS identity class. It produced no failed value, candidate cohort, archive payload, neural target, model, prediction, or score; FW2 remains closed |
