...
| MARC-2 confound triangulation | Five ordered work orders test Freewill target choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | `MARC2-FW1C` consumed safely after one structural read exposed a source-schema mismatch. Artifact-only `MARC2-SL1` localized it to `directory` versus `central_directory`; generated adapters then replayed the frozen 16-subject, 96-run, 384-member prefix. The new LA2 executor passes 56 proof, filesystem, privacy, and composition refusals in 0.274 seconds at 38.0 MB RSS, but the retained manifest remains unopened until this exact implementation is remotely green; archive, neural, target, model, and score access remain zero |
