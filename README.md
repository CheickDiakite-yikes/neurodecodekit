...
| MARC-2 confound triangulation | A five-work-order plan is designed to test task choice, conditional EEG information beyond peripheral controls, one orthogonal cohort, and a Spanish inner-speech control ladder before any LLM can receive neural evidence | Exact-task projection established only that the eligible total is above 195. The route-only VR37P packet is remotely proven but deliberately left unexecuted because it cannot freeze a cohort. VR38A now preregisters a generated selection-sufficiency repair so the next private structural pass must freeze at least 12 participants or park Freewill; no neural payload, target, model, prediction, or score has been accessed |
