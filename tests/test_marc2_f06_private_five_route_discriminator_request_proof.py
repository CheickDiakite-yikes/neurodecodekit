...
class Marc2F06PrivateFiveRouteDiscriminatorRequestProofTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_request_remote_proof_is_exact(self):
        ...
        self.assertEqual(
            proof["commit"], "b7fca404a85c9597f61b1016c388b544ee901595"
        )
        self.assertEqual(proof["CI_run_id"], 32_598_316_430)
        self.assertEqual(proof["base_python_job_id"], 97_092_614_972)
        self.assertEqual(proof["optional_neuro_job_id"], 97_092_615_100)
    ...
    def test_every_request_artifact_matches_bytes_hash_and_git_blob(self):
        ...
        self.assertEqual(sum(row["bytes"] for row in rows), 29_829)
        for row in rows:
    ...
    def test_scope_is_unchanged_and_still_all_false(self):
        ...
        self.assertEqual(scope["fixed_input_count"], 16)
        self.assertEqual(scope["fixed_input_bytes"], 224_299)
        self.assertEqual(scope["future_private_source_bytes"], 418_755)
        self.assertEqual(scope["private_route_count"], 9)
        self.assertEqual(scope["aggregate_F06_routes"], 5)
        self.assertFalse(scope["private_detail_or_cohort_allowed"])
