...
class Marc2F06PrivateFiveRouteAuthorizationDecisionTests(unittest.TestCase):
    @classmethod
    ...
    def test_identity_parent_and_delayed_effect_are_exact(self):
        ...
        self.assertEqual(self.decision["lane_id"], "MARC2-VR24P")
        self.assertEqual(
    ...
    def test_green_request_and_proof_are_exact(self):
        ...
        self.assertEqual(request["commit"], "b7fca404a85c9597f61b1016c388b544ee901595")
        self.assertEqual(request["CI_run_id"], 32_598_316_430)
        self.assertEqual(request["base_python_job_id"], 97_092_614_972)
        self.assertEqual(request["optional_neuro_job_id"], 97_092_615_100)
        self.assertTrue(request["both_required_jobs_green"])
        self.assertEqual(proof["commit"], self.decision["authorization_parent_commit"])
        self.assertEqual(proof["CI_run_id"], 32_598_893_728)
        self.assertEqual(proof["base_python_job_id"], 97_094_035_899)
        self.assertEqual(proof["optional_neuro_job_id"], 97_094_035_982)
    ...
    def test_actual_message_is_preserved_without_scope_inference(self):
        ...
        self.assertEqual(user["sole_active_Tier_C_packet"], "MARC2-VR24P")
    ...
    def test_bound_request_and_proof_artifacts_are_exact(self):
        ...
        self.assertEqual(sum(row["bytes"] for row in rows), 40_230)
        for row in rows:
    ...
    def test_authority_is_delayed_and_payload_work_remains_closed(self):
        ...
        self.assertTrue(authority["one_VR23A_call_after_stage_1_proof_green"])
        self.assertTrue(authority["one_nested_VR20A_call_after_stage_1_proof_green"])
        self.assertFalse(authority["private_cohort_freeze_authorized"])
    ...
    def test_execution_order_preserves_both_remote_barriers(self):
        ...
        self.assertIn(
            "one_exact_private_structural_read_hash_parse_VR23A_and_nested_VR20A_call",
            order,
        )
    ...
    def test_routes_resources_and_output_firewall_are_frozen(self):
        self.assertEqual(
            ...
            [f"MARC2VR24P-R{i}" for i in range(1, 10)],
        )
        self.assertTrue(
            all(
    ...
    def test_human_decision_states_delayed_effect_and_boundaries(self):
        ...
        self.assertIn("All 24 paths and at least 80", text)
        self.assertIn("No route creates a cohort", text)
