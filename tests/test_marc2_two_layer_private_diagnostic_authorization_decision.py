...
DECISION_PATH = (
    ROOT
    / "registries"
    / "marc2_two_layer_private_diagnostic_authorization_decision.v0.json"
)
DOC_PATH = (
    ROOT / "docs/MARC_2_TWO_LAYER_PRIVATE_DIAGNOSTIC_AUTHORIZATION_DECISION.md"
)
...
class Marc2TwoLayerPrivateDiagnosticAuthorizationDecisionTests(unittest.TestCase):
    @classmethod
    ...
    def test_identity_parent_and_delayed_effect_are_exact(self):
        ...
        self.assertEqual(self.decision["lane_id"], "MARC2-VR9P")
        self.assertEqual(
    ...
    def test_green_request_and_proof_are_exact(self):
        ...
        self.assertEqual(
            request["commit"], "de8e6dcfb60d78b52429d32c6bdd5f9656ab2d58"
        )
        self.assertEqual(request["CI_run_id"], 31_992_178_980)
        self.assertEqual(request["base_python_job_id"], 95_277_554_517)
        self.assertEqual(request["optional_neuro_job_id"], 95_277_554_619)
        self.assertTrue(request["both_required_jobs_green"])
        self.assertEqual(proof["commit"], self.decision["authorization_parent_commit"])
        self.assertEqual(proof["CI_run_id"], 31_992_563_746)
        self.assertEqual(proof["base_python_job_id"], 95_278_576_871)
        self.assertEqual(proof["optional_neuro_job_id"], 95_278_576_905)
    ...
    def test_authority_is_conditional_and_neural_payload_remains_closed(self):
        ...
        self.assertTrue(
            authority["one_VR6_call_and_two_layer_route_report_after_wrapper_green"]
        )
    ...
    def test_execution_order_has_both_remote_green_barriers(self):
        ...
        self.assertIn("one_aggregate_F03_or_F04_report_without_cohort", order)
    ...
    def test_two_layer_route_firewall_is_exact(self):
        route = self.decision["two_layer_route_contract"]
        self.assertEqual(route["expected_outer_VR6_route"], "MARC2VR6-F02")
        self.assertEqual(
            route["allowed_nested_VR2_routes"], ["MARC2VR2-F03", "MARC2VR2-F04"]
        )
        self.assertFalse(route["nested_VR2_F02_allowed"])
        self.assertFalse(route["VR6_success_or_cohort_acceptance_allowed"])
        self.assertFalse(route["reason_or_private_context_retained"])
    ...
    def test_generated_matrix_and_resources_are_bounded(self):
        ...
        self.assertGreaterEqual(generated["minimum_direct_mutations"], 64)
    ...
    def test_human_decision_states_delayed_effect_and_claim_boundary(self):
        ...
        self.assertIn("There is no private manifest", text)
        self.assertIn("cohort-output file", text)
