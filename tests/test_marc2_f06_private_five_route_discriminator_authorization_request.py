...
class Marc2F06PrivateFiveRouteDiscriminatorRequestTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.request = json.loads(REQUEST_PATH.read_text(encoding="utf-8"))

    def test_request_identity_and_authority_are_all_false(self):
        self.assertEqual(self.request["lane_id"], "MARC2-VR24P")
        self.assertEqual(
            self.request["status"],
    ...
    def test_green_vr23a_proof_chain_is_exact(self):
        proof = self.request["green_predecessor_proof"]
        self.assertEqual(
            proof["implementation_commit"],
            "9e1b12139ad9cd9bcd2245a1eb74b85d7a3cbeeb",
        )
        self.assertEqual(proof["implementation_CI_run_id"], 32_596_999_769)
        self.assertEqual(
            proof["proof_closeout_commit"],
            "7d6b6c59709cf069e5b119845565bb91d1f3303b",
        )
        self.assertEqual(proof["proof_closeout_CI_run_id"], 32_597_604_907)
    ...
    def test_fixed_paths_are_distinct_and_have_no_override(self):
        ...
        self.assertTrue(paths["readiness_certificate"].endswith("vr24p/readiness.v0.json"))
        self.assertTrue(paths["output_root"].endswith("marc2_f06_private_five_route/v0"))
    ...
    def test_two_stage_order_and_nine_routes_are_frozen(self):
        ...
        self.assertEqual(
            ...
            [f"MARC2VR24P-R{index}" for index in range(1, 10)],
        )
        self.assertTrue(all(not row["private_detail_allowed"] for row in routes))

    def test_generated_stage_uses_exact_five_class_map(self):
        stage = self.request["generated_stage_requirements"]
        self.assertEqual(stage["cases"], 6)
        self.assertEqual(stage["orders"], 2)
        self.assertEqual(stage["replays"], 2)
        self.assertEqual(stage["required_paths"], 24)
        self.assertEqual(stage["VR23A_calls_per_path"], 1)
        self.assertEqual(stage["nested_VR20A_calls_per_path"], 1)
        self.assertGreaterEqual(stage["direct_refusal_minimum"], 80)
        mapping = self.request["future_adapter_contract"]["frozen_class_map"]
        self.assertEqual(len(mapping), 5)

    def test_no_cohort_or_fw2_transition_is_requested(self):
        scope = self.request["future_result_scope"]
        self.assertFalse(scope["private_cohort_manifest_allowed"])
        self.assertFalse(scope["cohort_freeze_allowed"])
        self.assertFalse(scope["FW2_eligibility_created_by_result"])
        self.assertFalse(scope["FW2_or_CIL1_authorized"])
    ...
    def test_resources_and_one_shot_semantics_are_bounded(self):
        ...
        self.assertEqual(caps["VR23A_calls"], 1)
        self.assertEqual(caps["nested_VR20A_calls"], 1)
