...
class InventoryTaxonomyPrivateDiscriminatorDecisionTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_identity_and_exact_user_message_are_bound(self):
        ...
        self.assertEqual(self.decision["lane_id"], "MARC2-VR28P")
        ...
        self.assertEqual(user["sole_active_Tier_C_packet"], "MARC2-VR28P")
    ...
    def test_request_and_proof_green_are_exact(self):
        ...
        self.assertEqual(
            request["commit"], "4e5895fc0fc8bc3cf2c91f5211406115a8e2e6d5"
        )
        self.assertEqual(request["CI_run_id"], 32_613_575_234)
        self.assertEqual(request["base_python_job_id"], 97_130_420_447)
        self.assertEqual(request["optional_neuro_job_id"], 97_130_420_507)
        self.assertTrue(request["both_required_jobs_green"])
        proof = self.decision["green_proof_closeout"]
        self.assertEqual(
            proof["commit"], "87d4b30ca846d7481c45631ec7625d500e0f9595"
        )
        self.assertEqual(proof["CI_run_id"], 32_614_149_622)
        self.assertEqual(proof["base_python_job_id"], 97_131_943_291)
        self.assertEqual(proof["optional_neuro_job_id"], 97_131_943_377)
    ...
    def test_six_bound_packet_artifacts_are_exact(self):
        ...
        self.assertEqual(sum(row["bytes"] for row in rows), 42_142)
        for row in rows:
    ...
    def test_routes_and_resources_are_frozen(self):
        ...
        self.assertEqual(
            ...
            [f"MARC2VR28P-R{index}" for index in range(1, 6)],
        )
