...
class MARC1HTTPIdentityLiveTests(unittest.TestCase):
    @classmethod
    ...
    def test_machine_gate_requires_one_thread_disk_load_and_RSS(self) -> None:
        ...
        for environ, override in zip(mutations, kwargs, strict=True):
            ...
            with self.subTest(override=override), self.assertRaises(live.LiveIdentityRefusal):
    ...
    def test_private_reader_refuses_hash_mode_symlink_and_size(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for source, size, digest in cases:
                with self.subTest(size=size, digest=digest), self.assertRaises(live.LiveIdentityRefusal):
            ...
            path.chmod(0o644)
            with self.assertRaises(live.LiveIdentityRefusal):
                live.read_locked_freewill_manifest(
                    path,
                    expected_bytes=len(payload),
                    expected_sha256=hashlib.sha256(payload).hexdigest(),
    ...
    def test_wrist_parser_freezes_schema_name_rule_anchor_total_and_target_firewall(self) -> None:
        ...
        for case, value in cases.items():
            with self.subTest(case=case), self.assertRaises(live.LiveIdentityRefusal):
    ...
    def test_corrected_content_encoding_semantics_accept_only_uncoded_bodies(self) -> None:
        accepted = (
            (None, "absent"),
            ("identity", "identity"),
            ("IDENTITY", "identity"),
        ...
        hashes = []
        for encoding, expected_state in accepted:
            opener, resolver = live.generated_http_fixture(
                self.wrist_payload,
                redirects=2,
            ...
            opener.assert_consumed()
            self.assertEqual(summary["content_encoding_state"], expected_state)
            self.assertEqual(summary["decompression_or_decoding_operations"], 0)
            hashes.append(hashlib.sha256(payload).hexdigest())
        self.assertEqual(len(set(hashes)), 1)

        refused = ("", " ", "gzip", "br", "identity, gzip", "identity, identity")
        for encoding in refused:
            response = live.FixtureHTTPResponse(
                ...
                headers={
                    ...
                    "Content-Length": str(len(self.wrist_payload)),
    ...
    def test_transport_refuses_overflow_duplicate_framing_and_private_redirect(self) -> None:
        ...
        for response in cases:
            ...
            with self.subTest(response=response), self.assertRaises(live.LiveIdentityRefusal):
                live.fetch_wrist_metadata(
                    opener,
                    resolver=lambda _hostname: ("8.8.8.8",),
                    counters=None,
    ...
    def test_generated_qualification_is_bounded_deterministic_and_private(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            for index in range(2):
                ...
                self.assertEqual(outcome.report["mutation_summary"]["passed_count"], 31)
    ...
    def test_module_is_standard_library_plus_frozen_selector_and_has_no_payload_reader(self) -> None:
        source = (ROOT / "src/neurodecodekit/datasets/marc1_http_identity_live.py").read_text(
            encoding="utf-8"
        )
        for forbidden in (
            "import mne",
    ...
    def test_additive_source_and_private_root_cannot_reuse_consumed_lane(self) -> None:
        surface = live.inspect_source_surface()
        self.assertEqual(surface["consumed_executor_imports"], 0)
        self.assertEqual(surface["required_additive_imports"], 2)
        self.assertTrue(surface["network_client_present"])
        root = Path("/tmp/generated-marc1-http-identity-test-root")
        live._assert_new_private_root(root, root / live.PRIVATE_ROOT_RELATIVE_PATH)
        with mock.patch.object(os, "lstat", side_effect=AssertionError("filesystem touched")):
            with self.assertRaises(live.LiveIdentityRefusal):
                live._assert_new_private_root(
                    root,
                    root / live.OLD_PRIVATE_ROOT_RELATIVE_PATH,
                )

        with tempfile.TemporaryDirectory() as directory:
            source_path = Path(directory) / "forbidden_import.py"
            source_path.write_text(
                "from neurodecodekit.datasets import marc1_pilot_selection_live\n"
                "from neurodecodekit.datasets import marc1_http_identity_semantics\n"
                "from neurodecodekit.datasets import marc1_pilot_selection\n",
                encoding="utf-8",
            )
            with self.assertRaises(live.LiveIdentityRefusal):
                live.inspect_source_surface(source_path)
