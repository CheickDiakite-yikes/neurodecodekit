...
class NextScientificLoopsTests(unittest.TestCase):
    ...
    def test_critical_source_neural_gate_is_strict(self):
        ...
        self.assertIn("483,540,992", loop48["future_resource_cap"])
    ...
    def test_eeg_home_and_device_claims_stay_separate(self):
        ...
        self.assertIn("loop55_ai_research_policy.v0.json", loop55["build_deliverable"])
        self.assertIn("at most four future train-inner rounds", loop55["build_deliverable"])
        self.assertIn("synthetic AI proposal policy", " ".join(loop55["controls"]))
        self.assertIn("no real proposal round", loop55["authorization_boundary"])
