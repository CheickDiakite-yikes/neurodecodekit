...
from neurodecodekit.datasets import marc2_freewill_private_selection_recovery as live
...
REGISTRY_PATH = (
    ROOT
    / "registries"
    / "marc2_live_selection_recovery_implementation.v0.json"
)
...
class Marc2LiveSelectionRecoveryImplementationTests(unittest.TestCase):
    @classmethod
    ...
    def test_identity_and_status_are_exact(self):
        ...
        self.assertEqual(
            ...
            "MARC-2-FW1C-live-selection-recovery-implementation-v0",
        )
        self.assertEqual(
            self.record["status"],
    ...
    def test_surface_is_fixed_dependency_light_and_proof_gated(self):
        ...
        self.assertFalse(surface["consumed_FW1A_import_call_edit_or_expose"])
    ...
    def test_native_registry_and_shared_proof_certificate_are_distinct(self):
        certificate = self.record["proof_certificate"]
        self.assertEqual(certificate["lane_id"], proof_record.LANE_ID)
        self.assertEqual(certificate["native_registry_lane_id"], live.LANE_ID)
        self.assertEqual(
            certificate["path"],
        ...
        self.assertEqual(records["native_record"]["lane_id"], "MARC2-FW1C")
    ...
    def test_all_ninety_refusals_are_bound(self):
        result = self.record["generated_qualification"]
        self.assertEqual(result["proof_record_mutations_passed"], 32)
        self.assertEqual(result["inherited_selector_mutations_passed"], 40)
        self.assertEqual(result["wrapper_mutations_passed"], 18)
        self.assertEqual(result["total_mutations_passed"], 90)
        self.assertEqual(
            tuple(result["proof_record_mutation_order"]),
            proof_record.ORDERED_MUTATIONS,
        )
        self.assertEqual(
    ...
    def test_measured_qualification_is_within_every_cap(self):
        ...
        self.assertGreater(measured["generated_input_bytes"], 0)
        self.assertGreater(measured["combined_output_bytes"], 0)
    ...
    def test_verification_records_local_passes_and_pending_remote_CI(self):
        ...
        self.assertGreaterEqual(verification["focused_functional_tests"], 27)
        self.assertGreaterEqual(verification["focused_registry_tests"], 15)
        self.assertGreaterEqual(verification["complete_base_tests"], 3_200)
        self.assertEqual(verification["complete_base_skips"], 204)
        self.assertGreaterEqual(verification["complete_optional_tests"], 3_270)
