...
RESULT_SHA256 = "3c526ac52f8185f3fe29b8f3843fd808cd9646b5011e9638d6bf55f5a459153a"
DOCUMENT_PATH = (
    ROOT / "docs" / "MARC_1_PRIVACY_PRESERVING_PILOT_SELECTION_LIVE_RESULT.md"
)


class MARC1PilotSelectionLiveResultTests(unittest.TestCase):
    @classmethod
    ...
    def test_result_hash_identity_and_consumed_route_are_exact(self) -> None:
        ...
        self.assertEqual(
            ...
            "neurodecodekit.marc1_pilot_selection_live_result",
        )
        self.assertEqual(self.result["schema_version"], "0.1.0")
        self.assertEqual(self.result["lane_id"], "MARC1-P1A")
        self.assertEqual(self.result["route"], "MARC1PS-F03")
        self.assertEqual(
            self.result["status"],
            "consumed_failed_real_metadata_pilot_selection",
        )
    ...
    def test_exact_green_implementation_preceded_execution(self) -> None:
        ...
        self.assertEqual(
            ...
            "702e61377d41fd1d95939d5e4047be59e4631d4d",
        )
        self.assertEqual(proof["wrapper_CI_run_id"], 31578614616)
        self.assertEqual(proof["wrapper_base_job_id"], 94056321843)
        self.assertEqual(proof["wrapper_optional_neuro_job_id"], 94056321914)
        self.assertEqual(
            proof["implementation_registry_sha256"],
            "054ae02821b715de99b1b1ffe1eb2d3efb3a31c690fa4e18b2f31dae31a8c9ab",
        )
        self.assertTrue(proof["both_decision_jobs_green"])
        self.assertTrue(proof["both_wrapper_jobs_green"])

    def test_private_inventory_was_read_once_and_only_once(self) -> None:
    ...
    def test_public_response_failed_before_body_acceptance(self) -> None:
        counters = self.result["access_counters"]
        ...
        self.assertEqual(counters["public_Wrist_metadata_body_reads"], 0)
        self.assertEqual(counters["public_Wrist_metadata_body_bytes"], 0)
        self.assertEqual(counters["network_body_bytes"], 0)
        self.assertEqual(transport["accepted_response_bodies"], 0)
        self.assertEqual(transport["accepted_response_body_bytes"], 0)
    ...
    def test_selection_never_completed(self) -> None:
        counters = self.result["access_counters"]
    ...
    def test_resources_and_outputs_are_bounded(self) -> None:
        ...
        self.assertEqual(measurements["output_bytes"], 4_706)
        self.assertEqual(measurements["incremental_disk_bytes"], 5_159)
        self.assertEqual(
            (
        ...
        self.assertTrue(measurements["machine_gate"]["passed_before_consumed_marker"])
        self.assertGreaterEqual(
            measurements["machine_gate"]["free_disk_bytes"],
            12 * 1024**3,
        )
    ...
    def test_failure_gates_preserve_no_rerun_and_claim_boundary(self) -> None:
        ...
        self.assertIn("thought-to-text", self.result["claim_boundary"]["scientific_claim_not_established"])

    def test_result_document_is_exact_about_failure_and_same_path(self) -> None:
