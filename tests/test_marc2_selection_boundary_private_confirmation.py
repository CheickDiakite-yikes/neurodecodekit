...
class Marc2SelectionBoundaryPrivateConfirmationTests(unittest.TestCase):
    ...
    def test_module_import_is_standard_library_only_beyond_repo_modules(self):
        code = """
import sys
import neurodecodekit.datasets.marc2_selection_boundary_private_confirmation
...
"""
    ...
    def test_generated_qualification_passes_exact_matrix_and_refusal_floor(self):
        ...
        self.assertEqual(report["route"], "MARC2VR26P-G1")
        self.assertEqual(report["matrix"]["paths"], 40)
        self.assertEqual(report["matrix"]["success_paths"], 20)
        ...
        self.assertEqual(report["matrix"]["VR25A_calls"], 40)
        self.assertEqual(report["matrix"]["generated_source_content_opens"], 40)
        self.assertEqual(report["matrix"]["compatibility_true_paths"], 4)
        self.assertEqual(report["matrix"]["compatibility_false_paths"], 16)
    ...
    def test_qualification_replays_deterministically(self):
        ...
        for key in (
            ...
            "compatibility_true_paths",
            "compatibility_false_paths",
    ...
    def test_strict_json_rejects_duplicate_nonfinite_and_nonobject(self):
        for payload in (b'{"a":1,"a":2}', b'{"a":NaN}', b"[]"):
            with self.subTest(payload=payload):
                with self.assertRaises(wrapper.SelectionBoundaryPrivateConfirmationRefusal) as caught:
                    wrapper._strict_json(payload)
                self.assertEqual(caught.exception.route, "MARC2VR26P-F07")

    def test_generated_fixed_path_sequence_writes_bounded_private_outputs(self):
        source = firewall.build_generated_case("exact_public_control", "canonical")
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["report"]["route"], "MARC2VR26P-G1")
            self.assertEqual(
    ...
    def test_source_symlink_and_mode_drift_refuse(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(caught.exception.route, "MARC2VR26P-R4")

    def test_mock_private_failure_writes_only_aggregate_R4_report(self):
        payload = b'{"a":NaN}'
        identity = {
            "mode": 0o600,
            "bytes": len(payload),
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["report"]["route"], "MARC2VR26P-R4")
    ...
    def test_public_execute_refuses_before_readiness_when_proof_is_null(self):
        record = {
            "schema_name": "neurodecodekit.marc2_selection_boundary_private_confirmation_implementation",
            "lane_id": "MARC2-VR26P",
        ...
        self.assertEqual(caught.exception.route, "MARC2VR26P-F01")
    ...
    def test_green_proof_shape_is_strict(self):
        for proof in (
            ...
            with self.subTest(proof=proof):
                with self.assertRaises(wrapper.SelectionBoundaryPrivateConfirmationRefusal):
    ...
    def test_green_proof_binds_exact_artifact_set(self):
        ...
        proof = {
            ...
            "qualification_route": "MARC2VR26P-G1",
    ...
    def test_resource_and_output_caps_fail_closed(self):
        for values in (
            ...
            with self.subTest(values=values):
                ...
                self.assertEqual(caught.exception.route, "MARC2VR26P-F11")
    ...
    def test_cli_has_only_fixed_commands_and_no_path_overrides(self):
        help_result = subprocess.run(
            [
                sys.executable,
                "-m",
                "neurodecodekit.datasets.marc2_selection_boundary_private_confirmation",
                "--help",
            ],
            cwd=ROOT,
