...
DOC = (
    ROOT
    / "docs/MARC_2_EXACT_COUNT_PRIVATE_CONFIRMATION_AUTHORIZATION_PACKET.md"
)


class Marc2ExactCountPrivateConfirmationAuthorizationRequestTests(
    unittest.TestCase
):
    @classmethod
    ...
    def test_identity_and_predecessor_proof_are_exact(self):
        self.assertEqual(self.request["lane_id"], "MARC2-VR34P")
        self.assertEqual(
            self.request["status"],
        ...
        self.assertEqual(
            ...
            "22dcb05a1da90b153b715a0374eeb3673024f53c",
        )
        self.assertEqual(proof["proof_closeout_CI_run_id"], 32_636_104_743)
        self.assertEqual(proof["proof_closeout_base_job_id"], 97_185_995_887)
        self.assertEqual(proof["proof_closeout_optional_job_id"], 97_185_995_981)
    ...
    def test_all_fixed_inputs_match(self):
        ...
        for item in self.request["fixed_inputs"]:
            ...
            self.assertEqual(
                hashlib.sha256(payload).hexdigest(), item["sha256"], item["path"]
            )
            total += len(payload)
        self.assertEqual(len(self.request["fixed_inputs"]), 17)
    ...
    def test_future_interface_is_fixed_path_only(self):
        ...
        self.assertEqual(
            interface["CLI_commands"], ["plan", "qualify", "inspect", "execute"]
        )
        self.assertTrue(interface["execute_is_fixed_path_only"])
        self.assertFalse(interface["consumed_VR32P_import_or_reuse_allowed"])
    ...
    def test_private_source_is_copied_but_untouched(self):
        ...
        self.assertEqual(source["future_content_open_limit_if_ready"], 1)
        self.assertEqual(source["future_content_open_limit_if_not_ready"], 0)

    def test_exact_count_readiness_is_immutable(self):
        ...
        self.assertEqual(readiness["nonpassing_patterns"], 7)
        self.assertTrue(readiness["not_ready_consumes_invocation"])
    ...
    def test_direction_map_is_aggregate_only(self):
        ...
        self.assertEqual(discriminator["exact_real_VR31A_calls_if_ready"], 1)
        self.assertEqual(
            discriminator["frozen_map"],
            [
                {
                    "VR31A_route": "MARC2VR31A-R1",
                    "VR34P_route": "MARC2VR34P-R1",
                },
                {
                    "VR31A_route": "MARC2VR31A-R2",
                    "VR34P_route": "MARC2VR34P-R2",
                },
            ],
        )
    ...
    def test_generated_stage_and_resources_are_bounded(self):
        ...
        self.assertEqual(generated["required_paths"], 60)
        self.assertEqual(generated["VR33A_calls"], 60)
        self.assertEqual(generated["readiness_provider_calls"], 180)
        self.assertEqual(generated["readiness_sleeper_calls"], 120)
        self.assertEqual(generated["VR31A_calls"], 32)
        self.assertEqual(
        ...
        self.assertEqual(caps["private_source_content_opens_if_not_ready"], 0)
        self.assertEqual(caps["network_bytes"], 0)
        self.assertEqual(caps["new_payload_bytes"], 0)
        self.assertEqual(caps["retry_rerun_resume_count"], 0)

    def test_fresh_paths_exclude_consumed_vr32p(self):
        paths = self.request["future_fixed_paths"]
        self.assertIn("vr34p", paths["readiness_certificate"])
        self.assertIn("exact_count_private_confirmation", paths["output_root"])
        self.assertTrue(paths["fresh_readiness_parent_must_be_absent"])
        self.assertTrue(paths["fresh_output_root_must_be_absent"])
        self.assertFalse(paths["operation_on_named_consumed_path_allowed"])
        self.assertIn("MARC2-VR32P", paths["named_consumed_paths"])
