...
from neurodecodekit.datasets import marc2_r5_private_discriminator as wrapper
...
class Marc2R5PrivateDiscriminatorTests(unittest.TestCase):
    def test_decision_and_plan_are_exact_and_target_free(self):
        decision = wrapper.load_decision()
        plan = wrapper.build_plan()
        self.assertEqual(decision["lane_id"], "MARC2-VR22P")
        self.assertEqual(
            decision["user_authorization"]["actual_message_verbatim"], "continue"
        )
        self.assertEqual(plan["generated_paths"], 12)
        self.assertEqual(plan["generated_success_paths"], 4)
        self.assertEqual(plan["generated_refusal_paths"], 8)
        self.assertEqual(plan["VR20A_calls"], 12)
        self.assertEqual(plan["VR21A_map_calls"], 8)
        self.assertEqual(plan["direct_refusal_minimum"], 60)
    ...
    def test_module_import_is_standard_library_only_beyond_repo_modules(self):
        code = """
import sys
import neurodecodekit.datasets.marc2_r5_private_discriminator
...
"""
    ...
    def test_generated_qualification_passes_exact_matrix_and_refusal_floor(self):
        ...
        self.assertEqual(report["route"], "MARC2VR22P-G1")
        self.assertEqual(report["matrix"]["paths"], 12)
        self.assertEqual(report["matrix"]["success_paths"], 4)
        self.assertEqual(report["matrix"]["refusal_paths"], 8)
        self.assertEqual(
        ...
        self.assertEqual(report["matrix"]["VR21A_map_calls"], 8)
        self.assertEqual(report["matrix"]["generated_source_content_opens"], 12)
        self.assertTrue(
            all(
                len(set(values)) == 1
                for values in report["matrix"]["replay_source_hashes"].values()
            )
        )
        self.assertGreaterEqual(report["refusals"]["direct_refusals"], 60)
    ...
    def test_every_envelope_mutation_fails_closed(self):
        ...
        self.assertEqual(
            sum(counts.values()),
            len(wrapper._ENVELOPE_EXPECTED) + 2 + len(wrapper.FORBIDDEN_PUBLIC_KEYS),
        )
        self.assertGreaterEqual(sum(counts.values()), 60)
    ...
    def test_strict_json_rejects_duplicate_nonfinite_and_nonobject(self):
        for payload in (b'{"a":1,"a":2}', b'{"a":NaN}', b"[]"):
            with self.subTest(payload=payload):
                with self.assertRaises(wrapper.R5PrivateDiscriminatorRefusal) as caught:
                    wrapper._strict_json(payload)
                self.assertEqual(caught.exception.route, "MARC2VR22P-F07")
    ...
    def test_generated_fixed_path_sequence_writes_bounded_private_outputs(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["report"]["route"], "MARC2VR22P-G1")
            self.assertEqual(result["VR20A_calls"], 1)
            self.assertEqual(result["VR21A_map_calls"], 0)
            self.assertEqual(
    ...
    def test_generated_F06_and_F07_use_exact_conditional_map_once(self):
        expected = {
            "taxonomy_F06": "MARC2VR22P-R4",
            "selection_F07": "MARC2VR22P-R5",
        }
        for case, route in expected.items():
            with self.subTest(case=case):
                ...
                self.assertEqual(result["VR20A_calls"], 1)
                self.assertEqual(result["VR21A_map_calls"], 1)
                self.assertEqual(result["private_manifest_bytes"], 0)
    ...
    def test_source_symlink_and_mode_drift_refuse(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(caught.exception.route, "MARC2VR22P-R3")
    ...
    def test_mock_private_failure_writes_only_aggregate_R3_report(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["report"]["route"], "MARC2VR22P-R3")
    ...
    def test_public_execute_refuses_before_readiness_when_proof_is_null(self):
        record = {
            "schema_name": "neurodecodekit.marc2_r5_private_discriminator_implementation",
            "lane_id": "MARC2-VR22P",
        ...
        self.assertEqual(caught.exception.route, "MARC2VR22P-F01")
    ...
    def test_green_proof_shape_is_strict(self):
        for proof in (
            ...
            with self.subTest(proof=proof):
                with self.assertRaises(wrapper.R5PrivateDiscriminatorRefusal):
    ...
    def test_green_proof_binds_exact_artifact_set(self):
        ...
        proof = {
            ...
            "qualification_route": "MARC2VR22P-G1",
    ...
    def test_resource_and_output_caps_fail_closed(self):
        for values in (
            ...
            with self.subTest(values=values):
                ...
                self.assertEqual(caught.exception.route, "MARC2VR22P-F11")
        wrapper._assert_resources(46.0, 1, 1, generated=False)
        with self.assertRaises(wrapper.R5PrivateDiscriminatorRefusal):
    ...
    def test_cli_has_only_fixed_commands_and_no_path_overrides(self):
        help_result = subprocess.run(
            [
                sys.executable,
                "-m",
                "neurodecodekit.datasets.marc2_r5_private_discriminator",
                "--help",
            ],
            cwd=ROOT,
