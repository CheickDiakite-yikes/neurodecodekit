...
PROOF = (
    ROOT
    / "registries/ofner_2017_motor_imagery_acquisition_generated_qualification_proof.v0.json"
)
DOCUMENT = (
    ROOT
    / "docs/OFNER_2017_MOTOR_IMAGERY_ACQUISITION_GENERATED_QUALIFICATION_PROOF_CLOSEOUT.md"
)


class OfnerAcquisitionGeneratedQualificationProofTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.proof = json.loads(PROOF.read_text(encoding="utf-8"))

    def test_exact_implementation_commit_is_remotely_green(self) -> None:
        green = self.proof["green_implementation_commit"]
        self.assertEqual(
            green["commit"], "527dffd0cc18a5259eeaba796f38774bbf1f472c"
        )
        self.assertEqual(green["CI_run_id"], 33_267_249_838)
        self.assertEqual(green["base_python_job_id"], 99_139_395_588)
        self.assertEqual(green["optional_neuro_readers_job_id"], 99_139_395_701)
    ...
    def test_exact_artifacts_are_hash_size_and_git_bound(self) -> None:
        ...
        self.assertEqual(summary["bytes"], 58_191)
        self.assertEqual(
