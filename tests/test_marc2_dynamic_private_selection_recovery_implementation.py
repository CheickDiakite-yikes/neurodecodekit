...
PROOF_PATH = ROOT / recovery.PROOF_RECORD_RELATIVE_PATH
DOC_PATH = ROOT / "docs/MARC_2_DYNAMIC_PRIVATE_SELECTION_RECOVERY_IMPLEMENTATION.md"
...
class Marc2DynamicPrivateSelectionRecoveryImplementationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.registry = json.loads(REGISTRY_PATH.read_text(encoding="utf-8"))
        cls.proof_bytes = PROOF_PATH.read_bytes()
        cls.proof = json.loads(cls.proof_bytes.decode("ascii"))

    def test_registry_identity_and_green_decision_are_exact(self):
        self.assertEqual(
        ...
        self.assertEqual(self.registry["lane_id"], "MARC2-VR7P")
        decision = self.registry["green_authorization_decision"]
        self.assertEqual(
            decision["commit"], "a318521cf9adb057617e839ead0003d89c3cab84"
        )
        self.assertEqual(decision["CI_run_id"], 31_979_669_507)
        self.assertTrue(decision["both_required_jobs_green_before_implementation"])

    def test_generated_qualification_is_measured_and_target_free(self):
        qualification = self.registry["generated_qualification"]
        self.assertEqual(qualification["route"], recovery.SUCCESS_ROUTE)
        self.assertEqual(qualification["success_paths"], 10)
        self.assertEqual(qualification["replay_runs"], 20)
        self.assertEqual(qualification["VR6_direct_mutations"], 34)
        self.assertEqual(qualification["wrapper_direct_mutations"], 51)
        self.assertEqual(qualification["total_direct_mutations"], 85)
        ...
        self.assertEqual(
            qualification[
                "real_private_neural_target_model_score_or_hardware_operations"
            ],
            0,
        )

    def test_shared_proof_record_validates_through_exact_symbol(self):
    ...
    def test_proof_record_binds_every_new_implementation_artifact(self):
        tracked = {
        ...
        required = {
            ...
            "tests/test_marc2_dynamic_private_selection_recovery.py",
            "tests/test_marc2_dynamic_private_selection_recovery_implementation.py",
            "docs/MARC_2_DYNAMIC_PRIVATE_SELECTION_RECOVERY_IMPLEMENTATION.md",
        ...
        self.assertNotIn(recovery.PROOF_RECORD_RELATIVE_PATH.as_posix(), tracked)
        for path, digest in tracked.items():
            with self.subTest(path=path):
                self.assertEqual(sha256_file(ROOT / path), digest)

    def test_fixed_state_machine_and_caps_match_packet(self):
        ...
        self.assertEqual(identities["private_source"]["VR6_adapter_call_limit"], 1)
        cohort = identities["dynamic_cohort"]
        self.assertEqual(cohort["minimum_selected_subjects"], 12)
        self.assertEqual(cohort["maximum_selected_subjects"], 19)
        self.assertTrue(cohort["selected_count_bytes_and_hash_are_measured"])
    ...
    def test_real_execution_remains_unconsumed_and_FW2_closed(self):
        ...
        self.assertFalse(
            gate["archive_payload_neural_training_prediction_target_or_score_authorized"]
        )

    def test_document_states_both_required_boundaries(self):
        text = DOC_PATH.read_text(encoding="utf-8")
        self.assertIn("Engineering capability added", text)
        self.assertIn("Scientific claim not established", text)
        self.assertIn("85 direct refusal mutations", text)
        self.assertIn("no real or consumed `.codex_work` path", text)
