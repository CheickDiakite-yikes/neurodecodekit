...
class Marc2InventoryDistributionPrivateDiscriminatorTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_authority_and_fixed_inputs_are_exact(self):
        ...
        self.assertEqual(wrapper._verify_fixed_inputs(self.request, ROOT), 161_574)

    def test_plan_is_non_private_and_fixed_path_only(self):
        plan = wrapper.build_plan()
        self.assertEqual(plan["lane_id"], "MARC2-VR30P")
    ...
    def test_execute_refuses_before_readiness_or_private_path_operation(self):
        refusal = wrapper.InventoryDistributionPrivateDiscriminatorRefusal(
        ...
        with (
            ...
            self.assertRaises(
                wrapper.InventoryDistributionPrivateDiscriminatorRefusal
            ) as caught,
        ):
            wrapper.execute_fixed()
        self.assertEqual(caught.exception.route, "MARC2VR30P-F02")
    ...
    def test_all_generated_routes_are_exact(self):
        cases = {
            "exact_public_control": ("MARC2VR30P-G1", "MARC2VR29A-G1", 0),
            "single_session_exclusion_removed": (
                "MARC2VR30P-G2",
                "MARC2VR29A-G2",
                0,
            ),
            "eligible_bundle_removed": ("MARC2VR30P-R1", "MARC2VR29A-R1", 1),
            "eligible_bundle_added": ("MARC2VR30P-R1", "MARC2VR29A-R1", 1),
        ...
        for case, expected in cases.items():
            with self.subTest(case=case), tempfile.TemporaryDirectory() as temp:
                result = wrapper._run_generated_case(
                    case=case, order="reversed", root=Path(temp)
                )
                self.assertEqual(
                    (
                        result["route"],
                ...
                self.assertEqual(result["VR29A_calls"], 1)
    ...
    def test_generated_call_counts_and_source_immutability(self):
        original = wrapper.vr29a.discriminate_generated_source
        calls = []

        def guarded(source):
            before = wrapper.vr25a._source_bytes(source)
            result = original(source)
            self.assertEqual(wrapper.vr25a._source_bytes(source), before)
            calls.append(result)
            return result

        with tempfile.TemporaryDirectory() as temp, mock.patch.object(
            wrapper.vr29a, "discriminate_generated_source", side_effect=guarded
        ):
            for order in wrapper.ORDERS:
                for case in wrapper.CASES:
                    case_root = Path(temp) / f"{order}-{case}"
    ...
    def test_marker_precedes_source_open(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with mock.patch.object(
                ...
                result = wrapper._run_generated_case(
                    case="exact_public_control", order="canonical", root=root
                )
        self.assertEqual(result["source_content_opens"], 1)
    ...
    def test_unknown_upstream_routes_fail_closed(self):
        for route in ("MARC2VR29A-R4", "MARC2VR29A-F01", "unknown"):
            with self.subTest(route=route), self.assertRaises(
                wrapper.InventoryDistributionPrivateDiscriminatorRefusal
            ):
                wrapper._map_vr29a_route(route)
    ...
    def test_private_report_contains_only_aggregate_route(self):
        report = wrapper._private_report(
            ...
            peak_rss_bytes=1,
            readiness_samples=3,
            nested_filter_calls=1,
        )
        encoded = json.dumps(report, sort_keys=True)
        self.assertEqual(report["route"], "MARC2VR30P-R1")
