...
DOCUMENT = (
    ROOT / "docs/EEGMMIDB_UNSEEN_PARTICIPANT_METADATA_STAGE_M1_PROOF_CLOSEOUT.md"
)


class EEGMMIDBUnseenParticipantMetadataStageM1ProofCloseoutTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.proof = json.loads(PROOF.read_text(encoding="utf-8"))

    def test_exact_result_commit_was_remotely_green(self):
        green = self.proof["green_result_commit"]
        self.assertEqual(
            green["commit"], "3b343d74604b0b0d0e0732f14cf593c8e057ebbf"
        )
        self.assertEqual(green["CI_run_id"], 32716836238)
        self.assertEqual(green["base_python_job_id"], 97399780409)
        self.assertEqual(green["optional_neuro_readers_job_id"], 97399780188)
    ...
    def test_nine_artifacts_are_byte_hash_and_git_bound(self):
        ...
        self.assertEqual(summary["bytes"], 82278)
        self.assertEqual(
    ...
    def test_closeout_repeats_no_qualification_or_real_operation(self):
        ...
        self.assertFalse(scope["result_reconstructed_or_amended"])
        self.assertFalse(scope["authority_expanded"])

    def test_stage_m2_has_delayed_effect_and_payload_stays_closed(self):
        transition = self.proof["transition"]
        self.assertFalse(transition["effective_before_this_closeout_commit_remote_green"])
        self.assertTrue(
            transition[
                "stage_M2_eligible_after_exact_closeout_remote_green_under_existing_decision"
            ]
        )
        self.assertEqual(transition["stage_M2_real_invocations_maximum"], 1)
        self.assertFalse(transition["stage_M2_authorized_now"])
    ...
    def test_human_closeout_separates_engineering_and_science(self):
        ...
        self.assertIn("one sequential, body-blind `HEAD` invocation", document)
