...
RESULT_PATH = ROOT / "registries/marc2_r5_private_discriminator_result.v0.json"


class Marc2R5PrivateDiscriminatorResultTests(unittest.TestCase):
    @classmethod
    ...
    def test_exact_green_proof_chain_precedes_execution(self):
        ...
        self.assertEqual(
            ...
            "bc8ecc38374c57f6be9269874cce8d4497f83551",
        )
        self.assertEqual(proof["implementation_CI_run_id"], 32_594_134_914)
        self.assertEqual(
            proof["proof_closeout_commit"],
            "ea39319f4c2660154cf64bcb7e38aa5fce7829c5",
        )
        self.assertEqual(proof["proof_closeout_CI_run_id"], 32_594_732_196)
    ...
    def test_consumed_route_and_measured_resources_are_exact(self):
        ...
        self.assertEqual(self.result["route"], "MARC2VR22P-R4")
        resources = self.result["resources"]
        self.assertEqual(resources["input_bytes"], 418_755)
        self.assertEqual(resources["output_bytes"], 2_872)
        self.assertEqual(resources["runtime_seconds"], 0.056165415968280286)
        self.assertEqual(resources["peak_RSS_bytes"], 30_900_224)
    ...
    def test_one_shot_operation_counts_and_forbidden_counters_are_exact(self):
        ...
        self.assertEqual(operations["VR21A_map_calls"], 1)
    ...
    def test_route_ceiling_preserves_f06_uncertainty(self):
        route = self.result["route_interpretation"]
        self.assertEqual(
            route["frozen_class"],
            "VR20A_F06_source_domain_or_taxonomy_eligibility_arithmetic_refused",
        )
        self.assertEqual(route["compatible_VR20A_routes"], ["MARC2VR20A-F06"])
        self.assertEqual(route["exact_public_F06_call_site_total"], 2)
        self.assertEqual(route["excluded_VR20A_routes_for_this_execution"], ["MARC2VR20A-F07"])
        for key in (
            "failed_predicate_known",
            "failed_private_value_known",
            "private_filename_path_row_or_identity_known",
            "participant_taxonomy_selection_or_cohort_known",
            "real_cohort_frozen",
