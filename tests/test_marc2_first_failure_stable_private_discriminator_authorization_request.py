...
class Marc2FirstFailureStablePrivateDiscriminatorRequestTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.request = json.loads(REQUEST_PATH.read_text(encoding="utf-8"))

    def test_request_identity_and_authority_are_all_false(self):
        self.assertEqual(self.request["lane_id"], "MARC2-VR18P")
        self.assertEqual(
            self.request["status"],
    ...
    def test_green_vr17c_proof_chain_is_exact(self):
        proof = self.request["green_predecessor_proof"]
        self.assertEqual(
            proof["implementation_commit"],
            "fcff5140a9e8f106a42cf8c5a2a944ca1d52f42d",
        )
        self.assertEqual(proof["implementation_CI_run_id"], 32_472_572_881)
        self.assertEqual(proof["implementation_base_job_id"], 96_742_255_383)
        self.assertEqual(proof["implementation_optional_job_id"], 96_742_255_145)
        self.assertEqual(
            proof["proof_closeout_commit"],
            "820592e7a31de6c802fafa6b709506c3264179d6",
        )
        self.assertEqual(proof["proof_closeout_CI_run_id"], 32_473_172_345)
        self.assertEqual(proof["proof_closeout_base_job_id"], 96_744_045_204)
        self.assertEqual(proof["proof_closeout_optional_job_id"], 96_744_044_972)
    ...
    def test_fixed_paths_are_distinct_and_have_no_override(self):
        ...
        self.assertTrue(paths["readiness_certificate"].endswith("vr18p/readiness.v0.json"))
        self.assertTrue(
            paths["output_root"].endswith(
                "marc2_first_failure_stable_private_discriminator/v0"
            )
        )
    ...
    def test_two_stage_order_and_route_table_are_frozen(self):
        ...
        self.assertEqual(
            ...
            [f"MARC2VR18P-R{index}" for index in range(1, 9)],
        )
    ...
    def test_generated_stage_uses_exact_four_class_map(self):
        ...
        self.assertEqual(stage["VR16A_calls_per_path"], 1)
        self.assertEqual(stage["direct_refusal_minimum"], 80)
        adapter = self.request["future_adapter_contract"]
        self.assertEqual(len(adapter["frozen_failure_map"]), 4)
        self.assertEqual(adapter["exact_real_VR16A_calls"], 1)
        self.assertEqual(adapter["exact_real_VR17C_map_calls_maximum"], 1)
    ...
    def test_resources_and_one_shot_semantics_are_bounded(self):
        ...
        self.assertEqual(caps["VR16A_calls"], 1)
        self.assertEqual(caps["VR17C_map_calls_maximum"], 1)
