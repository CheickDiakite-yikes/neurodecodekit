...
PROOF_PATH = (
    ROOT
    / "registries"
    / "marc2_variable_width_private_confirmation_request_proof.v0.json"
)


class Marc2VariableWidthPrivateConfirmationRequestProofTests(unittest.TestCase):
    @classmethod
    ...
    def test_request_remote_proof_is_exact(self):
        ...
        self.assertEqual(
            proof["commit"], "619469a795a7044a4bbb77cef8986e0a7744473f"
        )
        self.assertEqual(proof["CI_run_id"], 32_461_465_238)
        self.assertEqual(proof["base_python_job_id"], 96_709_074_413)
        self.assertEqual(proof["optional_neuro_job_id"], 96_709_074_203)
    ...
    def test_every_request_artifact_matches_bytes_hash_and_git_blob(self):
        ...
        self.assertEqual(sum(row["bytes"] for row in rows), 32_234)
        for row in rows:
            path = ROOT / row["path"]
    ...
    def test_scope_is_unchanged_and_still_all_false(self):
        ...
        self.assertEqual(scope["fixed_input_count"], 15)
        self.assertEqual(scope["fixed_input_bytes"], 180_905)
