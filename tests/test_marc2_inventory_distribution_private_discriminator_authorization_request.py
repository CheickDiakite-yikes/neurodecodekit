...
class InventoryDistributionPrivateDiscriminatorAuthorizationRequestTests(
    unittest.TestCase
):
    @classmethod
    def setUpClass(cls):
    ...
    def test_identity_and_all_false_status_are_exact(self):
        self.assertEqual(
            ...
            "neurodecodekit.marc2_inventory_distribution_private_discriminator_authorization_request",
        )
        self.assertEqual(self.request["schema_version"], "0.1.0")
        self.assertEqual(self.request["lane_id"], "MARC2-VR30P")
        self.assertEqual(
            self.request["status"],
            "all_false_Tier_C_request_prepared_no_decision_no_private_access",
        )

    def test_green_VR29A_proof_is_bound(self):
        ...
        self.assertEqual(
            ...
            "2e73c9176d243b5deccbf8416bb59fdf053ba762",
        )
        self.assertEqual(
            proof["proof_closeout_commit"],
            "80badb9c1410c1661403aae966b1ea31fa0a45f1",
        )
    ...
    def test_future_paths_are_new_fixed_and_not_accessed(self):
        ...
        self.assertIn("vr30p", paths["readiness_certificate"])
        self.assertIn(
            "inventory_distribution_private_discriminator", paths["output_root"]
        )
        ...
        self.assertEqual(
            self.request["operation_counters"][
    ...
    def test_generated_stage_binds_VR29A_matrix(self):
        stage = self.request["generated_stage_requirements"]
        self.assertEqual(stage["cases"], 8)
        self.assertEqual(stage["orders"], 2)
        self.assertEqual(stage["replays"], 2)
        self.assertEqual(stage["required_paths"], 32)
        self.assertEqual(stage["VR29A_calls_per_path"], 1)
        self.assertGreaterEqual(stage["direct_refusal_minimum"], 90)
    ...
    def test_private_route_ceiling_is_binary_and_aggregate_only(self):
        ...
        self.assertEqual(
            [item["route"] for item in routes[:2]],
            ["MARC2VR30P-R1", "MARC2VR30P-R2"],
        )
        self.assertEqual(
            routes[0]["meaning"], "filtered eligible-total arithmetic differs"
        )
        self.assertEqual(
            routes[1]["meaning"],
            "participant-session distribution arithmetic differs",
        )
        self.assertTrue(all(not item["private_detail_allowed"] for item in routes))
        self.assertFalse(
            self.request["aggregate_output_firewall"]["per_item_outcomes_allowed"]
        )
