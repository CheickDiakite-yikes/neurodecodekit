...
DOC_PATH = (
    ROOT
    / "docs/MARC_2_F06_PRIVATE_FIVE_ROUTE_DISCRIMINATOR_PROOF_CLOSEOUT.md"
)


class Marc2F06PrivateFiveRouteProofCloseoutTests(unittest.TestCase):
    @classmethod
    ...
    def test_exact_remote_implementation_proof_is_bound(self):
        ...
        self.assertEqual(
            proof["commit"], "a2e19ba59d989ca27cada275ac460085cb07536c"
        )
        self.assertEqual(proof["CI_run_id"], 32_601_257_065)
        self.assertEqual(proof["base_job_id"], 97_099_725_036)
        self.assertEqual(proof["optional_neuro_job_id"], 97_099_725_145)
    ...
    def test_preproof_registry_and_artifact_set_are_exact(self):
        proof = self.record["remote_implementation_proof"]
        self.assertEqual(proof["implementation_registry_preproof_bytes"], 7138)
        self.assertEqual(
            proof["implementation_registry_preproof_sha256"],
            "9d4dc2268e22ea228fbbf76ea1ba9af15ea5955fe8c231a450f06dea9393390c",
        )
        self.assertGreater(REGISTRY_PATH.stat().st_size, 7138)
