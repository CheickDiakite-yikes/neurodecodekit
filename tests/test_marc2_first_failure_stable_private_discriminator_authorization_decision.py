...
class Marc2FirstFailureStablePrivateDiscriminatorAuthorizationDecisionTests(
    unittest.TestCase
):
    @classmethod
    ...
    def test_identity_parent_and_delayed_effect_are_exact(self):
        ...
        self.assertEqual(self.decision["lane_id"], "MARC2-VR18P")
        self.assertEqual(
    ...
    def test_green_request_and_proof_are_exact(self):
        ...
        self.assertEqual(request["commit"], "521f1de1f3141f3f970710447d072608253c2cca")
        self.assertEqual(request["CI_run_id"], 32_474_183_647)
        self.assertEqual(request["base_python_job_id"], 96_747_013_517)
        self.assertEqual(request["optional_neuro_job_id"], 96_747_013_910)
        self.assertTrue(request["both_required_jobs_green"])
        self.assertEqual(proof["commit"], self.decision["authorization_parent_commit"])
        self.assertEqual(proof["CI_run_id"], 32_474_864_890)
        self.assertEqual(proof["base_python_job_id"], 96_749_006_185)
        self.assertEqual(proof["optional_neuro_job_id"], 96_749_006_544)
    ...
    def test_actual_message_is_preserved_without_scope_inference(self):
        ...
        self.assertEqual(user["sole_active_Tier_C_packet"], "MARC2-VR18P")
    ...
    def test_bound_request_and_proof_artifacts_are_exact(self):
        ...
        self.assertEqual(sum(row["bytes"] for row in rows), 43_769)
        for row in rows:
    ...
    def test_authority_is_delayed_and_payload_work_remains_closed(self):
        ...
        self.assertTrue(authority["one_VR17C_map_lookup_after_stage_1_proof_green"])
    ...
    def test_execution_order_preserves_both_remote_barriers(self):
        ...
        self.assertIn(
            "one_exact_private_structural_read_hash_parse_and_VR16A_call", order
        )
        self.assertIn("at_most_one_VR17C_map_lookup_on_failure", order)
    ...
    def test_routes_resources_and_output_firewall_are_frozen(self):
        ...
        self.assertTrue(
            self.decision["private_route_contract"][0][
                "private_cohort_manifest_allowed"
            ]
        )
        self.assertTrue(
            all(
        ...
        self.assertEqual(generated["required_paths"], 20)
    ...
    def test_human_decision_states_delayed_effect_and_boundaries(self):
        ...
        self.assertIn("five registered residual cases", text)
