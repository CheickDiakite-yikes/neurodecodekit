...
class Marc2EligibleTotalDirectionPrivateDiscriminatorTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_authority_and_fixed_inputs_are_exact(self):
        ...
        self.assertEqual(wrapper._verify_fixed_inputs(self.request, ROOT), 92_949)

    def test_plan_is_non_private_and_fixed_path_only(self):
        plan = wrapper.build_plan()
        self.assertEqual(plan["lane_id"], "MARC2-VR32P")
    ...
    def test_execute_refuses_before_readiness_or_private_path_operation(self):
        refusal = wrapper.EligibleTotalDirectionPrivateDiscriminatorRefusal(
        ...
        with (
            ...
            self.assertRaises(
                wrapper.EligibleTotalDirectionPrivateDiscriminatorRefusal
            ) as caught,
        ):
            wrapper.execute_fixed()
        self.assertEqual(caught.exception.route, "MARC2VR32P-F02")
    ...
    def test_all_generated_routes_are_exact(self):
        cases = {
            "exact_public_control": ("MARC2VR32P-G1", "MARC2VR31A-G1", 0),
            "single_session_exclusion_removed": (
                "MARC2VR32P-G2",
                "MARC2VR31A-G2",
                0,
            ),
            "eligible_bundle_removed": ("MARC2VR32P-R1", "MARC2VR31A-R1", 1),
            "eligible_bundle_added": ("MARC2VR32P-R2", "MARC2VR31A-R2", 1),
    ...
    def test_generated_call_counts_and_source_immutability(self):
        original = wrapper.vr31a.discriminate_generated_source
        calls = []
        ...
        with tempfile.TemporaryDirectory() as temp, mock.patch.object(
            wrapper.vr31a, "discriminate_generated_source", side_effect=guarded
        ):
    ...
    def test_private_report_contains_only_aggregate_route(self):
        ...
        self.assertEqual(report["route"], "MARC2VR32P-R1")
