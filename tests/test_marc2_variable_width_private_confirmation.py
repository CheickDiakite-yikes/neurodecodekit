...
class Marc2VariableWidthPrivateConfirmationTests(unittest.TestCase):
    ...
    def test_module_import_is_standard_library_only_beyond_repo_modules(self):
        code = """
import sys
import neurodecodekit.datasets.marc2_variable_width_private_confirmation
...
"""
    ...
    def test_generated_qualification_passes_exact_matrix_and_refusal_floor(self):
        ...
        self.assertEqual(report["route"], "MARC2VR16P-G1")
        self.assertEqual(report["matrix"]["success_paths"], 24)
        self.assertEqual(report["matrix"]["VR16A_calls"], 24)
        self.assertEqual(report["matrix"]["generated_source_content_opens"], 24)
        self.assertGreaterEqual(report["refusals"]["direct_refusals"], 70)
    ...
    def test_every_envelope_mutation_fails_closed(self):
        ...
        self.assertGreaterEqual(sum(counts.values()), 70)
    ...
    def test_strict_json_rejects_duplicate_nonfinite_and_nonobject(self):
        for payload in (b'{"a":1,"a":2}', b'{"a":NaN}', b"[]"):
            with self.subTest(payload=payload):
                with self.assertRaises(wrapper.VariableWidthPrivateConfirmationRefusal) as caught:
                    wrapper._strict_json(payload)
                self.assertEqual(caught.exception.route, "MARC2VR16P-F07")
    ...
    def test_generated_fixed_path_sequence_writes_bounded_private_outputs(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["report"]["route"], "MARC2VR16P-G1")
            self.assertEqual(
    ...
    def test_source_symlink_and_mode_drift_refuse(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(caught.exception.route, "MARC2VR16P-R3")
    ...
    def test_mock_private_failure_writes_only_aggregate_R3_report(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["report"]["route"], "MARC2VR16P-R3")
    ...
    def test_public_execute_refuses_before_readiness_when_proof_is_null(self):
        record = {
            "schema_name": "neurodecodekit.marc2_variable_width_private_confirmation_implementation",
            "lane_id": "MARC2-VR16P",
        ...
        self.assertEqual(caught.exception.route, "MARC2VR16P-F01")
    ...
    def test_green_proof_shape_is_strict(self):
        for proof in (
            ...
            with self.subTest(proof=proof):
                with self.assertRaises(wrapper.VariableWidthPrivateConfirmationRefusal):
    ...
    def test_green_proof_binds_exact_artifact_set(self):
        ...
        proof = {
            ...
            "qualification_route": "MARC2VR16P-G1",
    ...
    def test_resource_and_output_caps_fail_closed(self):
        for values in (
            ...
            with self.subTest(values=values):
                ...
                self.assertEqual(caught.exception.route, "MARC2VR16P-F11")
    ...
    def test_cli_has_only_fixed_commands_and_no_path_overrides(self):
        help_result = subprocess.run(
            [
                sys.executable,
                "-m",
                "neurodecodekit.datasets.marc2_variable_width_private_confirmation",
                "--help",
            ],
            cwd=ROOT,
