...
DOC_PATH = ROOT / "docs/MARC_2_MACHINE_STABLE_PRIVATE_RECOVERY_RESULT.md"
...
class Marc2MachineStablePrivateRecoveryResultTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.record = json.loads(REGISTRY_PATH.read_text(encoding="utf-8"))

    def test_identity_route_and_consumption_are_exact(self):
        self.assertEqual(
            self.record["schema_name"],
            "neurodecodekit.marc2_machine_stable_private_recovery_failure_result",
        )
        self.assertEqual(self.record["schema_version"], "0.1.0")
        self.assertEqual(self.record["lane_id"], "MARC2-VR4P")
        self.assertEqual(self.record["route"], "MARC2MSP-F07")
    ...
    def test_green_implementation_proof_is_exact(self):
        ...
        self.assertEqual(
            proof["commit"], "24f7379301b03c8f5eff796afb3398b11e58ece9"
        )
        self.assertEqual(proof["CI_run_id"], 31970865212)
        self.assertEqual(proof["base_python_job_id"], 95223010696)
        self.assertEqual(proof["optional_neuro_job_id"], 95223010631)
    ...
    def test_exact_cleanup_readiness_marker_and_source_open_completed(self):
        execution = self.record["private_execution"]
        self.assertTrue(execution["exact_remote_proof_validation_passed"])
        self.assertEqual(execution["expired_certificate_content_opens"], 1)
        self.assertEqual(execution["expired_certificate_bytes"], 4_551)
        self.assertEqual(execution["expired_certificate_unlinks"], 1)
        ...
        self.assertEqual(execution["VR2_adapter_calls"], 1)

    def test_adapter_refusal_preceded_cohort_and_reports(self):
        stop = self.record["stop_result"]
        self.assertEqual(stop["stage"], "VR2_structural_adapter")
        self.assertEqual(stop["aggregate_safe_reason"], "VR2 structural adapter refused")
        self.assertFalse(stop["exact_failed_predicate_available"])
        execution = self.record["private_execution"]
        self.assertEqual(execution["VR2_adapter_successes"], 0)
    ...
    def test_measurements_preserve_unavailable_values(self):
        ...
        self.assertLess(measurements["external_invocation_wall_seconds"], 650)
        for key in (
    ...
    def test_warnings_and_unavailable_fields_are_explicit(self):
        ...
        self.assertIn("adapter", unavailable)
