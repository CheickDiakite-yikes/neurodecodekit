...
| 9 | Run grouped public motor positive control | C | Participant/run grouping, prediction + physiology + confound conjunction, remotely green prediction freeze | Gated; Implementation Qualified Locally; Target-Blind Execution Consumed; Freeze Pending Remote Green; No Rerun; prior gate: Execution Pending Remote Green |
...
then consumed one successful target-blind execution. Final scoring remains
closed until the exact aggregate freeze commit is pushed and remotely green.
...
work order 9 has separately consumed its one target-blind execution and awaits
a remotely green freeze before scoring. A
...
individual output. It must now be committed, pushed, and remotely green before
the same sealed 45 targets may open once. No rerun is authorized.
