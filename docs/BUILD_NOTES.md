...
### VR32P generated Stage 1 qualified
...
- Added the standard-library fixed-path VR32P wrapper with fixed `plan`,
  `qualify`, `inspect`, and proof-gated `execute` commands.
- The one measured generated qualification passed all 32 paths, 32 VR31A
  calls, eight direction comparisons, exact route counts 4/4/4/4/16, 152
  direct refusals, deterministic replay, marker-before-open ordering, source
  immutability, and zero retained output.
- It processed 14,137,216 generated bytes in 3.007118667010218 seconds at
  35,897,344-byte peak RSS; peak incremental output was 810 bytes and the
  canonical aggregate report was 3,422 bytes.
- Every private, consumed-state, archive, neural, target, model, score,
  network, device, FW2/CIL1, other-project, and claim counter remained zero.
...
### VR33A exact-count readiness repair qualified
...
- Exact registration `23adf07a328824d3b671e8fd8edf3c9b8d1f15ba` passed Base
  ...
  `32634409230` before implementation.
- Added the standard-library `plan`/`qualify` module with a bounded three-step
  collector, two fixed sleeps, strict generated sample validation, frozen
  dataclass copies, and no private executor or filesystem collection path.
- The sole generated qualification returned `MARC2VR33A-G1`: 16 paths, 48
  provider calls, 32 sleeper calls, 48 returned samples, only `PPP` ready, 67
  direct refusals, exact replay, and zero source mutations.
- It used 4,136 generated bytes in 0.004203916992992163 seconds at
  24,215,552-byte peak RSS. Canonical output was 2,390 bytes; retained output,
  network, new payload, raw data, cache, model, and training counts were zero.
- Every private, ignored-path, readiness-state, archive, neural, target, model,
  score, FW2/CIL1, device, release, other-project, and claim counter remained
  zero.
...
### VR36P generated Stage 1 qualified once

- Decision `fd08dd6ee40b16d3b4f4312601fed3370b7e2ca5` passed Base
  Python job `97215989173`, Optional Neuro Readers job `97215989332`, and CI
  `32648347577` before implementation.
- Added an independent standard-library wrapper with fixed `plan`, `qualify`,
  `inspect`, and `execute` surfaces; strict JSON, no-follow source binding,
  exclusive output, route, cardinality, resource, and public-output firewalls;
  and no consumed private executor import or call.
- The sole generated qualification passed all 40 paths. R1-R5 appeared four
  times each and R6 twenty times; VR33A/provider/sleeper/VR35A counts were
  40/120/80/20; eight temporary cohort files passed 16-subject, 96-bundle,
  384-row checks.
- Exact replay, zero source mutation, zero not-ready source construction,
  zero not-ready VR35A calls, 111 direct refusals, and zero retained output
  passed.
- The pass processed 8,847,228 generated bytes in 1.785233375034295 seconds
  at 35,340,288-byte peak RSS. Peak incremental output was 221,058 bytes;
  network, new payload, private, archive, neural, target, model, training,
  FW2/CIL1, and claim operations were zero.
- Twenty-two focused implementation/result tests pass without repeating the
  qualification. `execute` remains blocked on a remotely green implementation
  proof before any fixed private path operation.
...
### VR37A exact-task-surplus decomposition qualified
...
- Registration `a677e7abd2b89e92bb7bcc3f823a3493c6a32ad0` passed Base
  ...
  `32652807264` before implementation.
- Added a standard-library `plan`/`qualify` discriminator over the unchanged
  VR35A path and committed public 38-cell run-prefix map. There is no private
  executor, private path, generic override, network access, or retry surface.
- The sole qualification passed 24 paths. G1 and R1-R5 each appeared four
  times, VR35A was called exactly 24 times, 87 direct refusals passed, replay
  was exact, and all 24 sources remained byte-exact.
- It processed 10,606,968 generated bytes in 2.4954585829982534 seconds at
  49,266,688-byte peak RSS, emitted 2,980 aggregate bytes, and retained zero.
  Private, archive, neural, target, model, training, score, FW2/CIL1, network,
  device, other-project, and claim operations were zero.
- Twenty-two focused tests and 5,538 dependency-light tests pass with 204
  expected skips. Qualification was not repeated during verification.
...
### VR38A generated selection-sufficiency repair qualified

- Registration `25205b1d2a1033cf3cefcab022c885025ac76928` passed Base
  Python job `97270563617`, Optional Neuro Readers job `97270563773`, and CI
  `32670514251` before implementation.
- The additive standard-library implementation validates every generated row,
  task, normalized run, companion set, and participant/session taxonomy, then
  selects the exact eligible `reachingandgrasping` core without imposing global
  exact-195 equality.
- The sole qualification passed 40/40 paths with G1/G2/R1/R2/R3 counts
  4/16/8/8/4. All 20 accepted paths shared one full structural selection and
  one selected-name identity: 16 subjects, 96 bundles, and 384 core members.
- All 101 direct refusals passed. The run processed 17,674,816 generated bytes
  in 7.651170833967626 seconds at 50,528,256-byte peak RSS, emitted 3,606
  aggregate bytes, and retained zero.
- Raw-data reads, real-cache reads, model and training runs, network bytes,
  private operations, VR37P operations, FW2/CIL1 operations, and operations on
  other projects were zero. Qualification may not be repeated.

Immediate gate: run complete verification, commit, push, and green the exact
implementation/result, then add and separately green a proof-only closeout.
...
### BNCI-C3C5-1 replacement G1 passed generated qualification

- Recovery decision `c5dd49b3d29fcb348fc836812f5a48a6c5526f04`
  passed Base job `97547643345` in 6m36s, Optional Neuro Readers job
  `97547643658` in 8m32s, and CI `32763519623` before execution.
- The sole replacement generated/mock invocation passed all 11 registered
  engineering case classes across nine isolated participant folds.
- Exact schedule: 468 synthetic parameter-update fits, 495 synthetic
  prediction sets, one generated target delivery, and one synthetic scoring
  event. All nine folds selected E1 in the generated fixture.
- It processed 56,033,424 generated input bytes in 17.790334874996915 seconds
  at 566,231,040-byte peak process-tree RSS and emitted 3,296 aggregate bytes.
  Private generated bytes peaked at 44,193,628 and retained bytes were zero.
- Network, new payload, raw-data, real-cache, real MAT/signal/event/target,
  real fit, real inference, real target delivery, and real scoring counters
  were all zero.
- The synthetic router returned `BNCIC3C5-R2`; it has no scientific or claim
  value. End-to-end latency was not measured.
- The canonical source output SHA-256 is
  `b63ac687fcd6fad38868a7c2081ef1a50c3a3f1be3cbf3e0cfb78632d22d50c6`.
  The one replacement invocation is consumed and cannot be repeated.
- Local closeout verification passed 8 result tests, all 72 focused BNCI
  tests, and all 5,992 dependency-free tests with 216 expected skips in
  209.134 seconds. Pinned Ruff, compilation, JSON parsing, and diff hygiene
  also pass without repeating qualification or scoring.
...
### Fresh Dreyer replication prepared through the real-data gate

- Completed one aggregate-only BNCI postmortem without opening private or
  consumed evidence. It localized failed posterior specificity, failed
  probability reliability, weak/unvalidated EEG-over-EOG direction, and failed
  held-out-participant consistency.
- Selected Dreyer Dataset A `nm000250` `v1.0.4`: 60 participants, R1/R2 only,
  120 exact source EDFs, 1,779,763,388 declared bytes, and 4,800 expected
  trials. The paper reports 27 EEG, three EOG, two wrist-EMG channels, and
  512 Hz; the source EDF roster remains unverified.
- Preregistered 60 zero-calibration held-out-person folds, central EEG
  residualized against EOG/EMG/posterior/timing, matched derangement,
  posterior ablation, cue/timing controls, and one source-calibrated L2
  logistic family. The planned real schedule is 4,740 fits, 3,660 inference
  runs, 1,020 prediction sets, and 81,600 held-out prediction rows.
- Generated Stage G passed once and is consumed: 330 fits, 258 inference runs,
  102 prediction sets, 2,040 prediction rows, one synthetic target delivery,
  and one synthetic score over 197,632 generated bytes in 2.448583 seconds at
  218,497,024-byte peak RSS. Real/private operations were zero.
- Implemented a standard-library streaming fixed-EDF-header verifier with
  exact size/hash/sensor/sampling checks, no-clobber output, bounded chunks,
  and no signal or annotation parsing. Generated Stage H passed two valid
  replays and 18 adversarial refusals over 194,048 generated bytes in 0.017512
  seconds at 69,681,152-byte peak RSS. Real operations were zero.
- All-false one-file request `5191e79` passed Base `98074322914`, Optional
  `98074322739`, and CI `32934958878`. Proof closeout `821fad1` passed Base
  `98077895278`, Optional `98077895460`, and CI `32936247679`.
- Updated the current machine frontier, README, Start Here, AGENTS, tracker,
  handoff, decisions, and build notes without rewriting historical BNCI state.
- Preserved the unrelated untracked tracker-inspection NDJSON without reading,
  staging, editing, or deleting it.

Immediate gate: `DREYER-C5R-1-HL` is the sole active Tier C packet. Wait for a
...
- 2026-08-28: Exact numerical/scorer `b7e4d83` passed Base Python
  `98751222613`, Optional Neuro Readers `98751222562`, and CI `33140872221`,
  then reached GitHub `main`. Exact endpoint-separated live scorer `c46064d`
  passed Base Python `98754821063`, Optional Neuro Readers `98754821135`, and
  CI `33141715948`, then reached GitHub `main`. Free-choice live scoring is now
  primary; prompted success is diagnostic and cannot rescue it; inactive
  intervals are counted once; end-to-end latency remains unmeasured.
- 2026-08-28: Added the activation-locked `COMM-P0-G` runner scaffold and
  sidecar CLI. A bounded three-participant-per-cohort development pass ran two
  spawned model/scorer replays, matched all 15 canonical digests, emitted 140
  structural refusal observations, used 187,645,952-byte peak RSS and
  4,071,800 private generated bytes, made zero network requests, and retained
  zero payload. It is not the official qualification. Domain-specific refusal
