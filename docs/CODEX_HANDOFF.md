...
pass locally. Immediate gate: commit and push the exact implementation and
require both CI jobs green. Only then run the one registered generated
qualification. Do not touch either consumed root, a private path, archive
payload, neural data, target, model, score, network, or FW2.
