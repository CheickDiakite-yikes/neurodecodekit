...
## 0341 - Park VR17B At The First Residual Contradiction

Proof decision: registration `cde85696de8ed998d15c79630265059264ba1f2c`
passed Base Python job `96732149989`, Optional Neuro Readers job
`96732149634`, and CI `32469173279` before generated work.

Result decision: preserve the five-case preflight exactly. Wrong-task reached
F04 / `core identity differs`; the old collision witness reached F05 /
`companion run spelling differs`. Both contradict frozen route/reason gates.
Three other cases matched.

Integrity decision: park VR17B at `MARC2VR17B-P01` before full matrices,
direct refusals, report emission, or implementation. Record uninstrumented
generated bytes and peak RSS as unavailable rather than reconstructing them
with a post-failure rerun.

Next-lane decision: permit separate VR17C registration with the observed task
reason and a same-token, distinct-name normalized-collision witness. Infer no
private predicate, cohort, neural evidence, FW2/CIL1 eligibility, or scientific
claim.

## 0343 - Accept The Generated VR17C Four-Route Classifier

Proof decision: registration `a34896d1d0e4ebc548f4b92bcbd80a70355dc8c2`
passed Base Python job `96737040056`, Optional Neuro Readers job
`96737040177`, and CI `32470828824` before implementation.

Result decision: accept `MARC2VR17C-G1` after 24 paired equivalence paths, 20
residual paths, exact two-replay agreement, all four frozen hypotheses, and 50
direct refusals. G1 and R1-R4 each appeared four times. The pass processed
19,213,944 generated bytes in 3.7416582500445656 seconds at 36,569,088-byte
peak RSS and retained zero.

Verification decision: preserve the fresh-process RSS as the resource
measurement. In the shared complete suite, inject that measured RSS into the
unit assertion because macOS `ru_maxrss` carries unrelated earlier process
high-water use. Keep live CLI RSS enforcement and direct cap refusals active.
All 4,576 dependency-light tests pass with 204 expected skips.

Boundary decision: this is a generated structural classifier only. Require
the exact implementation/result commit and both remote jobs green, then a
...
## 0356 - Accept The Generated Published-Task Selector Repair

Proof decision: registration
`cd71807ac68f449796b6bc97745e9a0b200b2cd3` passed Base Python job
`96778573327`, Optional Neuro Readers job `96778573092`, and CI
`32484725113` before implementation.

Result decision: accept `MARC2VR20A-G1` after 20 generated success paths, two
exact replays, source immutability, source-exact selected names, one semantic
cohort digest, and 53 direct refusals. Record 16 subjects, 96 run bundles, 384
core members, 17,273,948 generated bytes, 1.7907995419809595 seconds,
35,143,680-byte peak RSS, and zero retention.

Architecture decision: permit a validation-only deep-copy projection into the
old row-schema vocabulary, but require grouping, selection, reservation, and
returned identities to use the original `task-reachingandgrasping` rows. Do
not edit the frozen predecessor.
...
## 0377 - Bind VR24P Request Proof Without Changing Scope

Proof decision: exact all-false request
`b7fca404a85c9597f61b1016c388b544ee901595` passed Base Python job
`97092614972`, Optional Neuro Readers job `97092615100`, and CI `32598316430`.

Identity decision: bind the unchanged packet document, machine request, and
request test as three Git blobs totaling 29,829 bytes with exact SHA-256
digests.
...
## 0413 - Bind Exact Remote Proof For The VR32P Request

Proof decision: exact all-false request
`9dc13cb29804a7adfeaa45aa821e36e160a0f6ee` passed CI `32629224038`, Base
Python job `97169278061`, and Optional Neuro Readers job `97169278037`.

Identity decision: bind the unchanged 4,328-byte authorization packet,
13,544-byte machine request, and 6,525-byte request test by SHA-256 and Git
blob. The exact request artifact set totals 24,397 bytes.

Boundary decision: repeat no qualification and perform no readiness, private,
ignored-path, consumed-state, archive, neural, target, model, prediction,
score, network, provider, device, FW2/CIL1, other-project, release, or claim
operation. This closeout is ineffective until its own exact commit is pushed
and both required jobs are green. Only afterward may VR32P be identified as
the sole active Tier C gate and fresh packet-bound maintainer words accepted;
the current `continue` is not retroactive authority.

## 0414 - Record Packet-Bound VR32P Authorization

Proof decision: exact request-proof closeout
`41d7ef7f5ae7f2288ce3af870ae786652d0aade3` passed CI `32630153639`, Base
Python job `97171587597`, and Optional Neuro Readers job `97171587706` before
VR32P was identified as the sole active Tier C packet.

Authorization decision: the maintainer's next exact message was the 16 UTF-8
bytes `continuecontinue`, with SHA-256
`2965eb147f8f3c914314b30190eba8105f58f176b0dfc0c6ed71e40c024b6d5f`.
Bind those actual words only to the unchanged six-file, 36,522-byte VR32P
...
pushed and both required jobs are green. Recording it performs zero
...
## 0428 - Accept The Sole Generated VR35A Task-Aware Qualification

Proof decision: exact registration
`aa4c39a5ce8ca04627c9252600971ee878f20e3e` passed CI `32643351246`, Base
Python job `97203738713`, and Optional Neuro Readers job `97203738637` before
implementation and qualification.

Engineering decision: accept the task-aware grouping and projection repair.
It preserves task as a logical-bundle dimension, projects exact lowercase
`reachingandgrasping` before eligibility arithmetic, and then uses the
unchanged classifier, rank-prefix selector, session split, reservation cap,
source-exact row validation, and target firewall.

Qualification decision: accept the sole 20-path generated pass. G1/G2/R1/R2/
R3 each appeared four times; eight selections and eight validations passed;
the mixed-task semantic cohort matched baseline; non-target selected rows
were zero; 99 direct refusals, 20 immutability checks, and exact replay passed.
Runtime was 1.2351410000119358 seconds at 36,044,800-byte peak RSS over
8,836,768 generated bytes, with 2,950 aggregate bytes and zero retention.

Boundary decision: qualification may not be repeated. This proves a generated
defect mechanism and repair, not that mixed-task surplus caused private VR34P
...
- 2026-08-29: Accept packet-bound decision
  `53f2c48831d1db7875bf09a35f107e35f97c6bf4` after Base Python
  `99114895023`, Optional Neuro Readers `99114895128`, and CI `33257975186`
  passed on GitHub `main`. Implement the authorized additive standard-library
  H-L2 adapter and generated-only qualification without modifying frozen
  predecessors. The complete 32-case / 33-attempt matrix produced two matching
  H1 replays, 21 aggregate H0 outcomes, ten raised fail-closed refusals, zero
  network or real/private operations, and zero retained generated bytes in
  0.599492667010054 seconds at 39,141,376-byte peak RSS. Require this exact
  implementation commit, push, and both remote jobs green before creating one
