...
Task 3 remains in progress until the exact wrapper is committed, pushed, and
both remote jobs are green. Only then may its single machine-gated live
metadata/tail/directory execution run. Task 4 member-level pilot selection and
every signal, target, model, score, replication, and scientific promotion
step remain closed regardless of a metadata-only success.
