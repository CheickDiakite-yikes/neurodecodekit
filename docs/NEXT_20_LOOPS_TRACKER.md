...
> task-aware qualification. Commit, push, and green the exact implementation
> and result, then green one proof-only closeout without rerunning it. A real
> cohort confirmation remains a separate Tier C packet; archive, neural,
> target, model, score, FW2/CIL1, and claim work remains closed.
...
green one proof-only closeout without repeating qualification. The generated
mechanism does not identify the private cause of VR34P R2. Any private wrapper
or cohort freeze remains a separately frozen Tier C packet and packet-bound
decision.
