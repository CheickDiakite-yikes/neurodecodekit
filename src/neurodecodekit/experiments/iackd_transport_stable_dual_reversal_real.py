"""One-shot transport-stable executor for the frozen IACKD-2R dual reversal."""
...
PUBLIC_RECEIPT_RELATIVE_PATH = Path(
    "registries/iackd_transport_stable_dual_reversal_acquisition_receipt.v0.json"
)
PUBLIC_FREEZE_RELATIVE_PATH = Path(
    "registries/iackd_transport_stable_dual_reversal_prediction_freeze.v0.json"
)
PUBLIC_RESULT_RELATIVE_PATH = Path(
    "registries/iackd_transport_stable_dual_reversal_result.v0.json"
)
PRIVATE_ROOT_RELATIVE_PATH = Path(
    ".codex_work/iackd_transport_stable_dual_reversal/real_execution_v0"
)
CONTRACT_SHA256 = "f3b38cb2c5bf0a55e0816072ef654cc87bd2e2f36bab50df19947d66d2abdb7f"
DECISION_SHA256 = "ff0dd757c612df53aa3e9020b80d8da420e921f7087f8e02be7fcb6213508cc8"
INVENTORY_SHA256 = "aeaa4928192cca9086fcb0abf4711147c68a68ef5c5aacda2ebc67d162a1ef19"
DECISION_COMMIT = "feef8f721c5441f98829099a63a20dd264c98204"
DECISION_CI_RUN_ID = 31_476_158_747
DECISION_BASE_JOB_ID = 93_730_242_015
DECISION_OPTIONAL_JOB_ID = 93_730_242_090
TRANSPORT_IMPLEMENTATION_COMMIT = "93a067c4dcdb89ea5e5d17db6e5adaca454a64d1"
TRANSPORT_IMPLEMENTATION_CI_RUN_ID = 31_474_412_246
TRANSPORT_IMPLEMENTATION_BASE_JOB_ID = 93_724_709_807
TRANSPORT_IMPLEMENTATION_OPTIONAL_JOB_ID = 93_724_709_840
MINIMUM_FREE_DISK_BYTES = 10_737_418_240
...
REFUSAL_IDS = (
    "IACKD2R-F01-evidence_binding_or_green_gate_mismatch",
    "IACKD2R-F02-dependency_configuration_or_machine_safety_drift",
    "IACKD2R-F03-insufficient_free_disk_or_load_metric_unavailable",
    "IACKD2R-F04-forbidden_preexisting_path_interface_attempt",
    "IACKD2R-F05-metadata_transport_or_identity_drift",
    "IACKD2R-F06-unregistered_object_redirect_retry_or_substitution",
    "IACKD2R-F07-path_link_overwrite_or_nonexclusive_root",
    "IACKD2R-F08-payload_size_ETag_SHA_or_run_group_cap_failure",
    "IACKD2R-F09-source_semantics_role_or_geometry_failure",
    "IACKD2R-F10-marker_stream_trial_or_arm_join_failure",
    "IACKD2R-F11-motion_guard_or_minimum_count_failure",
    "IACKD2R-F12-target_firewall_or_fit_final_partition_leak",
    "IACKD2R-F13-fit_or_prediction_inventory_incomplete",
    "IACKD2R-F14-resource_or_output_cap_breach",
    "IACKD2R-F15-freeze_hash_privacy_or_remote_green_failure",
    "IACKD2R-F16-second_delivery_retry_rerun_or_post_target_update",
    "IACKD2R-F17-preexisting_path_cleanup_attempt",
    "IACKD2R-F18-public_artifact_privacy_failure",
    "IACKD2R-F19-claim_boundary_breach",
)
...
class RealDualReversalRefusal(RuntimeError):
    ...
    def __init__(self, refusal_id: str, message: str) -> None:
        if refusal_id not in REFUSAL_IDS:
            raise ValueError("unknown IACKD-2R refusal ID")
...
def load_registered_decision(repo_root: str | Path | None = None) -> dict[str, Any]:
    ...
    if (
        ...
        or decision.get("green_transport_implementation", {}).get("commit")
        != TRANSPORT_IMPLEMENTATION_COMMIT
        or decision.get("authorization", {}).get(
            "additive_executor_implementation_authorized_after_decision_green"
        )
        is not True
        or decision.get("conditional_access_rules", {}).get(
            "consumed_marker_may_be_written_before_machine_safety_passes"
        )
        is not False
    ):
...
def preconsumption_machine_safety(
    ...
    *,
    ...
    """Measure the registered disk, thread, and one-minute-load gate."""
    ...
    try:
        ...
        raise RealDualReversalRefusal(
            REFUSAL_IDS[2], "machine safety metric is unavailable"
        ) from exc
    if logical_cpus is None or isinstance(logical_cpus, bool) or logical_cpus <= 0:
        raise RealDualReversalRefusal(REFUSAL_IDS[2], "logical CPU count is unavailable")
    if not math.isfinite(one_minute_load) or one_minute_load < 0.0:
        raise RealDualReversalRefusal(REFUSAL_IDS[2], "one-minute load is invalid")
    load_per_logical_cpu = one_minute_load / logical_cpus
    if free_bytes < MINIMUM_FREE_DISK_BYTES:
        raise RealDualReversalRefusal(REFUSAL_IDS[2], "free disk is below 10 GiB")
    if load_per_logical_cpu > MAXIMUM_ONE_MINUTE_LOAD_PER_LOGICAL_CPU:
        raise RealDualReversalRefusal(
            REFUSAL_IDS[2], "one-minute load exceeds one per logical CPU"
        )
    return {
        ...
        "one_minute_load_per_logical_CPU": load_per_logical_cpu,
...
def registered_plan(repo_root: str | Path | None = None) -> dict[str, Any]:
    """Return the exact IACKD-2R plan without local path stats or network."""

    contract = load_registered_contract(repo_root)
    load_registered_decision(repo_root)
    return {
        "schema_name": "neurodecodekit.iackd_transport_stable_dual_reversal_real_plan",
        ...
        "next_gate": "exact_additive_executor_implementation_must_be_remotely_green",
...
def _open_url_once(url: str, maximum_bytes: int) -> BinaryIO:
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "NeuroDecodeKit-IACKD2R/0.1", "Accept-Encoding": "identity"},
        method="GET",
    )
    del maximum_bytes
    return urllib.request.build_opener(_RejectRedirect).open(request, timeout=120)
...
class _TransportAlias:
    """Expose a generated-only validator alias after the real URL is checked."""

    def __init__(self, stream: BinaryIO, alias_url: str) -> None:
        self._stream = stream
        self._alias_url = alias_url
        self.status = _response_status(stream)
        self.headers = getattr(stream, "headers", {})
        self.redirected = bool(getattr(stream, "redirected", False))

    def geturl(self) -> str:
        return self._alias_url
...
def _transport_stable_metadata_response(
    stream: BinaryIO,
    *,
    url: str,
    expected_bytes: int,
    ...
    ordinal: int,
) -> tuple[transport.ResponseValidation, Any]:
    """Apply the green validator without weakening the real final-URL check."""

    if (
        _response_status(stream) != 200
        or _response_url(stream) != url
        or bool(getattr(stream, "redirected", False))
    ):
        raise RealDualReversalFailure(REFUSAL_IDS[4], "metadata status or final URL differs")
    alias_url = f"fixture://iackd2r/registered-metadata-{ordinal}"
    try:
        return transport.validate_and_parse_response(
            transport.AuditedResponse(_TransportAlias(stream, alias_url)),
            spec=transport.ResponseSpec(
                url=alias_url,
    ...
    except transport.TransportStableRefusal as exc:
        raise RealDualReversalFailure(
            REFUSAL_IDS[4], f"metadata transport refused with {exc.refusal_id}"
        ) from exc
...
def _fetch_metadata(
    ...
    def parse_description(body: bytes) -> dict[str, Any]:
        value = json.loads(body.decode("utf-8"))
        if not isinstance(value, dict):
            raise ValueError("dataset description root differs")
        return value

    def parse_changes(body: bytes) -> str:
        return body.decode("utf-8")

    def parse_listing(body: bytes) -> tuple[list[dict[str, Any]], bool, str | None]:
        return legacy_acquisition._listing_objects(body)

    requests = (
        (
            source["dataset_description"]["url"],
            int(source["dataset_description"]["bytes"]),
            str(source["dataset_description"]["sha256"]),
            parse_description,
        ),
        (
            source["changes"]["url"],
            int(source["changes"]["bytes"]),
            str(source["changes"]["sha256"]),
            parse_changes,
        ),
        (
            first_url,
            int(snapshot["pages"][0]["body_bytes"]),
            str(snapshot["pages"][0]["body_sha256"]),
            parse_listing,
        ),
    )
    validations: list[transport.ResponseValidation] = []
    parsed: list[Any] = []
    for ordinal, (url, expected, expected_sha256, parser) in enumerate(requests, start=1):
        if expected > cap:
            raise RealDualReversalFailure(REFUSAL_IDS[13], "metadata body cap exceeded")
        with _managed_stream(opener(url, expected + 1)) as stream:
            validation, value = _transport_stable_metadata_response(
                stream,
                url=url,
    ...
    changes = parsed[1]
    first, first_truncated, token = parsed[2]
    binding = contract["dataset_binding"]
    if (
        description.get("BIDSVersion") != binding["bids_version"]
        or description.get("License") != binding["license"]
        ...
        raise RealDualReversalFailure(REFUSAL_IDS[4], "dataset identity differs")
    if not first_truncated or not token:
    ...
    if second_size > cap:
        raise RealDualReversalFailure(REFUSAL_IDS[13], "metadata body cap exceeded")
    with _managed_stream(opener(second_url, second_size + 1)) as stream:
        validation, second_parsed = _transport_stable_metadata_response(
            stream,
            url=second_url,
            expected_bytes=second_size,
            expected_sha256=str(snapshot["pages"][1]["body_sha256"]),
            parser=parse_listing,
            ordinal=4,
        )
    validations.append(validation)
    second, second_truncated, _ = second_parsed
    ...
    if (
        second_truncated
        or len(listed) != metadata_contract["expected_listed_object_count"]
        or listed_total_bytes != metadata_contract["expected_listed_total_bytes"]
    ):
        raise RealDualReversalFailure(REFUSAL_IDS[4], "object listing identity differs")
    selected = [
        {
    ...
    metadata_body_bytes = sum(row.observed_bytes for row in validations)
    if metadata_body_bytes > cap:
        raise RealDualReversalFailure(REFUSAL_IDS[13], "metadata byte cap exceeded")
    return {
        "metadata_requests": 4,
        "metadata_body_bytes": metadata_body_bytes,
        "metadata_framing_profiles": [row.framing_profile for row in validations],
        "metadata_Content_Length_states": [
            row.content_length_state for row in validations
        ],
        "metadata_transport_warnings": [
            warning for row in validations for warning in row.warnings
        ],
        "metadata_read_calls": sum(row.read_calls for row in validations),
        "metadata_hash_calls": sum(row.hash_calls for row in validations),
        "metadata_parse_calls": sum(row.parse_calls for row in validations),
        "listed_object_count": len(listed),
        "listed_total_bytes": listed_total_bytes,
        "selected_object_count": len(selected),
        "selected_payload_bytes": sum(row["size_bytes"] for row in selected),
        "canonical_identity_bytes": identity_bytes,
        "canonical_identity_sha256": identity_sha256,
...
def _validate_public_receipt(receipt: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "neurodecodekit.iackd2r_acquisition_derivative_receipt"
        or receipt.get("status") != "passed_complete_target_firewalled_derivatives"
        or measurements.get("metadata_requests") != 4
        or measurements.get("metadata_read_calls") != 4
        or measurements.get("metadata_hash_calls") != 4
        or measurements.get("metadata_parse_calls") != 4
        or not set(measurements.get("metadata_framing_profiles", ())).issubset(
            {"fixed_length", "chunked", "close_delimited"}
        )
        or counters.get("old_retained_bundle_operations") != 0
...
def run_streaming_derivative_build(
    *,
    ...
    implementation_commit: str,
    machine_safety_evidence: Mapping[str, Any] | None = None,
    ...
    if strict_registered:
        ...
        free_value = (
            machine_safety_evidence.get("free_disk_bytes")
            if machine_safety_evidence is not None
            else None
        )
        load_value = (
            machine_safety_evidence.get("one_minute_load_per_logical_CPU")
            if machine_safety_evidence is not None
            else None
        )
        if (
            machine_safety_evidence is None
            or machine_safety_evidence.get("passed_before_consumed_marker") is not True
            or not isinstance(free_value, int)
            or isinstance(free_value, bool)
            or free_value < MINIMUM_FREE_DISK_BYTES
            or not isinstance(load_value, int | float)
            or isinstance(load_value, bool)
            or not math.isfinite(float(load_value))
            or float(load_value) > MAXIMUM_ONE_MINUTE_LOAD_PER_LOGICAL_CPU
        ):
            raise RealDualReversalRefusal(
                REFUSAL_IDS[2], "pre-consumption machine safety evidence differs"
            )
    ...
    _write_exclusive(
        ...
        _json_bytes(
            {
                "schema_name": "neurodecodekit.iackd2r_execution_consumed",
                "schema_version": SCHEMA_VERSION,
    ...
    private_manifest = {
        "schema_name": "neurodecodekit.iackd2r_private_derivative_manifest",
        "schema_version": SCHEMA_VERSION,
    ...
    receipt = {
        "schema_name": "neurodecodekit.iackd2r_acquisition_derivative_receipt",
        "schema_version": SCHEMA_VERSION,
        ...
        "measurements": {
            **metadata_summary,
            "preconsumption_machine_safety": dict(machine_safety_evidence or {}),
...
def _mock_contract_inventory_transport() -> tuple[
    ...
    description = json.dumps(
        {
            "Name": "Generated IACKD2R transport fixture",
            "BIDSVersion": "1.7.0",
            "License": "CC0",
    ...
    metadata_framing = {
        source["dataset_description"]["url"]: ("exact", None),
        source["changes"]["url"]: (str(len(changes) + 1), None),
        first_url: (None, "chunked"),
        second_url: (None, None),
    }
    etags: dict[str, str] = {}
    for row in rows:
    ...
    def opener(url: str, maximum_bytes: int) -> BinaryIO:
        ...
        calls.append(url)
        content_length, transfer_encoding = metadata_framing.get(
            url, ("exact", None)
        )
...
def load_target_free_model_stage(
    *,
    ...
    if (
        ...
        != "neurodecodekit.iackd2r_private_derivative_manifest"
...
def build_prediction_freeze(
    *,
    ...
    _validate_matrix(matrix, model_stage=model_stage, contract=contract)
    if source_kind not in {"generated_fixture", "real_public_IACKD2R"}:
...
def validate_public_freeze(freeze: Mapping[str, Any]) -> None:
    ...
    if (
        freeze["schema_name"] != "neurodecodekit.iackd2r_prediction_freeze"
        or freeze["schema_version"] != SCHEMA_VERSION
        or freeze["status"] != "frozen_target_blind_predictions_targets_sealed"
        or freeze["source_kind"] not in {"generated_fixture", "real_public_IACKD2R"}
...
def _validate_implementation_registry(
    *,
    ...
    if (
        ...
        or registry.get("green_transport_implementation", {}).get("commit")
        != TRANSPORT_IMPLEMENTATION_COMMIT
        or registry.get("implemented_interface", {}).get(
            "old_executor_modified_or_called"
        )
        is not False
...
def run_registered_streaming_execution(
    *,
    ...
    """Perform the sole registered fresh IACKD-2R stream after green implementation."""
    ...
    machine_safety = preconsumption_machine_safety(root, environ=environ)
    return run_streaming_derivative_build(
        ...
        machine_safety_evidence=machine_safety,
    )
...
def run_registered_target_blind_analysis(
    *,
    ...
    _write_exclusive(
        ...
        _json_bytes(
            {
                "schema_name": "neurodecodekit.iackd2r_analysis_consumed",
                "schema_version": SCHEMA_VERSION,
                "started_at_UTC": _utc_now(),
                "implementation_commit": evidence.implementation_commit,
                "retry_allowed": False,
...
def score_registered_execution(
    *,
    ...
    validate_public_freeze(freeze)
    if freeze["source_kind"] != "real_public_IACKD2R":
    ...
    _write_exclusive(
        ...
        _json_bytes(
            {
                "schema_name": "neurodecodekit.iackd2r_scoring_consumed",
                "schema_version": SCHEMA_VERSION,
                "started_at_UTC": _utc_now(),
...
def _generated_refusal_suite(
    *,
    ...
    metadata_url = "https://fixture.invalid/metadata"
    metadata_body = b'{"fixture":"transport-stable"}'
    metadata_sha256 = _sha256_bytes(metadata_body)
    mutations.extend(
        (
            _expect_generated_refusal(
                lambda: _transport_stable_metadata_response(
                    FixtureResponse(
                        body=metadata_body,
                        url=metadata_url,
                        etag="fixture",
                        content_length="malformed",
                    ),
                    url=metadata_url,
                    expected_bytes=len(metadata_body),
                    expected_sha256=metadata_sha256,
                    parser=json.loads,
                    ordinal=1,
                ),
                expected_id=REFUSAL_IDS[4],
            ),
            _expect_generated_refusal(
                lambda: _transport_stable_metadata_response(
                    FixtureResponse(
            ...
            _expect_generated_refusal(
                lambda: preconsumption_machine_safety(
                    Path("fixture-machine-safety"),
                    environ=environ,
                    disk_usage_reader=lambda _: _DiskUsageFixture(
                        free=MINIMUM_FREE_DISK_BYTES - 1
                    ),
                    cpu_count_reader=lambda: 8,
                    loadavg_reader=lambda: (1.0, 0.0, 0.0),
                ),
                expected_id=REFUSAL_IDS[2],
            ),
            _expect_generated_refusal(
                lambda: preconsumption_machine_safety(
                    Path("fixture-machine-safety"),
...
def run_generated_real_executor_qualification(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="iackd2r-real-executor-generated-") as temporary:
        workspace = Path(temporary)
        fixture_safety = preconsumption_machine_safety(
            workspace,
            environ=environment,
            disk_usage_reader=lambda _: _DiskUsageFixture(
                free=MINIMUM_FREE_DISK_BYTES + 1
        ...
        receipt = run_streaming_derivative_build(
            ...
            machine_safety_evidence=fixture_safety,
        )
    ...
    report = {
        "schema_name": "neurodecodekit.iackd2r_real_executor_qualification",
        "schema_version": SCHEMA_VERSION,
        ...
        "mock_stream": {
            ...
            "metadata_framing_profiles": receipt["measurements"][
                "metadata_framing_profiles"
            ],
            "metadata_Content_Length_states": receipt["measurements"][
                "metadata_Content_Length_states"
            ],
            "metadata_transport_warnings": receipt["measurements"][
                "metadata_transport_warnings"
            ],
            "metadata_read_hash_parse_calls": [
                receipt["measurements"]["metadata_read_calls"],
                receipt["measurements"]["metadata_hash_calls"],
                receipt["measurements"]["metadata_parse_calls"],
            ],
        },
        "target_firewall": {
        ...
        "measurements": {
            ...
            "workers": 1,
            "concurrent_numerical_jobs": 1,
            "generated_machine_safety": fixture_safety,
            "producer_is_causal_in_samples": True,
            "end_to_end_latency_measured": False,
        },
        "access_counters": {
            ...
            "generated_machine_safety_checks": 1,
        ...
        "acceptance_gates": {
            ...
            "transport_stable_metadata_profiles_passed": set(
                receipt["measurements"]["metadata_framing_profiles"]
            )
            == {"fixed_length", "chunked", "close_delimited"},
            "metadata_read_hash_parse_order_passed": receipt["measurements"][
                "metadata_read_calls"
            ]
            == receipt["measurements"]["metadata_hash_calls"]
            == receipt["measurements"]["metadata_parse_calls"]
            == 4,
            "generated_machine_safety_gate_passed": fixture_safety[
                "passed_before_consumed_marker"
            ],
...
def validate_qualification_report(report: Mapping[str, Any]) -> None:
    ...
    if (
        report["schema_name"]
        != "neurodecodekit.iackd2r_real_executor_qualification"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plan, qualify, or execute the frozen IACKD-2R pipeline."
    )
