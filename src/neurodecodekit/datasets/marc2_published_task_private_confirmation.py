"""Proof-gated MARC2 published-task structural confirmation wrapper.
...
PLAN_SCHEMA_NAME = "neurodecodekit.marc2_published_task_private_confirmation_plan"
QUALIFICATION_SCHEMA_NAME = "neurodecodekit.marc2_published_task_private_confirmation_qualification"
REPORT_SCHEMA_NAME = "neurodecodekit.marc2_published_task_private_confirmation_report"
PRIVATE_SCHEMA_NAME = "neurodecodekit.marc2_published_task_private_cohort"
CERTIFICATE_SCHEMA_NAME = "neurodecodekit.marc2_published_task_readiness_certificate"
MARKER_SCHEMA_NAME = "neurodecodekit.marc2_published_task_private_confirmation_consumed"
...
READINESS_RELATIVE_PATH = Path(".codex_work/marc2_machine_readiness/vr20p/readiness.v0.json")
OUTPUT_ROOT_RELATIVE_PATH = Path(".codex_work/marc2_published_task_private_confirmation/v0")
...
DECISION_SHA256 = "e65a06e61147ba7ba23b3633e5ab26e7f3b385a34fccdc37549536cd23618bc6"
REQUEST_SHA256 = "e79085dd4d63cf57ca4d73bfc5c05d5c479f266bd0a699153e29d3d34a9b14fa"
GREEN_DECISION_COMMIT = "49b0f4b9b8ec13f885b0010947326fbd3b9641ac"
GREEN_DECISION_CI_RUN_ID = 32_556_695_686
GREEN_DECISION_BASE_JOB_ID = 96_991_921_763
GREEN_DECISION_OPTIONAL_JOB_ID = 96_991_921_631
...
SUCCESS_ROUTE = "MARC2VR20P-G1"
PRIVATE_SUCCESS_ROUTE = "MARC2VR20P-R1"
PRIVATE_FAILURE_ROUTES = tuple(f"MARC2VR20P-R{index}" for index in range(2, 7))
REFUSAL_ROUTES = tuple(f"MARC2VR20P-F{index:02d}" for index in range(1, 13))
...
GENERATED_CASE_ROUTES = {
    ...
    "adapter_precondition": PRIVATE_FAILURE_ROUTES[0],
    "source_envelope": PRIVATE_FAILURE_ROUTES[1],
    "task_identity": PRIVATE_FAILURE_ROUTES[2],
    "selection": PRIVATE_FAILURE_ROUTES[3],
    "privacy": PRIVATE_FAILURE_ROUTES[4],
}
...
class PublishedTaskPrivateConfirmationRefusal(RuntimeError):
    ...
    def __init__(self, route: str, reason: str):
        if route not in REFUSAL_ROUTES and route not in PRIVATE_FAILURE_ROUTES:
            raise ValueError("unknown MARC2-VR20P refusal route")
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise PublishedTaskPrivateConfirmationRefusal(
...
def _strict_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "duplicate JSON key")
        result[key] = value
    return result


def _reject_constant(_value: str) -> None:
    raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "non-finite JSON number")
...
def _strict_json(payload: bytes) -> dict[str, Any]:
    ...
    except PublishedTaskPrivateConfirmationRefusal:
        raise
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "strict JSON parse failed") from exc
    if not isinstance(value, dict):
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "JSON root is not an object")
    return value


def _read_tracked(root: Path, relative: Path) -> bytes:
    if relative.is_absolute() or ".." in relative.parts:
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "tracked path is unsafe")
    path = root / relative
    info = path.lstat()
...
def load_decision(root: Path | None = None) -> dict[str, Any]:
    ...
    if _sha256_bytes(payload) != DECISION_SHA256:
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "decision identity differs")
    decision = _strict_json(payload)
    _validate_decision(decision)
    return decision


def _validate_vr20a_artifacts(root: Path | None = None) -> None:
    repo = root or _repo_root()
    expected = (
    ...
    for relative, digest in expected:
        if _sha256_bytes(_read_tracked(repo, relative)) != digest:
            raise PublishedTaskPrivateConfirmationRefusal(
                REFUSAL_ROUTES[0], "VR20A artifact identity differs"
            )
...
def _validate_decision(decision: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or request.get("commit") != "bef2391d8edf92c5edf8a3624831e50430636626"
        or request.get("both_required_jobs_green") is not True
        or not isinstance(proof, dict)
        or proof.get("commit") != "88b3b4aaa4436655ce6f4de65215982e2b8ff9de"
        or proof.get("both_required_jobs_green") is not True
        or not isinstance(user, dict)
        or user.get("actual_message_verbatim") != "conitnue"
        or user.get("actual_message_SHA256")
        != "7c9672298889dcfc5aa8136e83f1c278f66dff32fd7a448f6f142cb56f4ede17"
        or user.get("unambiguous_transposition_typo_of_continue") is not True
        ...
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "decision contract differs")
...
def _validate_thread_environment(environment: Mapping[str, str] | None = None) -> None:
    ...
    if any(values.get(name) != "1" for name in THREAD_ENVIRONMENT):
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[1], "thread environment is not one")
...
def _assert_resources(
    ...
    if (
        ...
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[10], "resource or output cap exceeded")


def _safe_parent_chain(root: Path, relative: Path) -> None:
    if relative.is_absolute() or ".." in relative.parts:
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[2], "unsafe fixed path")
    current = root
    for part in relative.parts[:-1]:
        current = current / part
        if current.exists() and current.is_symlink():
            raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[2], "fixed parent is a symlink")
...
def _create_fresh_directory(root: Path, relative: Path) -> Path:
    ...
    if path.exists() or path.is_symlink():
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[2], "fixed output already exists")
...
def _write_exclusive(path: Path, payload: bytes, mode: int) -> int:
    ...
    try:
        ...
        if written != len(payload):
            raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[9], "short fixed-path write")
    finally:
        os.close(descriptor)
    os.chmod(path, mode)
    return len(payload)
...
def _sample_time(value: Any) -> datetime:
    if not isinstance(value, str):
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness timestamp is missing")
    try:
    ...
    if parsed.tzinfo is None:
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness timestamp lacks timezone")
    return parsed.astimezone(UTC)


def _validate_samples(samples: Sequence[Mapping[str, Any]]) -> None:
    if len(samples) != REQUIRED_PASSING_SAMPLES:
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness sample count differs")
    ...
    for index, sample in enumerate(samples):
        if (
            ...
            raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness sample failed")
...
def _preflight_source(path: Path, identity: Mapping[str, Any]) -> None:
    ...
    if stat.S_IMODE(info.st_mode) != identity["mode"] or info.st_size != identity["bytes"]:
        raise PublishedTaskPrivateConfirmationRefusal(PRIVATE_FAILURE_ROUTES[1], "source identity differs")
...
def _private_route_for_refusal(exc: PublishedTaskPrivateConfirmationRefusal) -> str:
...
def _call_generated_adapter(
    ...
    if generated_case == "adapter_precondition":
        changed_contract = dict(repair.load_registered_contract())
        changed_contract["schema_version"] = "9.9.9"
        contract = changed_contract
    elif generated_case == "source_envelope":
        candidate["schema_name"] = "generated.invalid"
    elif generated_case == "task_identity":
        candidate = repair._mutated_witness(candidate, "task_freewill")
    elif generated_case == "selection":
        candidate = repair._mutated_witness(candidate, "semantic_run_zero")
    try:
        selected = repair.adapt_published_task_source(candidate, contract=contract)
    except repair.PublishedTaskSelectorRepairRefusal:
        raise
    if generated_case == "privacy":
        raise PublishedTaskPrivateConfirmationRefusal(
            PRIVATE_FAILURE_ROUTES[4], "generated privacy control refused"
        )
    return selected
...
def _run_fixed_sequence(
    ...
    *,
    ...
    if len(certificate_bytes) > MAX_CERTIFICATE_BYTES:
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[10], "readiness certificate exceeds cap")
    ...
    def finish_failure(exc: PublishedTaskPrivateConfirmationRefusal) -> dict[str, Any]:
    ...
    except PublishedTaskPrivateConfirmationRefusal as exc:
    ...
    try:
        ...
        repaired = (
            ...
            if generated
            else repair.adapt_published_task_source(source)
        )
    except repair.PublishedTaskSelectorRepairRefusal as exc:
        if exc.route == repair.REFUSAL_ROUTES[0]:
            route = PRIVATE_FAILURE_ROUTES[0]
        elif exc.route == repair.REFUSAL_ROUTES[1]:
            route = PRIVATE_FAILURE_ROUTES[1]
        elif exc.route in repair.REFUSAL_ROUTES[2:5]:
            route = PRIVATE_FAILURE_ROUTES[2]
        elif exc.route in repair.REFUSAL_ROUTES[5:7]:
            route = PRIVATE_FAILURE_ROUTES[3]
        else:
            route = PRIVATE_FAILURE_ROUTES[4]
        return finish_failure(
            PublishedTaskPrivateConfirmationRefusal(route, "VR20A refused source")
        )
    except PublishedTaskPrivateConfirmationRefusal as exc:
        return finish_failure(exc)
...
def _validate_envelope(envelope: Mapping[str, Any]) -> None:
    if set(envelope) != set(_ENVELOPE_EXPECTED):
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[11], "execution envelope fields differ")
...
def qualify_generated(
    *,
    ...
    for replay in range(2):
        for generated_case in GENERATED_CASES:
            for order in repair.ORDERS:
                source = repair.build_generated_variant("published_four_digit", order)
                ...
                with tempfile.TemporaryDirectory(prefix="ndk-vr20p-") as temporary:
                    root = Path(temporary)
                    source_path = root / PRIVATE_SOURCE_RELATIVE_PATH
    ...
    if (
        ...
        or sum(item["VR20A_calls"] for item in outcomes) != 24
    ...
    if direct_refusals < 90:
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[11], "direct refusal floor not met")
    elapsed = clock() - started
    rss = peak_rss()
    report = {
...
def _load_implementation(root: Path) -> dict[str, Any]:
    ...
    if (
        record.get("schema_name") != "neurodecodekit.marc2_published_task_private_confirmation_implementation"
        or record.get("lane_id") != LANE_ID
    ):
        raise PublishedTaskPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "implementation record differs")
    return record
...
def _collect_readiness(root: Path) -> list[dict[str, Any]]:
    ...
    raise PublishedTaskPrivateConfirmationRefusal(PRIVATE_FAILURE_ROUTES[0], "machine readiness did not pass")
...
def execute_registered() -> dict[str, Any]:
    ...
    _validate_decision(decision)
    _validate_vr20a_artifacts(root)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Proof-gated MARC2 published-task target-free structural confirmation."
    )
