"""Shared-proof-gated MARC2-FW1C private-manifest prefix selection."""
...
from neurodecodekit.datasets import marc2_proof_record_recovery as proof_record


SCHEMA_VERSION = "0.1.0"
LANE_ID = "MARC2-FW1C"
GENERATED_ROUTE = "MARC2FWC-G1"
SUCCESS_ROUTE = "MARC2FWC-R1"
FAILURE_ROUTES = tuple(f"MARC2FWC-F0{index}" for index in range(7))

DECISION_RELATIVE_PATH = Path(
    "registries/marc2_live_selection_recovery_authorization_decision.v0.json"
)
DECISION_SHA256 = "f8801c348d8fe5656fdbc0f1fb7afb24281e32e54aca9f9910560971e0fca9b1"
GREEN_DECISION_COMMIT = "b0466e56602d9694d5941a92203a456f7994f514"
GREEN_DECISION_CI_RUN_ID = 31_928_627_432
GREEN_DECISION_BASE_JOB_ID = 95_120_011_473
GREEN_DECISION_OPTIONAL_JOB_ID = 95_120_011_519

REQUEST_RELATIVE_PATH = Path(
    "registries/marc2_live_selection_recovery_authorization_request.v0.json"
)
REQUEST_SHA256 = "2dfd45c9e9b607f6f7f442e60c6a359b4d5f550d07f4cb4b8a474a8469d147c0"
PACKET_RELATIVE_PATH = Path(
    "docs/MARC_2_LIVE_SELECTION_RECOVERY_AUTHORIZATION_PACKET.md"
)
PACKET_SHA256 = "f517f0b4d88f1adcb181e1f8ae1896747f45e416497e269aea80c492614bd84d"
...
MODULE_RELATIVE_PATH = Path(
    "src/neurodecodekit/datasets/marc2_freewill_private_selection_recovery.py"
)
...
CERTIFICATE_TRACKED_ARTIFACTS = (
    proof_record.MODULE_RELATIVE_PATH,
    proof_record.CONTRACT_RELATIVE_PATH,
    MODULE_RELATIVE_PATH,
    IMPLEMENTATION_RELATIVE_PATH,
    Path("tests/test_marc2_live_selection_recovery.py"),
    Path("tests/test_marc2_live_selection_recovery_implementation.py"),
    DECISION_RELATIVE_PATH,
    Path("docs/MARC_2_LIVE_SELECTION_RECOVERY_AUTHORIZATION_DECISION.md"),
...
CONSUMED_MARKER_NAME = "recovery_execution_consumed.v0.json"
PRIVATE_SELECTION_NAME = "marc2_live_selection_recovery.private.v0.json"
AGGREGATE_REPORT_NAME = "marc2_live_selection_recovery_result.v0.json"

REPORT_SCHEMA_NAME = "neurodecodekit.marc2_live_selection_recovery_result"
PRIVATE_SELECTION_SCHEMA_NAME = (
    "neurodecodekit.marc2_live_selection_recovery_manifest"
)
IMPLEMENTATION_SCHEMA_NAME = (
    "neurodecodekit.marc2_live_selection_recovery_implementation"
)
...
class PrivateSelectionRefusal(RuntimeError):
    ...
    def __init__(self, route: str, reason: str):
        if route not in FAILURE_ROUTES:
            raise ValueError("unknown MARC2-FW1C route")
...
def _read_regular_bounded(path: Path, maximum_bytes: int) -> bytes:
    try:
        ...
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "tracked artifact unavailable") from exc
    if stat.S_ISLNK(before.st_mode) or not stat.S_ISREG(before.st_mode):
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "tracked artifact type differs")
        ...
        try:
            ...
            if (
                ...
                raise PrivateSelectionRefusal(
                    FAILURE_ROUTES[0],
                    "tracked artifact changed during open",
                )
            payload = _read_fd_bounded(descriptor, maximum_bytes)
        finally:
            os.close(descriptor)
    except PrivateSelectionRefusal:
        raise
    except OSError as exc:
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "tracked artifact open failed") from exc
    if len(payload) > maximum_bytes or len(payload) != before.st_size:
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "tracked artifact size differs")
    return payload
...
def load_green_decision(repo_root: str | Path | None = None) -> dict[str, Any]:
    ...
    if (
        ...
        != "continue"
        or proof.get("CI_run_id") != 31_769_518_851
        or proof.get("base_python_job_id") != 94_672_387_003
        or proof.get("optional_neuro_job_id") != 94_672_386_941
    ...
    if (
        ...
        or _sha256_file(root / PROOF_MODULE_RELATIVE_PATH) != PROOF_MODULE_SHA256
    ):
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "frozen upstream differs")
    selector.load_registered_contract(root)
    proof_record.load_registered_contract(root)
    return decision
...
def load_implementation_record(
    ...
    *,
    ...
    if (
        ...
        != "generated_mock_recovery_wrapper_qualified_requires_remote_green_before_private_selection"
    ...
    required_bindings = {
        ...
        SELECTOR_RELATIVE_PATH.as_posix(),
    }
    ...
    for binding in record.get("tracked_file_hashes", ()):
        ...
        observed_bindings.add(relative)
    if not required_bindings.issubset(observed_bindings):
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "implementation binding missing")
    return record


def _proof_envelope(
    evidence: GreenImplementationEvidence,
) -> proof_record.ProofEnvelope:
    return proof_record.ProofEnvelope(
...
def validate_proof_certificate(
    ...
    *,
    expected_sha256: str,
    expected_proof: proof_record.ProofEnvelope,
    observed_proof: proof_record.ProofEnvelope,
) -> proof_record.ValidationSummary:
    """Validate the distinct FW1B certificate through the exact shared symbol."""

    if HEX64_RE.fullmatch(expected_sha256) is None:
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "certificate hash malformed")
    root = Path(repo_root)
    try:
        ...
        summary = proof_record.validate_implementation_record(
            payload,
            repo_root=root,
        ...
        raise
    except proof_record.ProofRecordRefusal as exc:
        raise PrivateSelectionRefusal(
            FAILURE_ROUTES[0],
            "shared proof certificate refused",
        ) from exc
    if summary.lane_id != proof_record.LANE_ID:
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "certificate lane differs")
    return summary
...
def verify_green_implementation(
    ...
    observed = _observed_proof_envelope(
        ...
        observed_head=(
            head.stdout.strip()
            if not head.returncode and HEX40_RE.fullmatch(head.stdout.strip())
            else "0" * 40
        ),
        tracked_worktree_clean=not clean.returncode and not clean.stdout.strip(),
        green_decision_ancestor=not ancestor.returncode,
    )
    observed_certificate = validate_proof_certificate(
        root,
        expected_sha256=evidence.proof_certificate_sha256,
        expected_proof=expected,
        observed_proof=observed,
    )
    if certificate.to_mapping() != observed_certificate.to_mapping():
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "certificate proof replay differs")
    return {
        "native_implementation": implementation,
        "shared_proof_certificate": certificate.to_mapping(),
    }
...
def run_wrapper_mutations() -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="marc2fw1c-mutations-") as temporary:
        root = Path(temporary)
        safe = root / "safe"
        safe.mkdir()
...
def _green_evidence(
    ...
    proof_certificate_sha256: str | None,
) -> dict[str, Any]:
    value: dict[str, Any] = {
...
def _build_report(
    ...
    *,
    ...
    return {
        ...
        "mutation_summary": {
            "proof_record_required": 32,
            "proof_record_passed": len(proof_mutations or {}) if generated else 32,
            "proof_record_names": list(proof_record.ORDERED_MUTATIONS),
            "proof_record_route_counts": dict(
                sorted(Counter((proof_mutations or {}).values()).items())
            ),
            "inherited_required": 40,
            "inherited_passed": len(inherited_mutations or {}) if generated else 40,
            "wrapper_required": 18,
            "wrapper_passed": len(wrapper_mutations or {}) if generated else 18,
            ...
            "total_required": 90,
            "total_passed": (
                len(proof_mutations or {})
                + len(inherited_mutations or {})
                + len(wrapper_mutations or {})
                if generated
                else 90
            ),
        },
        ...
        "acceptance_gates": {
            ...
            "exact_shared_validator_and_FW1B_certificate": True,
            "native_FW1C_registry_validated": True,
            "all_32_proof_record_refusals": True,
    ...
    *,
    ...
    return {
        ...
        "green_evidence": _green_evidence(
            evidence,
            evidence.implementation_registry_sha256,
            evidence.proof_certificate_sha256,
        ),
        "cohort_summary": {
        ...
        "mutation_summary": {
            "proof_record_required": 32,
            "proof_record_passed": 32,
            "proof_record_names": list(proof_record.ORDERED_MUTATIONS),
            "proof_record_route_counts": {},
            "inherited_required": 40,
            "inherited_passed": 40,
            "wrapper_required": 18,
            "wrapper_passed": 18,
...
def validate_public_report(
    ...
    *,
    ...
    if not isinstance(gates, dict) or len(gates) != 18:
    ...
    required_safety_gates = (
        ...
        "exact_shared_validator_and_FW1B_certificate",
        "native_FW1C_registry_validated",
        "all_32_proof_record_refusals",
    ...
    if generated and (
        mutations.get("proof_record_passed") != 32
        or mutations.get("inherited_passed") != 40
        or mutations.get("wrapper_passed") != 18
        or mutations.get("total_passed") != 90
    ):
...
def validate_local_qualification_records(
    ...
    """Validate the actual native registry and distinct certificate locally."""
    ...
    native_record = load_implementation_record(
        root,
        expected_sha256=native_sha256,
    )
    ...
    if first.to_mapping() != second.to_mapping():
        raise PrivateSelectionRefusal(FAILURE_ROUTES[0], "certificate replay differs")
    return {
        "native_record": native_record,
        "native_registry_sha256": native_sha256,
        "certificate_record": proof_record.parse_record_bytes(certificate_bytes),
...
def build_local_proof_certificate(
    ...
    """Build the deterministic FW1B certificate without writing a file."""

    root = Path(repo_root) if repo_root is not None else _repo_root()
    return proof_record.build_generated_candidate_record(
        root,
...
def qualify_generated_mock_wrapper(
    ...
    *,
    ...
    proof_mutations = proof_record.run_generated_mutation_matrix(
...
def _create_consumed_root(
    ...
    marker = {
        "schema_name": "neurodecodekit.marc2_live_selection_recovery_consumed",
        "schema_version": SCHEMA_VERSION,
        "lane_id": LANE_ID,
...
def registered_plan() -> dict[str, Any]:
    ...
    return {
        ...
        "reservation_cap_bytes": selector.RESERVATION_CAP_BYTES,
        "proof_record_mutations": 32,
        "inherited_selector_mutations": 40,
        "wrapper_mutations": 18,
        "total_mutations": 90,
        "proof_certificate_lane_id": proof_record.LANE_ID,
        "native_registry_lane_id": LANE_ID,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=(
            ...
            "marc2_freewill_private_selection_recovery"
        ),
        description="Shared-proof-gated MARC2-FW1C target-free private selection.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the dependency-free MARC2-FW1C module CLI."""
