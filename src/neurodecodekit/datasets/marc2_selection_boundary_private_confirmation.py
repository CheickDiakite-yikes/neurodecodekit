"""Proof-gated MARC2 selection-boundary structural confirmation wrapper.
...
PLAN_SCHEMA_NAME = "neurodecodekit.marc2_selection_boundary_private_confirmation_plan"
QUALIFICATION_SCHEMA_NAME = "neurodecodekit.marc2_selection_boundary_private_confirmation_qualification"
REPORT_SCHEMA_NAME = "neurodecodekit.marc2_selection_boundary_private_confirmation_report"
PRIVATE_SCHEMA_NAME = "neurodecodekit.marc2_selection_boundary_private_cohort"
CERTIFICATE_SCHEMA_NAME = "neurodecodekit.marc2_selection_boundary_readiness_certificate"
MARKER_SCHEMA_NAME = "neurodecodekit.marc2_selection_boundary_private_confirmation_consumed"
...
VR25A_MODULE_RELATIVE_PATH = Path(
    "src/neurodecodekit/datasets/marc2_selection_boundary_firewall.py"
)
VR25A_CONTRACT_RELATIVE_PATH = Path(
    "registries/marc2_selection_boundary_firewall_contract.v0.json"
)
VR25A_IMPLEMENTATION_RELATIVE_PATH = Path(
    "registries/marc2_selection_boundary_firewall_implementation.v0.json"
)
VR25A_RESULT_RELATIVE_PATH = Path(
    "registries/marc2_selection_boundary_firewall_result.v0.json"
)
READINESS_RELATIVE_PATH = Path(".codex_work/marc2_machine_readiness/vr26p/readiness.v0.json")
OUTPUT_ROOT_RELATIVE_PATH = Path(".codex_work/marc2_selection_boundary_private_confirmation/v0")
MARKER_RELATIVE_NAME = "consumed_marker.v0.json"
PRIVATE_MANIFEST_RELATIVE_NAME = "cohort.private.v0.json"
REPORT_RELATIVE_NAME = "selection_boundary_private_confirmation.aggregate.v0.json"
PRIVATE_SOURCE_RELATIVE_PATH = Path(
    ".codex_work/marc1_central_directory/live_audit_v0/member_inventory.private.v0.json"
)

DECISION_SHA256 = "7d7adf8972f8bf6a021c29bcabed1d6cb57cda9df9d4e5d4e5b97175972ddbbf"
REQUEST_SHA256 = "27a21a90e96f6b0fca6fe7a997ca814e043356a30c5ada4e5fb850f3ff83ff49"
GREEN_DECISION_COMMIT = "8b9ce248f6cbce1205addf113f97325a98c1992e"
GREEN_DECISION_CI_RUN_ID = 32_608_318_342
GREEN_DECISION_BASE_JOB_ID = 97_116_976_065
GREEN_DECISION_OPTIONAL_JOB_ID = 97_116_976_016
VR25A_MODULE_SHA256 = "20516d466de74256a6e1640a96a757719d7c48c664483e7cc429481d70839868"
VR25A_CONTRACT_SHA256 = "c0af130d51e68151757e85d14674e327673cb2586dab866bc4437320952d6ea1"
VR25A_IMPLEMENTATION_SHA256 = "e186853a79a3abe9143de32ee1f83ec0dd6c1ee4bba8e65069d5c255d3cd2aaf"
VR25A_RESULT_SHA256 = "a860511578617ac1d75dec2df07b045e50df6dabc9c0756b4bbc0d1fb936cabd"
...
SUCCESS_ROUTE = "MARC2VR26P-G1"
PRIVATE_SUCCESS_ROUTES = ("MARC2VR26P-R1", "MARC2VR26P-R2")
PRIVATE_FAILURE_ROUTES = tuple(f"MARC2VR26P-R{index}" for index in range(3, 8))
REFUSAL_ROUTES = tuple(f"MARC2VR26P-F{index:02d}" for index in range(1, 13))
...
GENERATED_CASES = firewall.CASES
GENERATED_CASE_ROUTES = {
    "exact_public_control": SUCCESS_ROUTE,
    "single_session_exclusion_removed": SUCCESS_ROUTE,
    "single_session_exclusion_added": SUCCESS_ROUTE,
    "sampling_exclusion_removed": SUCCESS_ROUTE,
    "outside_pair_bundle_added": SUCCESS_ROUTE,
    "eligible_bundle_removed": PRIVATE_FAILURE_ROUTES[2],
    "eligible_bundle_added": PRIVATE_FAILURE_ROUTES[2],
    "eligible_distribution_shift": PRIVATE_FAILURE_ROUTES[2],
    "unknown_participant_bundle": PRIVATE_FAILURE_ROUTES[2],
    "incomplete_companion_set": PRIVATE_FAILURE_ROUTES[1],
}
...
class SelectionBoundaryPrivateConfirmationRefusal(RuntimeError):
    ...
    def __init__(self, route: str, reason: str):
        if route not in REFUSAL_ROUTES and route not in PRIVATE_FAILURE_ROUTES:
            raise ValueError("unknown MARC2-VR26P refusal route")
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise SelectionBoundaryPrivateConfirmationRefusal(
...
def _strict_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "duplicate JSON key")
        result[key] = value
    return result


def _reject_constant(_value: str) -> None:
    raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "non-finite JSON number")
...
def _strict_json(payload: bytes) -> dict[str, Any]:
    ...
    except SelectionBoundaryPrivateConfirmationRefusal:
        raise
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "strict JSON parse failed") from exc
    if not isinstance(value, dict):
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "JSON root is not an object")
    return value


def _read_tracked(root: Path, relative: Path) -> bytes:
    if relative.is_absolute() or ".." in relative.parts:
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "tracked path is unsafe")
    path = root / relative
    info = path.lstat()
...
def load_decision(root: Path | None = None) -> dict[str, Any]:
    ...
    if _sha256_bytes(payload) != DECISION_SHA256:
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "decision identity differs")
    decision = _strict_json(payload)
    _validate_decision(decision)
    return decision


def _validate_vr25a_artifacts(root: Path | None = None) -> None:
    repo = root or _repo_root()
    expected = (
        (VR25A_MODULE_RELATIVE_PATH, VR25A_MODULE_SHA256),
        (VR25A_CONTRACT_RELATIVE_PATH, VR25A_CONTRACT_SHA256),
        (VR25A_IMPLEMENTATION_RELATIVE_PATH, VR25A_IMPLEMENTATION_SHA256),
        (VR25A_RESULT_RELATIVE_PATH, VR25A_RESULT_SHA256),
    )
    for relative, digest in expected:
        if _sha256_bytes(_read_tracked(repo, relative)) != digest:
            raise SelectionBoundaryPrivateConfirmationRefusal(
                REFUSAL_ROUTES[0], "VR25A artifact identity differs"
            )
...
def _validate_decision(decision: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "efd779a2d8bafbd4efbf5618fadf2355f4f89ee4"
        or not isinstance(request, dict)
        or request.get("commit") != "00db8254f67dd349bddb8a906b57d7e28c2f7101"
        or request.get("both_required_jobs_green") is not True
        or not isinstance(proof, dict)
        or proof.get("commit") != "efd779a2d8bafbd4efbf5618fadf2355f4f89ee4"
        ...
        or authority.get("one_VR25A_call_after_stage_1_proof_green") is not True
        or authority.get("private_cohort_freeze_on_R1_or_R2") is not True
        or authority.get("MARC2_FW2_or_CIL1_real_execution_authorized_now") is not False
    ):
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "decision contract differs")
...
def _validate_thread_environment(environment: Mapping[str, str] | None = None) -> None:
    ...
    if any(values.get(name) != "1" for name in THREAD_ENVIRONMENT):
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[1], "thread environment is not one")
...
def _assert_resources(
    ...
    if (
        ...
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[10], "resource or output cap exceeded")


def _safe_parent_chain(root: Path, relative: Path) -> None:
    if relative.is_absolute() or ".." in relative.parts:
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[2], "unsafe fixed path")
    current = root
    for part in relative.parts[:-1]:
        current = current / part
        if current.exists() and current.is_symlink():
            raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[2], "fixed parent is a symlink")
...
def _create_fresh_directory(root: Path, relative: Path) -> Path:
    ...
    if path.exists() or path.is_symlink():
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[2], "fixed output already exists")
...
def _write_exclusive(path: Path, payload: bytes, mode: int) -> int:
    ...
    try:
        ...
        if written != len(payload):
            raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[9], "short fixed-path write")
    finally:
        os.close(descriptor)
    os.chmod(path, mode)
    return len(payload)
...
def _sample_time(value: Any) -> datetime:
    if not isinstance(value, str):
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness timestamp is missing")
    try:
    ...
    if parsed.tzinfo is None:
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness timestamp lacks timezone")
    return parsed.astimezone(UTC)


def _validate_samples(samples: Sequence[Mapping[str, Any]]) -> None:
    if len(samples) != REQUIRED_PASSING_SAMPLES:
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness sample count differs")
    ...
    for index, sample in enumerate(samples):
        if (
            ...
            raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness sample failed")
...
def _preflight_source(path: Path, identity: Mapping[str, Any]) -> None:
    ...
    if stat.S_IMODE(info.st_mode) != identity["mode"] or info.st_size != identity["bytes"]:
        raise SelectionBoundaryPrivateConfirmationRefusal(PRIVATE_FAILURE_ROUTES[1], "source identity differs")
...
def _private_manifest(
    repaired: firewall.FirewallSelection,
    *,
    source_sha256: str,
    ...
    return {
        ...
        "implementation_commit": implementation_commit,
        "VR25A_module_sha256": VR25A_MODULE_SHA256,
        "VR25A_contract_sha256": VR25A_CONTRACT_SHA256,
        ...
        "full_source_bundle_count_matches_public": (
            repaired.full_source_bundle_count_matches_public
        ),
...
def _aggregate_report(
    ...
    *,
    ...
    counters["VR25A_calls"] = 1
    counters["private_cohort_freezes"] = 0 if generated else 1
    private_route = PRIVATE_SUCCESS_ROUTES[
        0 if repaired.full_source_bundle_count_matches_public else 1
    ]
    report = {
        "schema_name": REPORT_SCHEMA_NAME,
        "schema_version": SCHEMA_VERSION,
        "lane_id": LANE_ID,
        "route": SUCCESS_ROUTE if generated else private_route,
        "status": (
            "generated_fixture_qualified"
            if generated
            else "consumed_exact_cohort_frozen"
        ),
        "proof": {
        ...
        "warnings": [
            ...
            "CIL1_not_authorized",
            "observed_complete_bundle_count_difference_and_direction_not_exposed",
            "no_neural_decoding_or_scientific_claim",
        ],
        "unavailable_fields": [
            "archive_member_payload",
            "observed_complete_bundle_count_difference_or_direction",
            "neural_signal",
            "target_or_label",
...
def _private_route_for_refusal(exc: SelectionBoundaryPrivateConfirmationRefusal) -> str:
    ...
    counters.update(
        {
            ...
            "VR25A_calls": accounting["VR25A_calls"],
...
def _call_generated_adapter(
    source: Mapping[str, Any], generated_case: str
) -> firewall.FirewallSelection:
    if generated_case not in GENERATED_CASES:
        raise SelectionBoundaryPrivateConfirmationRefusal(
            REFUSAL_ROUTES[7], "generated case is not registered"
        )
    try:
        selected = firewall.apply_selection_boundary_firewall(source)
    except firewall.SelectionBoundaryFirewallRefusal as exc:
        expected = firewall.EXPECTED_CASE_ROUTES[generated_case]
        if exc.route != expected:
            raise SelectionBoundaryPrivateConfirmationRefusal(
                PRIVATE_FAILURE_ROUTES[4], "VR25A generated route differs"
            ) from exc
        route = {
            "MARC2VR25A-R1": PRIVATE_FAILURE_ROUTES[2],
            "MARC2VR25A-R2": PRIVATE_FAILURE_ROUTES[2],
            "MARC2VR25A-R3": PRIVATE_FAILURE_ROUTES[1],
            "MARC2VR25A-R4": PRIVATE_FAILURE_ROUTES[3],
        }.get(exc.route, PRIVATE_FAILURE_ROUTES[4])
        raise SelectionBoundaryPrivateConfirmationRefusal(
            route, "VR25A refused generated source"
        ) from exc
    if selected.route != firewall.EXPECTED_CASE_ROUTES[generated_case]:
        raise SelectionBoundaryPrivateConfirmationRefusal(
            PRIVATE_FAILURE_ROUTES[4], "VR25A generated success route differs"
        )
    return selected
...
def _run_fixed_sequence(
    ...
    *,
    ...
    implementation_commit: str,
    generated: bool,
    generated_case: str = "exact_public_control",
    ...
    if len(certificate_bytes) > MAX_CERTIFICATE_BYTES:
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[10], "readiness certificate exceeds cap")
    ...
    def finish_failure(exc: SelectionBoundaryPrivateConfirmationRefusal) -> dict[str, Any]:
    ...
    except SelectionBoundaryPrivateConfirmationRefusal as exc:
    ...
    try:
        ...
        accounting["VR25A_calls"] = 1
        repaired = (
            _call_generated_adapter(source, generated_case)
            if generated
            else firewall.apply_selection_boundary_firewall(source)
        )
    except firewall.SelectionBoundaryFirewallRefusal as exc:
        route = {
            "MARC2VR25A-R1": PRIVATE_FAILURE_ROUTES[2],
        ...
        return finish_failure(
            SelectionBoundaryPrivateConfirmationRefusal(route, "VR25A refused source")
        )
    except SelectionBoundaryPrivateConfirmationRefusal as exc:
        return finish_failure(exc)
...
_ENVELOPE_EXPECTED: dict[str, Any] = {
    ...
    "marker_writes": 1,
    "VR25A_calls": 1,
    "VR25A_calls_per_path": 1,
    "VR25A_source_exact": True,
    "VR25A_source_immutable": True,
    "VR25A_all_rows_validated": True,
    "VR25A_all_companion_groups_validated": True,
    "VR25A_exact_eligible_bundles": 195,
    "VR25A_exact_eligible_distribution": True,
    "VR25A_known_ineligible_quarantined": True,
    "VR25A_unknown_participants_refused": True,
    "VR25A_eligible_drift_refused": True,
    "VR25A_incomplete_companions_refused": True,
    "VR25A_selection_deterministic": True,
    "VR25A_source_exact_names": True,
    "VR25A_source_exact_reservation": True,
    "VR25A_identity_task_companion_broadening": False,
    "VR25A_selection_cap_or_rank_relaxation": False,
    "VR25A_success_route_1": "MARC2VR25A-G1",
    "VR25A_success_route_2": "MARC2VR25A-G2",
    "VR26P_success_route_1": "MARC2VR26P-R1",
    "VR26P_success_route_2": "MARC2VR26P-R2",
    "cohort_selected_subjects": 16,
    "cohort_runs_per_subject": 6,
    "cohort_members_per_subject": 24,
    "cohort_selected_run_bundles": 96,
    "cohort_fit_run_bundles": 48,
    "cohort_heldout_run_bundles": 48,
    "cohort_selected_core_members": 384,
    "cohort_equal_splits": True,
    "cohort_contiguous_prefix": True,
    "cohort_reservation_cap": 8 * 1024**3,
    "cohort_fit_session": "ses-01",
    "cohort_heldout_session": "ses-02",
    "cohort_fit_heldout_overlap": 0,
    "cohort_target_fields": 0,
    "cohort_quality_fields": 0,
    "cohort_outcome_fields": 0,
    "private_manifest_mode": 0o600,
    "private_manifest_R1_or_R2_only": True,
    "private_manifest_source_exact": True,
    "private_manifest_failure_route_allowed": False,
    ...
    "aggregate_compatibility_boolean_allowed": True,
    "aggregate_observed_complete_bundle_count_allowed": False,
    "aggregate_count_difference_or_direction_allowed": False,
    "network_bytes": 0,
    "new_payload_bytes": 0,
    "archive_bytes": 0,
    "signal_bytes": 0,
...
def _validate_envelope(envelope: Mapping[str, Any]) -> None:
    if set(envelope) != set(_ENVELOPE_EXPECTED):
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[11], "execution envelope fields differ")
...
def qualify_generated(
    *,
    ...
    for replay in range(2):
        for generated_case in GENERATED_CASES:
            for order in firewall.ORDERS:
                source = firewall.build_generated_case(generated_case, order)
                payload = firewall._source_bytes(source)
                ...
                with tempfile.TemporaryDirectory(prefix="ndk-vr26p-") as temporary:
                    root = Path(temporary)
                    source_path = root / PRIVATE_SOURCE_RELATIVE_PATH
                    ...
                    if result["report"]["route"] == SUCCESS_ROUTE:
                        cohort = result["report"]["cohort"]
                        current = cohort["semantic_cohort_sha256"]
                        current_names = cohort[
                            "source_exact_selected_names_sha256"
                        ]
                        if semantic_hash is None:
                            semantic_hash = current
                            names_hash = current_names
                        elif current != semantic_hash:
                            raise SelectionBoundaryPrivateConfirmationRefusal(
                                REFUSAL_ROUTES[7],
                                "generated semantic cohort differs",
                            )
                        elif current_names != names_hash:
                            raise SelectionBoundaryPrivateConfirmationRefusal(
                                REFUSAL_ROUTES[7],
                                "generated source-exact selection differs",
                            )
                        outcome.update(
                            {
                                "selected_subject_count": cohort[
                                    "selected_subject_count"
                                ],
                                "selected_run_bundle_count": cohort[
                                    "selected_run_bundle_count"
                                ],
                                "selected_core_member_count": cohort[
                                    "selected_core_member_count"
                                ],
                                "full_source_bundle_count_matches_public": cohort[
                                    "full_source_bundle_count_matches_public"
                                ],
                            }
                        )
    ...
    expected_route_counts = {
        SUCCESS_ROUTE: 20,
        PRIVATE_FAILURE_ROUTES[1]: 4,
        PRIVATE_FAILURE_ROUTES[2]: 16,
    }
    route_counts = {
    ...
    if (
        ...
        or len(success_outcomes) != 20
        or sum(item["VR25A_calls"] for item in outcomes) != 40
        or sum(item["source_content_opens"] for item in outcomes) != 40
        or any(len(set(values)) != 1 for values in replay_hashes.values())
        or sum(
            bool(item.get("full_source_bundle_count_matches_public"))
    ...
    if direct_refusals < 90:
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[11], "direct refusal floor not met")
    elapsed = clock() - started
    rss = peak_rss()
    report = {
        ...
        "matrix": {
            ...
            "VR25A_calls": sum(item["VR25A_calls"] for item in outcomes),
            ...
            "selected_subject_count": success_outcomes[0]["selected_subject_count"],
            "selected_run_bundle_count": success_outcomes[0][
                "selected_run_bundle_count"
            ],
            "selected_core_member_count": success_outcomes[0][
                "selected_core_member_count"
            ],
            "compatibility_true_paths": 4,
            "compatibility_false_paths": 16,
...
def _load_implementation(root: Path) -> dict[str, Any]:
    ...
    if (
        record.get("schema_name") != "neurodecodekit.marc2_selection_boundary_private_confirmation_implementation"
        or record.get("lane_id") != LANE_ID
    ):
        raise SelectionBoundaryPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "implementation record differs")
    return record
...
def _collect_readiness(root: Path) -> list[dict[str, Any]]:
    ...
    raise SelectionBoundaryPrivateConfirmationRefusal(PRIVATE_FAILURE_ROUTES[0], "machine readiness did not pass")
...
def execute_registered() -> dict[str, Any]:
    ...
    _validate_decision(decision)
    _validate_vr25a_artifacts(root)
...
def build_plan() -> dict[str, Any]:
    ...
    return {
        ...
        "decision_green": True,
        "generated_paths": 40,
        "generated_success_paths": 20,
        "generated_refusal_paths": 20,
        "VR25A_calls": 40,
        "direct_refusal_minimum": 90,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Proof-gated MARC2 selection-boundary target-free structural confirmation."
    )
