"""Proof-gated MARC2 variable-width structural confirmation wrapper.
...
from neurodecodekit.datasets import marc2_variable_width_run_index_repair as repair


LANE_ID = "MARC2-VR16P"
SCHEMA_VERSION = "0.1.0"
PLAN_SCHEMA_NAME = "neurodecodekit.marc2_variable_width_private_confirmation_plan"
QUALIFICATION_SCHEMA_NAME = "neurodecodekit.marc2_variable_width_private_confirmation_qualification"
REPORT_SCHEMA_NAME = "neurodecodekit.marc2_variable_width_private_confirmation_report"
PRIVATE_SCHEMA_NAME = "neurodecodekit.marc2_variable_width_private_cohort"
CERTIFICATE_SCHEMA_NAME = "neurodecodekit.marc2_variable_width_readiness_certificate"
MARKER_SCHEMA_NAME = "neurodecodekit.marc2_variable_width_private_confirmation_consumed"

DECISION_RELATIVE_PATH = Path(
    "registries/marc2_variable_width_private_confirmation_authorization_decision.v0.json"
)
...
VR16A_MODULE_RELATIVE_PATH = Path("src/neurodecodekit/datasets/marc2_variable_width_run_index_repair.py")
VR16A_IMPLEMENTATION_RELATIVE_PATH = Path(
    "registries/marc2_variable_width_run_index_repair_implementation.v0.json"
)
VR16A_RESULT_RELATIVE_PATH = Path("registries/marc2_variable_width_run_index_repair_result.v0.json")
READINESS_RELATIVE_PATH = Path(".codex_work/marc2_machine_readiness/vr16p/readiness.v0.json")
OUTPUT_ROOT_RELATIVE_PATH = Path(".codex_work/marc2_variable_width_private_confirmation/v0")
MARKER_RELATIVE_NAME = "consumed_marker.v0.json"
PRIVATE_MANIFEST_RELATIVE_NAME = "cohort.private.v0.json"
REPORT_RELATIVE_NAME = "variable_width_private_confirmation.aggregate.v0.json"
PRIVATE_SOURCE_RELATIVE_PATH = Path(
    ".codex_work/marc1_central_directory/live_audit_v0/member_inventory.private.v0.json"
)

DECISION_SHA256 = "da8afdb0b543e39c0fe9402532525489c3cbf27ea742689e59f208e50c6f721f"
REQUEST_SHA256 = "7c4a6f17b288eb3635a50b8199e9490c1191d4fa63b10495aeb77265c0c85067"
GREEN_DECISION_COMMIT = "79d9b49c94063e68663d3774d6e6296961a544a6"
GREEN_DECISION_CI_RUN_ID = 32_463_248_736
GREEN_DECISION_BASE_JOB_ID = 96_714_374_482
GREEN_DECISION_OPTIONAL_JOB_ID = 96_714_374_314
VR16A_MODULE_SHA256 = "1a69d42236af61b1e5565dc89903aa20e9416732752aa5b5b1ffe3a95003f3f1"
VR16A_IMPLEMENTATION_SHA256 = "a11714c7860713a2b8ce82ed2d7a4f045d48582a36e1870b3f034719acbef7bd"
VR16A_RESULT_SHA256 = "5d873da94343c3b4a55c812088c00a5d2c9cc2b0f0e97bea9fd3b8c79854a652"
...
SUCCESS_ROUTE = "MARC2VR16P-G1"
PRIVATE_SUCCESS_ROUTE = "MARC2VR16P-R1"
PRIVATE_FAILURE_ROUTES = tuple(f"MARC2VR16P-R{index}" for index in range(2, 7))
REFUSAL_ROUTES = tuple(f"MARC2VR16P-F{index:02d}" for index in range(1, 13))
...
class VariableWidthPrivateConfirmationRefusal(RuntimeError):
    ...
    def __init__(self, route: str, reason: str):
        if route not in REFUSAL_ROUTES and route not in PRIVATE_FAILURE_ROUTES:
            raise ValueError("unknown MARC2-VR16P refusal route")
...
def _canonical_json_bytes(value: Any) -> bytes:
    try:
        ...
        raise VariableWidthPrivateConfirmationRefusal(
...
def _strict_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "duplicate JSON key")
        result[key] = value
    return result


def _reject_constant(_value: str) -> None:
    raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "non-finite JSON number")
...
def _strict_json(payload: bytes) -> dict[str, Any]:
    ...
    except VariableWidthPrivateConfirmationRefusal:
        raise
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "strict JSON parse failed") from exc
    if not isinstance(value, dict):
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[6], "JSON root is not an object")
    return value


def _read_tracked(root: Path, relative: Path) -> bytes:
    if relative.is_absolute() or ".." in relative.parts:
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "tracked path is unsafe")
    path = root / relative
    info = path.lstat()
...
def load_decision(root: Path | None = None) -> dict[str, Any]:
    ...
    if _sha256_bytes(payload) != DECISION_SHA256:
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "decision identity differs")
    decision = _strict_json(payload)
    _validate_decision(decision)
    return decision


def _validate_vr16a_artifacts(root: Path | None = None) -> None:
    repo = root or _repo_root()
    expected = (
        (VR16A_MODULE_RELATIVE_PATH, VR16A_MODULE_SHA256),
        (VR16A_IMPLEMENTATION_RELATIVE_PATH, VR16A_IMPLEMENTATION_SHA256),
        (VR16A_RESULT_RELATIVE_PATH, VR16A_RESULT_SHA256),
    )
    for relative, digest in expected:
        if _sha256_bytes(_read_tracked(repo, relative)) != digest:
            raise VariableWidthPrivateConfirmationRefusal(
                REFUSAL_ROUTES[0], "VR16A artifact identity differs"
            )
...
def _validate_decision(decision: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        != "neurodecodekit.marc2_variable_width_private_confirmation_authorization_decision"
        or decision.get("lane_id") != LANE_ID
        or decision.get("authorization_parent_commit")
        != "b12aeabfa1d192915cebfdbd79693c937801db90"
        or not isinstance(request, dict)
        or request.get("commit") != "619469a795a7044a4bbb77cef8986e0a7744473f"
        or request.get("both_required_jobs_green") is not True
        or not isinstance(proof, dict)
        or proof.get("commit") != "b12aeabfa1d192915cebfdbd79693c937801db90"
        ...
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "decision contract differs")
...
def _validate_thread_environment(environment: Mapping[str, str] | None = None) -> None:
    ...
    if any(values.get(name) != "1" for name in THREAD_ENVIRONMENT):
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[1], "thread environment is not one")
...
def _assert_resources(
    ...
    if (
        ...
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[10], "resource or output cap exceeded")


def _safe_parent_chain(root: Path, relative: Path) -> None:
    if relative.is_absolute() or ".." in relative.parts:
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[2], "unsafe fixed path")
    current = root
    for part in relative.parts[:-1]:
        current = current / part
        if current.exists() and current.is_symlink():
            raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[2], "fixed parent is a symlink")
...
def _create_fresh_directory(root: Path, relative: Path) -> Path:
    ...
    if path.exists() or path.is_symlink():
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[2], "fixed output already exists")
...
def _write_exclusive(path: Path, payload: bytes, mode: int) -> int:
    ...
    try:
        ...
        if written != len(payload):
            raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[9], "short fixed-path write")
    finally:
        os.close(descriptor)
    os.chmod(path, mode)
    return len(payload)
...
def _sample_time(value: Any) -> datetime:
    if not isinstance(value, str):
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness timestamp is missing")
    try:
    ...
    if parsed.tzinfo is None:
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness timestamp lacks timezone")
    return parsed.astimezone(UTC)


def _validate_samples(samples: Sequence[Mapping[str, Any]]) -> None:
    if len(samples) != REQUIRED_PASSING_SAMPLES:
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness sample count differs")
    ...
    for index, sample in enumerate(samples):
        if (
            ...
            raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[4], "readiness sample failed")
...
def _preflight_source(path: Path, identity: Mapping[str, Any]) -> None:
    ...
    if stat.S_IMODE(info.st_mode) != identity["mode"] or info.st_size != identity["bytes"]:
        raise VariableWidthPrivateConfirmationRefusal(PRIVATE_FAILURE_ROUTES[1], "source identity differs")
...
def _private_manifest(
    repaired: repair.VariableWidthSelection,
    *,
    source_sha256: str,
    ...
    return {
        ...
        "implementation_commit": implementation_commit,
        "VR16A_module_sha256": VR16A_MODULE_SHA256,
...
def _private_route_for_refusal(exc: VariableWidthPrivateConfirmationRefusal) -> str:
    ...
    counters.update(
        {
            ...
            "VR16A_calls": accounting["VR16A_calls"],
...
def _run_fixed_sequence(
    ...
    *,
    ...
    if len(certificate_bytes) > MAX_CERTIFICATE_BYTES:
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[10], "readiness certificate exceeds cap")
    ...
    def finish_failure(exc: VariableWidthPrivateConfirmationRefusal) -> dict[str, Any]:
        if generated:
            raise exc
    ...
    except VariableWidthPrivateConfirmationRefusal as exc:
    ...
    try:
        ...
        accounting["VR16A_calls"] = 1
        repaired = repair.adapt_variable_width_source(source)
    except repair.VariableWidthRunIndexRepairRefusal as exc:
        if exc.route == repair.REFUSAL_ROUTES[1]:
        ...
        return finish_failure(VariableWidthPrivateConfirmationRefusal(route, "VR16A refused source"))
    except VariableWidthPrivateConfirmationRefusal as exc:
        return finish_failure(exc)
...
_ENVELOPE_EXPECTED: dict[str, Any] = {
    ...
    "marker_writes": 1,
    "VR16A_calls": 1,
    "VR16A_source_exact": True,
    "VR16A_selection_deterministic": True,
    "VR16A_run_token_regex": "[0-9]+",
    "VR16A_width_ceiling_absent": True,
    "VR16A_numeric_identity_canonical": True,
    "VR16A_selected_semantic_runs": "1,2,3",
    "VR16A_source_exact_names": True,
    "VR16A_source_exact_reservation": True,
    "VR16A_companion_spelling_exact": True,
    "VR16A_normalized_collision_refused": True,
    "VR16A_source_immutable": True,
    "VR16A_calls_per_path": 1,
...
def _validate_envelope(envelope: Mapping[str, Any]) -> None:
    if set(envelope) != set(_ENVELOPE_EXPECTED):
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[11], "execution envelope fields differ")
...
def qualify_generated(
    *,
    ...
    for replay in range(2):
        for variant in repair.VARIANTS:
            for order in repair.ORDERS:
                ...
                with tempfile.TemporaryDirectory(prefix="ndk-vr16p-") as temporary:
                    root = Path(temporary)
                    source_path = root / PRIVATE_SOURCE_RELATIVE_PATH
                    ...
                    outcomes.append(
                        {
                            ...
                            "VR16A_calls": result["VR16A_calls"],
    ...
    if (
        ...
        or sum(item["VR16A_calls"] for item in outcomes) != 24
        or sum(item["source_content_opens"] for item in outcomes) != 24
        or peak_generated_output_bytes > MAX_COMBINED_OUTPUT_BYTES
    ):
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[7], "generated success matrix differs")
    refusals = _run_direct_refusals()
    direct_refusals = sum(refusals.values())
    if direct_refusals < 70:
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[11], "direct refusal floor not met")
    elapsed = clock() - started
    rss = peak_rss()
    report = {
        ...
        "matrix": {
            ...
            "VR16A_calls": sum(item["VR16A_calls"] for item in outcomes),
...
def _load_implementation(root: Path) -> dict[str, Any]:
    ...
    if (
        record.get("schema_name") != "neurodecodekit.marc2_variable_width_private_confirmation_implementation"
        or record.get("lane_id") != LANE_ID
    ):
        raise VariableWidthPrivateConfirmationRefusal(REFUSAL_ROUTES[0], "implementation record differs")
    return record
...
def _collect_readiness(root: Path) -> list[dict[str, Any]]:
    ...
    raise VariableWidthPrivateConfirmationRefusal(PRIVATE_FAILURE_ROUTES[0], "machine readiness did not pass")
...
def execute_registered() -> dict[str, Any]:
    ...
    _validate_decision(decision)
    _validate_vr16a_artifacts(root)
...
def build_plan() -> dict[str, Any]:
    ...
    return {
        ...
        "decision_green": True,
        "generated_success_paths": 24,
        "direct_refusal_minimum": 70,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Proof-gated MARC2 variable-width target-free structural confirmation."
    )
