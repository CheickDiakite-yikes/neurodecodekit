...
and its machine decision. Decision commit
`1efeac7f0b7b316bb94effb1a2eeeb1bbf99f50a` is now green in CI
`31355944651`, including Base Python job `93355535398` and Optional Neuro
Readers job `93355535361`. The exact acquisition, target-firewall, 144-fit,
216-prediction-set, combined-freeze, and isolated-scorer implementation is now
generated-fixture qualified in
...
One 72-run synthetic qualification completed in 12.083017 seconds at
260,784,128-byte peak RSS with 4,215,687 generated bytes, zero real reads, and
zero network bytes; all generated files were removed. Implementation `8242674`
then passed Base Python job `93365527795` and Optional Neuro Readers job
`93365527849` in CI `31359548779` before the one real acquisition and target-
blind analysis. The acquisition matched all 72 official hashes over
184,252,032 bytes in 518.051205 seconds at 73,089,024-byte peak RSS. The
analysis accepted 1,080 events, created 720 fit rows and 360 target-free final
rows, completed exactly 144 fits and 216 prediction sets, and froze them in
`registries/physionet_low_frequency_cohort_confirmation_prediction_freeze.v0.json`
in 19.864386 seconds at 303,153,152-byte peak RSS. No final target reached the
model stage and no score exists. Do not open either final target set until this
exact combined hash-only freeze commit is pushed and remotely green.
