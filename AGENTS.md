...
Work order 9 has a frozen contract, remotely green exact Tier C decision, and
locally fixture-qualified implementation. Its real execution is still closed
until the exact implementation commit is remotely green:
...
value. Do not stat or open the registered PhysioNet bundle until this exact
implementation is committed, pushed, and both CI jobs are green. After one
successful target-blind execution, do not open the sealed targets until the
aggregate freeze ledger is committed, pushed, and remotely green. No retry or
rerun is authorized at either real stage.
