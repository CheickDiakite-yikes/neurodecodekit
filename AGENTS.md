...
pushed and remotely green. Decision commit
...
zero network bytes; all generated files were removed. Do not acquire or inspect
the registered S004-S015 bundle until this exact implementation commit is
pushed and both CI jobs are green. After that, consume only the one registered
acquisition and target-blind analysis; the 360 final targets remain closed
until the combined hash-only freeze commit is remotely green.
