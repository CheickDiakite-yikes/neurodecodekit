...
VR35A qualification may not be repeated. Immediate gate: commit, push, and
green the exact implementation/result, then add and green a proof-only
closeout without requalification. This proves a generated architectural
mechanism and repair, not that mixed-task surplus caused private VR34P R2. Do
not touch any private/ignored source, readiness or consumed state, archive,
neural payload, target, model, score, FW2/CIL1, device, release, or claim
surface. A private cohort confirmation remains a separate Tier C packet and
decision.
