...
`MARC2-VR30P` Stage 1 is generated-qualified and pending exact remote proof.
...
`97154390379`, and CI `32623171395` before implementation. Read the VR30P
authorization packet, request proof, decision, implementation document,
module, tests, and machine records first.

One measured `MARC2VR30P-G1` qualification passed all 32 generated paths with
32 unchanged VR29A calls, 32 nested unchanged VR25A calls, 16 nested unchanged
VR2 eligible-filter calls, G1/G2/R1/R2/R3 counts 4/4/8/8/8, 151 direct
refusals, exact replay, marker-before-open ordering, source immutability, and
zero retention. It processed 14,137,216 generated bytes in
2.467852458008565 seconds at 34,127,872-byte peak RSS. Every private,
`.codex_work`, consumed-state, archive, neural, target, model, prediction,
score, network, FW2/CIL1, hardware, other-project, and claim counter was zero.

Immediate gate: commit, push, and green the exact Stage 1 implementation. Then
prepare a proof-only closeout binding those exact bytes without repeating the
qualification or touching private state; commit, push, and green that closeout.
Only after both jobs pass may the already authorized one-shot Stage 2 readiness
and 418,755-byte target-free structural read run. Until then `execute` must
refuse at `MARC2VR30P-F02` before request loading, readiness, path construction,
or private access. Do not touch any consumed/private state or infer a route.
