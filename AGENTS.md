...
`MARC2-VR29A` is generated-qualified with remote implementation proof pending.
Read
...
and separately green a proof-only closeout without rerunning qualification or
touching any private state. VR29A has no private executor and does not identify
the consumed private R1 subclass. Only after that closeout may Tier A prepare
a new all-false Tier C packet; another private read still requires fresh
packet-bound maintainer words and a separately green decision. Do not touch
VR28P private source, readiness, consumed marker, or output.
