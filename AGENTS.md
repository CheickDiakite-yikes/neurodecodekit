...
on GitHub `main`. Generated-only `FMSR1-DISCOVERY-M0-G1` is exact-green at
`e92e9c21a044b05a187a04812f21e7e7bd5a76b6`, Base Python
`99327605264`, Optional Neuro Readers `99327605346`, and CI `33337741165`
on GitHub `main`. It passes two
...
The historical all-false `DREYER-C5R-1-HL1R1` recovery packet required and received
...
then reached GitHub `main`. Its historical all-false qualification request was
...
then reached GitHub `main`. Exact proof
`d1c003f303083d23685da858bca069397b1f9c58` passed Base
...
then reached GitHub `main`. Its additive proof-only closeout
...
then reached GitHub `main`. The later all-false
`DREYER-C5R-1-HL2-A0` fixed-header activation request proposed one
...
exact-green at `53f2c48831d1db7875bf09a35f107e35f97c6bf4`, Base Python
`99114895023`, Optional Neuro Readers `99114895128`, and CI `33257975186`, and
is on GitHub `main`. The additive standard-library H-L2 adapter and its
generated qualification are exact-green at
`a9cd0be7c22996154c28bb568e05c623606e7424`, Base Python `99121591361`,
Optional Neuro Readers `99121591482`, and CI `33260534900` on GitHub `main`;
read their implementation document and machine record. The 32-case /
33-attempt matrix produced two matching H1 replays, 31 refusal observations,
zero network or real/private operations, and zero retained generated bytes in
0.599492667010054 seconds at 39,141,376-byte peak RSS. No-authority activation
...
`33261346763` on GitHub `main`. The sole real invocation then consumed at H0:
one GET and response open, zero body bytes, zero EDF header reads, zero payload
retained, 0.6479716249741614 seconds, and 36,454,400-byte peak RSS. The result
is public; the ignored evidence remains closed.
`COMM-P0-G-FS3` is paused as an adjacent engineering diagnostic; do not activate
it under general continuation. No new target, model, score, device, release, or
claim operation is authorized by this state.
...
activation, or claim upgrade is authorized. This is historical routing;
`DREYER-C5R-1-HL2-R0` later consumed at transport H0 and no Tier C gate is
active now.
...
target, model, score, stream, provider, device, or claim operation. It did not
replace the then-active Dreyer gate and remains inactive now.
...
The generated-only COMM-L0 implementation is recorded in
`src/neurodecodekit/datasets/communication_eeg_source_identity.py`, with its
exact record in
`registries/communication_eeg_source_identity_implementation.v0.json`. Exact
implementation `be6c44fcfd7363bf0c0899bc1b737e588070c760` passed Base job
`98405103426`, Optional Neuro Readers job `98405103509`, and CI
`33038044421`. Its sole generated qualification then passed two deterministic
replays and all 20 adversarial refusals in 0.07287466689012945 seconds at
22,069,248-byte peak RSS. Read
`docs/COMMUNICATION_EEG_SOURCE_IDENTITY_GENERATED_QUALIFICATION_RESULT.md` and
its machine result. The qualification is consumed and must not be rerun. It
...
was the sole active Tier C lane at this historical point. H-L1 R0 was consumed
and rejected; the later H-L2 R0 also consumed at transport H0 and parked the
lane.
...
Exact numerical/scorer `b7e4d8376e7ebc907d530df54d261667fa25c807`
passed Base Python `98751222613`, Optional Neuro Readers `98751222562`, and CI
`33140872221`, then reached GitHub `main`. The additive endpoint-separated live
scorer is exact and green at
`c46064d19bd0ec74bc960d155ad9752989d9c54c`, Base Python `98754821063`,
Optional Neuro Readers `98754821135`, and CI `33141715948`, and is also on
GitHub `main`. It makes free choice the sole primary live gate, keeps prompted
performance diagnostic and non-rescuing, counts inactive intervals once, and
keeps end-to-end latency false.

Read the additive runner scaffold implementation document and machine record
next. The reduced two-replay development route completed over three fictional
participants per cohort in 6.678607375011779 seconds at 187,645,952-byte peak
RSS, 4,071,800 private generated bytes, zero network bytes, and zero retained
payload. Its two replays matched the complete 15-digest surface and emitted 140
structural refusal observations. This is not the official qualification: the
70 families are not all routed through domain-specific validators, full
predictions still require bounded file/descriptor transport, scorer authority
still needs a cryptographic freeze attestation and no-model capability, and no
...
Historical `DREYER-C5R-1-HL` detail follows. The packet is now consumed and
parked, and no Tier C packet is active. Read, in order:

1. `registries/current_research_frontier.v0.json`;
2. `docs/BNCI_2014_001_ARTIFACT_POSTMORTEM_RESULT.md`;
3. `docs/DREYER_2023_DATASET_A_REPLICATION_PRIMARY_SOURCE_RESEARCH.md`;
4. `docs/DREYER_C5R_1_PREREGISTRATION.md`;
5. `registries/dreyer_c5r_1_generated_qualification_result.v0.json`;
...
specificity, probability reliability, and participant consistency; EEG beyond
recorded EOG remained weak directional evidence only. Never reopen, inspect,
move, modify, delete, publish, retune, retry, or rerun any consumed BNCI
private payload, derivative, prediction, target, score, key, marker, or receipt.

The fresh Dreyer design freezes exactly 60 participants, R1/R2 only, 120 source
EDFs totaling 1,779,763,388 declared bytes, 4,800 expected trials, 60 isolated
held-out-person folds, central EEG residualized against EOG, wrist EMG,
posterior EEG, and timing, matched derangement and cue controls, and one compact
source-calibrated L2 logistic family. R3-R6 remain untouched.

Generated Stage G passed once and is consumed: 330 fits, 258 inference runs,
102 prediction sets, 2,040 prediction rows, one synthetic target delivery, and
one synthetic score over 197,632 generated bytes in 2.448583166929893 seconds
at 218,497,024-byte peak process-tree RSS. Generated Stage H also passed once
and is consumed: two valid deterministic header replays and 18 adversarial
refusals over 194,048 generated bytes in 0.01751233299728483 seconds at
69,681,152-byte peak RSS. Both have zero scientific value and zero real-data
operation.

All-false request `5191e79e2ccfa50b5042a24357c0c22d68d8f088` passed Base
job `98074322914`, Optional Neuro Readers job `98074322739`, and CI
...
rerun, repair in place, or reinterpret R0 as accepted. A generated-only
successor later qualified under a new exact packet and decision; its sole H-L2
attempt consumed at transport H0 before body streaming and is also closed.
...
release, touch another project, or upgrade a claim under this gate. An H1
preflight permits only a future narrowing amendment; H0 permanently parks the
lane. Either outcome consumes the one H-L2 invocation.

Current scientific boundary: the project has a real held-out BNCI score with
above-chance and above-timing candidate EEG plus a small directional increment
over EOG, but its preregistered EEG-specific and consistency gates failed. No
fresh Dreyer EEG has been accessed, and no unseen-person, EEG-beyond-EOG,
...
and its exact result is remotely green. Read, in order:
...
Exact Stage A result `96d7f0569a54b05f8031d2e3943658ef598e38a5`
passed Base job `97700176631`, Optional Neuro Readers job `97700176787`, and CI
`32814564120`.

The Stage Q generated semantic core has passed once and is consumed. It read
40,069,562 generated bytes, recognized three calibration structs and six task
runs / 288 trials, and emitted a deterministic 644,708-byte target-free archive
in 3.6570341249462217 seconds at 498,434,048-byte peak RSS. Every private,
real, network, model, training, prediction, delivery, and score counter was
zero. Its original provisional live entry point is permanently fail-closed.

The additive Stage Q live control plane is prepared with one-copy target-free
participant/session shards, fold-scoped delivery manifests and source targets,
zero held-out-T delivery, authenticated held-out-E target envelopes, and
scoring keys outside every fold capability tree. It requires the 227,843,968-
...
The request binds 20 tracked VR38A/VR33A artifacts totaling 294,409 bytes and
...
registration `aa4c39a5ce8ca04627c9252600971ee878f20e3e` passed Base job
`97203738713`, Optional job `97203738637`, and CI `32643351246` before
implementation.

The 20-path pass produced four each of G1/G2/R1/R2/R3, eight successful
selection and validation calls, 99 direct refusals, exact replays, 20 source
immutability checks, and zero non-target selected rows. Exact-task projection
removed a generated mixed-task surplus and reproduced the baseline semantic
cohort while preserving genuine target-task surplus and deficit routes. It
processed 8,836,768 generated bytes in 1.2351410000119358 seconds at
36,044,800-byte peak RSS, emitted 2,950 aggregate bytes, and retained zero.

VR35A proof closeout `6744568115f1f4871b68f4605d14732bf3af38f8`
passed Base job `97209528132`, Optional job `97209528186`, and CI
`32645704669`. The next gate is all-false `MARC2-VR36P`. Read
...
then read the request proof closeout and its machine proof.

The request proposes a generated/mock fixed-path wrapper, then only after its
implementation and proof-only closeout are remotely green one exact-readiness,
one-open, 418,755-byte target-free structural read and one VR35A call. G1/G2
may freeze one 16-subject, 96-bundle, 384-row private cohort; R1/R2/R3 and
readiness/output failures freeze none. Public output may expose only bounded
...
The generated-only `MARC2-VR37A` implementation has now passed its sole
qualification. Registration `a677e7abd2b89e92bb7bcc3f823a3493c6a32ad0`
passed Base job `97226913287`, Optional job `97226913421`, and CI
`32652807264` before implementation. Read the exact-task surplus decomposition
preregistration, contract, implementation document, implementation registry,
result registry, and matching tests.

All 24 paths passed with G1 and R1-R5 each appearing four times, exactly 24
unchanged VR35A calls, 87 direct refusals, exact replay, and 24 source
immutability checks. The pass processed 10,606,968 generated bytes in
2.4954585829982534 seconds at 49,266,688-byte peak RSS, emitted 2,980
aggregate bytes, and retained zero. Qualification may not be repeated.
...
Read the proof-only closeout. It binds the preproof registry hashes and exact
...
`32670514251` before implementation. Read the preregistration, implementation
document, contract, implementation registry, result registry, and five matching
test modules.

The sole generated qualification passed all 40 paths with route counts
G1/G2/R1/R2/R3 = 4/16/8/8/4. All 20 accepted paths shared one complete
structural selection identity and one selected-name identity: 16 generated
subjects, 96 bundles, and 384 core members using only exact task
`reachingandgrasping`, sessions 1/2, and runs 1-3. All 101 direct refusals
passed. Runtime was 7.651170833967626 seconds at 50,528,256-byte peak RSS over
17,674,816 generated bytes; aggregate output was 3,606 bytes and retained
output was zero. Qualification may not be repeated.

Exact implementation `7ef4a8dface0c2a00e27b38f1f91b4043c12535f` passed Base
job `97275279259`, Optional job `97275279380`, and CI `32672478625`. Read
`docs/MARC_2_SELECTION_SUFFICIENCY_REPAIR_PROOF_CLOSEOUT.md` and its matching
proof test. The closeout binds both preproof registry hashes, every exact
...
unchanged VR25A once, and called the unchanged VR2 eligible filter once. It
consumed at R1 in 10.091467417078093 seconds at 30,752,768-byte peak RSS,
with zero network and new-payload bytes.
...
private value, row, path, identity, participant, selection, and cohort were
not retained and must not be inferred. The later participant-session
distribution check was not reached, so its outcome is unknown.

VR30P has no retry, rerun, resume, repair, fallback, substitution, cleanup,
amendment, output inspection, or private reinspection. Do not touch its
private source, readiness state, consumed marker, or output root. The next
safe task is a separately frozen artifact-only and generated-only direction
discriminator for below- versus above-expected eligible totals. Any new
private read remains a new Tier C packet and fresh decision. FW2/CIL1,
archive members, neural data, targets, models, scores, streams, devices,
releases, and claims remain closed.
...
implementation. Generated route `MARC2VR29A-G1` passed all 32 paths with 32
unchanged VR25A calls, 16 additional unchanged VR2 eligible-filter calls,
G1/G2/R1/R2/R3 counts 4/4/8/8/8, 77 direct refusals, exact replay, order-
invariant route distribution, source immutability, and zero retention. Exact
implementation `2e73c9176d243b5deccbf8416bb59fdf053ba762` passed CI
`32620018855`; exact proof-only closeout
`80badb9c1410c1661403aae966b1ea31fa0a45f1` passed Base Python job
`97148293434`, Optional Neuro Readers job `97148293619`, and CI
`32620685817`. VR29A has no private executor and does not identify the consumed
private R1 subclass. Do not touch VR28P private source, readiness, consumed
marker, or output.
...
Exact decision `718c3de6ddb0030b1ba39fa0e42250e97db01072` passed Base
Python job `97133595196`, Optional Neuro Readers job `97133595235`, and CI
`32614796767` before Stage 1. Read
`docs/MARC_2_INVENTORY_TAXONOMY_PRIVATE_DISCRIMINATOR_IMPLEMENTATION.md`,
the implementation and result registries, the additive module, and its three
test modules. One generated qualification passed all 20 fixed-path cases with
20 source opens, 20 unchanged VR25A calls, 20 VR27A map calls, expected
G1/R1/R2 counts 4/12/4, deterministic replay, marker-before-open ordering,
and 110 direct refusals. The pass processed 8,836,136 generated bytes in
1.097564124967903 seconds at 34,177,024-byte peak RSS, reached 785 peak
incremental output bytes, and retained zero.

Exact initial implementation `06174e1456c57c050c48f5b1bce9b276629e2f25`
...
The proof-only closeout binds the 7,743-byte preproof implementation registry,
3,580-byte result registry, their exact hashes and Git blobs, the canonical
...
The current lane is `MARC2-VR23A`. Read its preregistration, implementation
document, and three machine records first. Registration
`cee91b0473cd97a91feab22d7fd420e0b550b99f` passed Base job `97087038676`,
Optional job `97087038522`, and CI `32596045581` before implementation.
Generated `MARC2VR23A-G1` passed all 24 paths: G1 and R1-R5 each appeared four
times, unchanged VR20A was called 24 times, diagnostic/upstream disagreements
and source mutations were zero, and 62 direct refusals passed. Static proof
bound two VR20A F06 wrapper sites and seven VR2 helper reasons, proved two
defensive guards non-independent, and left five independently reachable F06
classes. The pass processed 10,603,766 generated bytes in 2.412034 seconds at
39,944,192-byte peak RSS and retained zero output.

Exact implementation `9e1b12139ad9cd9bcd2245a1eb74b85d7a3cbeeb` passed Base
job `97089462251`, Optional job `97089462366`, and CI `32596999769`.
Proof-only closeout `7d6b6c59709cf069e5b119845565bb91d1f3303b` then passed
Base job `97090919812`, Optional job `97090919708`, and CI `32597604907`
without repeating qualification or touching private state.

The next all-false gate is now prepared as `MARC2-VR24P`. Read
`docs/MARC_2_F06_PRIVATE_FIVE_ROUTE_DISCRIMINATOR_AUTHORIZATION_PACKET.md`
and
`registries/marc2_f06_private_five_route_discriminator_authorization_request.v0.json`.
It binds 16 tracked inputs totaling 224,299 bytes and requests only a future
...
unchanged two-stage packet. Exact decision
`53bc114db9a51a256cbf86d470f24397a50f12b2` passed Base job `97096723985`,
Optional job `97096724073`, and CI `32599996090` before Stage 1. Read the
VR24P implementation document, module, machine record, and tests. Generated
`MARC2VR24P-G1` passes all 24 paths: G1 and R1-R5 each appear four times, with
24 exact VR23A calls, 24 nested unchanged VR20A calls, deterministic replay,
and 130 direct refusals. The pass processed 10,603,766 generated bytes in
3.135488 seconds at 48,152,576-byte peak RSS, reached 3,038 peak temporary
bytes, and retained zero. No cohort manifest is possible.

Exact Stage 1 implementation `a2e19ba59d989ca27cada275ac460085cb07536c`
passed CI `32601257065`; proof-only closeout
`589b0f51db9a1bd157d90b0f4260fbb9c6475045` passed Base job
`97101585701`, Optional job `97101585762`, and CI `32602001717` before the
sole invocation. Read
`docs/MARC_2_F06_PRIVATE_FIVE_ROUTE_DISCRIMINATOR_RESULT.md` and its machine
result. The executor read and strict-parsed exactly 418,755 target-free bytes
once, called VR23A once with one nested VR20A call, and consumed at
`MARC2VR24P-R2` without a cohort freeze. R2 means only complete-bundle count
arithmetic against the public 238-bundle assumption. The observed count,
difference, predicate, value, row, identity, selection, and cohort were not
retained. Do not retry, reopen, rehash, list, or inspect VR24P private state.
The next safe task is artifact-only/generated-only complete-bundle arithmetic
analysis. FW2/CIL1, archive members, neural data, targets, models, predictions,
...
and F07 to R2. Registration `7ce0e16392fed2576031766bead32a5cab44031a`
passed Base job `96998477692`, Optional job `96998477649`, and CI
`32559365362` before implementation. Read the VR21A implementation document
and machine records. Generated `MARC2VR21A-G1` passes all 12 paths, with four
copies each of G1/R1/R2, 12 unchanged VR20A calls, 48 direct refusals, exact
replays, and zero retention. The pass used 5,301,432 generated bytes in
0.789015 seconds at 32,636,928-byte peak RSS. Exact implementation
`661b18d896eef93cd1135c780b4cdca2e7917d04` passed Base job
`97000708150`, Optional job `97000708047`, and CI `32560284291`. Read
`docs/MARC_2_R5_TWO_ROUTE_DISCRIMINATOR_PROOF_CLOSEOUT.md`; exact closeout
`3aa166a5ce7a68ed60ee70786ab0892194fbd6d4` passed Base job
`97002244081`, Optional job `97002244114`, and CI `32560933310` without
repeating qualification or touching private state. The next all-false gate is
...
two-stage packet. Exact decision `197f253e9e0411085f6ecdccf466f9f7059bd479`
passed Base job `97080176920`, Optional job `97080176717`, and CI
`32593234295` before Stage 1. Read the VR22P implementation document, machine
record, module, and tests. Generated `MARC2VR22P-G1` passes all 12 paths: four
each at G1/R4/R5, 12 exact VR20A calls, eight conditional VR21A map calls, and
121 direct refusals. The pass processed 5,301,432 generated bytes in 0.867054
...
Portable activation `a9ebef4fb7cafdd281cfa1c4034a63ddcd08f0a1` passed Base
Python job `96694803139`, Optional Neuro Readers job `96694803152`, and CI
`32456531938` before the sole invocation. The executor collected three fresh
readiness samples, opened and strict-parsed exactly 418,755 target-free
structural bytes once, called VR15A once with one nested unchanged VR12A call,
and consumed at `MARC2VR15P-R15`.

The frozen R15 ceiling is only the run-token width class: the real structural
source does not satisfy VR12A's one-or-two-ASCII-digit `run-<index>`
assumption. The exact token, filename, path, row, identity, participant,
selection, and cohort were not retained and must not be inferred. Runtime was
10.096426583011635 seconds at 29,016,064-byte peak RSS; input was 418,755
bytes, combined output was 2,288 bytes, and network/new-payload bytes were
zero.
...
`32003674374` before implementation. Now read
`docs/MARC_2_F03_FIVE_ROUTE_DISCRIMINATOR_IMPLEMENTATION.md`,
`docs/MARC_2_F03_FIVE_ROUTE_DISCRIMINATOR_RESULT.md`, and their registries.
Local `MARC2VR10B-G1` sends the six exact generated cases through 24 parser/
producer paths, 24 VR6 calls, and 24 discriminator calls. Each of G1 and R1-R5
appears exactly four times across two orders and two replays. The pass made
29,448 parser-entry visits, passed 60 direct refusals, processed 6,979,708
generated bytes in 2.725759166991338 seconds at 44,564,480-byte peak RSS,
emitted 7,515 aggregate bytes, and retained zero. Thirty-two focused and all
4,074 base tests pass with 204 expected skips. Exact implementation/result
`61bb801689eb2885b1e96aa4b56c86658dc3b333` passed Base Python job
`95320325187`, Optional Neuro Readers job `95320325136`, and CI
`32007641751`. No qualification or private operation was repeated for proof
closeout.
VR10B is artifact-only and generated-only. It has no private executor and
authorizes no private read, consumed-state reuse, archive payload, signal,
event, target, model, training, prediction, score, FW2/CIL1, network, provider,
...
two-stage packet. Decision `4fa277121f24dde3f6f7c917ef6c2bb7506134d6`
passed Base Python job `95414004791`, Optional Neuro Readers job `95414004814`,
and CI `32038683203` before Stage 1 began.

Read `docs/MARC_2_F03_PRIVATE_DISCRIMINATOR_IMPLEMENTATION.md`,
`registries/marc2_f03_private_discriminator_implementation.v0.json`, and
`tests/test_marc2_f03_private_discriminator.py`. The additive standard-library
wrapper does not import or call consumed VR9P. Generated `MARC2VR11P-G1`
completed 24 exact VR6/VR10B paths and one temporary fixed-path state-machine
fixture; every G1/R1-R5 route appeared four times. The measured pass processed
7,398,463 generated bytes in 3.424091666995082 seconds at 63,619,072-byte peak
RSS, emitted 7,268 aggregate bytes, retained zero, passed 81 direct refusals,
and kept every forbidden counter at zero. The clean base suite passes 4,117
...
rank, and 8 GiB cap. Generated `MARC2VR12A-G1` passed all 12 success paths and
36 direct refusals over 5,147,208 generated bytes in 1.092868332983926 seconds
at 44,417,024-byte peak RSS with zero retention and every private or scientific
counter zero. Exact implementation `873484aaf270bc5b1499e4b0449c9e8ef138c623`
passed Base Python job `95819297085`, Optional Neuro Readers job `95819297010`,
and CI `32170217284`. Read the implementation and result docs and registries.
The proof-only closeout must itself be remotely green before one all-false Tier
C structural-confirmation packet may be prepared. Do not rerun qualification,
touch any private/ignored source, or infer that an unpadded value caused P15.
...
Decision `fe16400fd0ccb5fa2ff40fffd413fee34eb620d6` passed Base Python
job `96648078587`, Optional Neuro Readers job `96648078452`, and CI
`32439821302` before Stage 1. Read
`docs/MARC_2_R4_PRIVATE_DISCRIMINATOR_IMPLEMENTATION.md`,
`docs/MARC_2_R4_PRIVATE_DISCRIMINATOR_GENERATED_RESULT.md`, the implementation
registry, module, and two matching tests. Generated `MARC2VR13P-G1` passes 32
matrix paths plus one temporary fixed-path success path, calls VR12A 33 times
and the VR13A residual map 28 times, and passes 81 direct refusals. The
measured pass processed 14,171,146 generated bytes in 3.208020000020042
seconds at 34,324,480-byte peak RSS, emitted 2,087 aggregate bytes, and
retained zero. Every private, archive, neural, target, model, score, network,
FW2/CIL1, other-project, and claim counter is zero. The implementation record
keeps `remote_implementation_proof` null, so execute refuses before readiness
...
`96661242381`, Optional job `96661242496`, and CI `32444425790` before Stage
1. Read the VR14P implementation and generated result docs, implementation
registry, module, and two matching tests. Generated `MARC2VR14P-G1` passes 32
route/order/replay paths and 89 direct refusals over 50,370 generated bytes in
0.008616000006441027 seconds at 28,901,376-byte peak RSS with zero retained or
ignored-path output. Execute now requires explicit one-shot arming plus a
tracked, clean implementation-proof registry, preventing ordinary tests from
...
## Latest MARC2-VR16A Boundary

VR15P is consumed at aggregate route `MARC2VR15P-R15`, the run-token width
class. It permits no retry or reinspection and retained no token, filename,
path, identity, participant, selection, or cohort. Read
`docs/MARC_2_VARIABLE_WIDTH_RUN_INDEX_PRIMARY_SOURCE_RESEARCH.md`, the VR16A
preregistration, implementation, result, and their registries.

Registration `7dba59355ca45c8ab5eafb9d8b7757edfc9755c5` passed Base Python
job `96699811237`, Optional Neuro Readers job `96699811051`, and CI
`32458280634` before implementation. Generated `MARC2VR16A-G1` passes six
numeric width patterns in two orders and two replays: 24/24 paths, 50 direct
refusals, one semantic cohort digest, six distinct source and selected-name
hashes, and source-exact reservation replay. The measured pass processed
17,532,166 generated bytes in 2.224372999975458 seconds at 34,717,696-byte
peak RSS and retained zero.

VR16A is generated-only. Exact implementation `6f92b84c7be67848c7d09b567f13b08a14d33f5c`
passed Base job `96704807926`, Optional job `96704808178`, and CI
`32459984049`. It proves the adapter mechanics, not that the real source
passes. Tier A may prepare one all-false private-confirmation packet. A real structural read,
...
`32467147580` before one bounded generated canonical-order preflight. Read
`docs/MARC_2_VARIABLE_WIDTH_R4_DECOMPOSITION_RESULT.md` and
`registries/marc2_variable_width_r4_decomposition_result.v0.json`. The
preflight falsified literal H3: `unpadded` and `two_digit_control` already pass
VR15A and VR16A, while `three_digit`, `six_digit`, `sixty_four_digit`, and
`bundle_consistent_mixed_width` move from VR15A R15 to VR16A success. All six
preserve semantic digest
`254bca5e0a39b52ca9791b917df6af554e02dda8f18f8e01b524d0e3ce8d9cba`.

VR17A is parked at `MARC2VR17A-P01` without amendment. Its full 24-path
equivalence and 20-path residual matrices did not run. The preflight used
2,651,670 generated bytes in 0.6208636660012417 seconds at 33,390,592-byte
peak RSS, retained zero output, and performed zero private or scientific
operation. The next safe task is a separately frozen corrected `MARC2-VR17B`
registration with two G1-to-G1 controls and four R15-to-G1 repairs. Do not
rerun or reinterpret VR17A, touch consumed VR15P/VR16P state, or infer a real
...
`32469173279` before a five-case generated residual preflight. Read
`docs/MARC_2_CORRECTED_VARIABLE_WIDTH_R4_DECOMPOSITION_RESULT.md` and its
result registry. Two frozen expectations failed: wrong-task returned
VR16A F04 / `core identity differs`, and the existing duplicate-companion
witness returned F05 / `companion run spelling differs` before reaching the
normalized-collision branch. Control, mixed spelling, and incomplete-set cases
matched.

VR17B is parked at `MARC2VR17B-P01` without amendment, retry, or full matrix.
No implementation module was added. The probe made five VR16A calls, zero
VR15A calls, and zero private operations; generated bytes and peak RSS were not
instrumented and are explicitly unavailable. The next safe task is a separate
VR17C registration with F04 / `core identity differs` and a distinct optional-
...
`32470828824` before implementation. Read
`docs/MARC_2_FIRST_FAILURE_STABLE_R4_DECOMPOSITION_IMPLEMENTATION.md`,
`docs/MARC_2_FIRST_FAILURE_STABLE_R4_DECOMPOSITION_RESULT.md`, and their two
registries. Generated `MARC2VR17C-G1` passes 24 paired equivalence paths, 20
residual paths, all four hypotheses, and 50 direct refusals. G1 and R1-R4 each
appear four times under two source orders and two exact replays.

The measured qualification processed 19,213,944 generated bytes in
3.7416582500445656 seconds at 36,569,088-byte peak RSS, emitted 2,719
aggregate bytes, and retained zero. Twenty focused and all 4,576 dependency-
light tests pass with 204 expected skips. Every private, consumed-state,
archive, neural, target, model, score, FW2/CIL1, network, hardware,
other-project, release, and claim counter is zero.

The immediate gate is exact implementation/result remote proof followed by a
separately green proof-only closeout. Do not repeat qualification or touch
`.codex_work`, consumed VR15P/VR16P, parked VR17A/VR17B, or the unrelated
tracker inspection NDJSON during closeout. No private discriminator, cohort,
...
`32475765286` before Stage 1. Read
`docs/MARC_2_FIRST_FAILURE_STABLE_PRIVATE_DISCRIMINATOR_IMPLEMENTATION.md`,
`docs/MARC_2_FIRST_FAILURE_STABLE_PRIVATE_DISCRIMINATOR_GENERATED_RESULT.md`,
and their implementation/result registries. Generated `MARC2VR18P-G1`
completed 20 exact matrix paths and one temporary fixed-path success rehearsal.
G1 and R4-R7 each appeared four times across two orders and two replays.

The measured pass made 21 generated VR16A calls, 16 failure-map lookups, and
82 direct refusals. It processed 9,037,650 generated bytes in
1.4131330830277875 seconds at 37,879,808-byte peak RSS, reached 217,041
temporary output bytes, and retained zero. All 4,620 dependency-light tests
pass with 204 expected skips. Every private, ignored-path, archive, neural,
target, model, score, FW2/CIL1, network, hardware, other-project, retry,
release, and claim counter is zero.

The implementation registry deliberately retains
`remote_implementation_proof: null`; `execute` refuses at F01 before readiness
or private preflight. Commit, push, and green the exact implementation/result,
then add and green a separate proof-only closeout without repeating
qualification. Only after both barriers pass may the one registered Stage 2
...
`32480420157` before implementation. Read
`docs/MARC_2_F04_TASK_IMPLICATION_IMPLEMENTATION.md`,
`docs/MARC_2_F04_TASK_IMPLICATION_RESULT.md`, and their registries. Generated
`MARC2VR19A-G1` passes all four hypotheses, 32 exact VR16A paths, and 40
direct refusals. Route counts are G1=4, R1=16, and R2=12; both replays match.

The pass processed 13,748,732 generated bytes in 1.759938333008904 seconds at
51,429,376-byte peak RSS, emitted 2,326 aggregate bytes, and retained zero.
All 4,649 dependency-light tests pass with 204 expected skips. The exact
consumed R4 pair now implies a non-lowercase-`freewill` task class under bound
VR16A semantics, but the private token and every private identity remain
unknown.

Commit, push, and green this exact implementation/result, then add a proof-
only closeout without repeating qualification. Do not touch `.codex_work`,
VR18P consumed state, archive or neural payload, targets, models, scores,
...
`32484725113` before implementation. Read
`docs/MARC_2_PUBLISHED_TASK_SELECTOR_REPAIR_IMPLEMENTATION.md`,
`docs/MARC_2_PUBLISHED_TASK_SELECTOR_REPAIR_RESULT.md`, and their registries.
Generated `MARC2VR20A-G1` passes 20 source-exact paths, two exact replays, and
53 direct refusals. Every path selects 16 subjects, 96 bundles, and 384 core
members while preserving the exact `task-reachingandgrasping` names.

The pass processed 17,273,948 generated bytes in 1.7907995419809595 seconds at
35,143,680-byte peak RSS and retained zero. Commit, push, and green the exact
implementation/result, then add a proof-only closeout without repeating
qualification. Do not touch `.codex_work`, VR18P consumed state, private
values, archive or neural payload, targets, models, scores, FW2/CIL1, other
