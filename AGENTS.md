...
The implementation now covers strict no-retry S3 acquisition, 128-run
BrainVision/event/ball/Leap reconciliation, the causal 0.5-4 Hz feature path,
the dual-target firewall, 300 fixed fits, 420 target-blind prediction sets, a
strict aggregate freeze, and isolated scoring. One generated 128-run
qualification passed in 8.674020 seconds at 263,618,560-byte peak RSS with
5,683,285 generated bytes; all files were removed. Real IACKD metadata,
payload, local path, EEG/EOG, event, trajectory, target, model, and score
counters remain zero. Commit, push, and obtain both green CI jobs for this
exact implementation before the single 7,249,113,684-byte acquisition. The
dual final targets remain closed until the later prediction-freeze commit is
remotely green. Do not rerun or expand the registered sequence.
